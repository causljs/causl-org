...
5. **I will ship the smallest thing that earns the right to grow.** The public Graph surface is a single-digit canonical set in §12.1 plus a curated second tier in §12.2: **twenty-one public Graph members today (six canonical members plus fifteen second-tier; the seventh canonical entrypoint, `createCausl`, is the construction factory, not a `Graph` member), audited quarterly.** GraphQL, Postgres and the rest of the deferred list are not phases of this spec; they live in §13 with explicit reopen triggers or they do not appear at all. The Rust core is no longer among them: it graduated through §13.8's gate and is the engine this package ships, compiled to WebAssembly and reached through the thin TypeScript API of §18A.

The brutal-critical review the team runs is real and visible in the closed-PR record (#272 closed the bounded model checker as not-planned; #386, #391, #396, #398, #404 closed the honesty pass; #366 / #378 re-routed `hydrate` through the commit pipeline; #359 / #368 collapsed the IR to two `kind` constants). Every commitment above survived a "name the unavoidable concept the engine cannot express without it" challenge. The ones that didn't are in §13 with a reopen trigger or struck entirely. The "single-digit" promise the previous draft made for §12 was one of the casualties: the team's read was that §12.2's fifteen second-tier rows are also load-bearing, that the canonical seven plus second-tier fifteen is the honest count, and that pretending otherwise reproduces the drift PRs #266, #395, #420 kept fixing. The §0 surface promise is now the two-tier framing the team will defend.
...
I ship one engine. It is the Rust core in `causl/causl-core-rs`, compiled to WebAssembly and reached through this package's thin TypeScript API layer (§18A); `createCausl()` returns that engine or throws (§18A.13.1). The TypeScript implementation that once shipped beside it lives on only in `causl/causl-core-ts`, as the org's reference floor and as this spec's conformance oracle (§18A.1.1). Where this document pins a behaviour, it pins the behaviour of that one engine; §18A.13 records how the roster got here.
...
- A spreadsheet engine. I will support spreadsheet *patterns* on top of the core through `@causl/formula`; I will not ship VLOOKUP.
- A database. There is no query language, no index, no on-disk storage format. The commit log is a bounded ring buffer in process memory; persistence is an adapter (`@causl/persistence`) that snapshots inputs at chosen commits.
...
- A virtual DOM, a renderer, or any kind of view layer. `@causl/react` translates engine commits into `useSyncExternalStore` snapshot semantics; the rendering decision is React's, not the engine's.
...
| **E**: Publish derived deltas | Assemble the immutable `Commit` record (changed nodes, intent string, `originatedAt?: GraphTime`, `time`). `originatedAt` is present only on privileged-caller records: `undefined` on plain `commit` records, `snap.time` on `hydrate` records (§5.3); its presence is the normative hydrate discriminator. Frozen identically here whether the caller is `commit`, `hydrate`, or any future privileged caller. |
...
§5.1 Amendment 4 (#1333): Phase G subscriber container invariant (post-FFI amendment). The per-node subscriber index; i.e. the mapping from `NodeId` to the ordered list of observer ids that receive Phase G fires; MUST preserve **insertion order**. In implementations using ordered containers (`IndexMap<NodeId, SmallVec<[ObserverId; 2]>>` in Rust, `Map<NodeId, Array<ObserverId>>` in JavaScript), insertion order is preserved by the container itself. Implementations using hash-based containers whose iteration order depends on hash seed (`HashMap`, `FxHashMap`, `hashbrown::HashMap`) are **non-conformant for the byte-identity gate** because hashbrown's iteration order changes between minor crate versions; the engine's cross-backend determinism contract requires deterministic iteration. Implementations using sorted containers (`BTreeMap` sorts by id, losing insertion semantics) are likewise non-conformant. Subscriber fire order is: insertion-order per node, then `changedNodes`-insertion-order across nodes; sorted-by-id and hash-iteration containers violate both axes. The shipped Rust engine satisfies the invariant through the parallel order-bearing structures §18A.1.1's amendment names (the per-cell insertion-ordered observer list plus `subscriptions_in_order`); the TypeScript floor this package used to ship satisfied it through JS `Map`'s spec-mandated insertion-order iteration. The amendment was written ahead of the port so no native backend could ship without honouring it. Landed standalone-value post-STOP-VERDICT on epic #1133 (see §19 trail row and `docs/epic-1133/HANDOFF.md`).
...
§5.1 Amendment 5 (review 2026-07-06): Phase-G/H observable environment and observer-error policy. Two facts the shipped engine enforces (and the reference floor in `causl/causl-core-ts` agrees on), previously unpinned by §5.1:
- **Observable environment.** A Phase G (per-node) or Phase H (commit-level) observer frame observes **post-F.6** state: `now` is the *new* GraphTime, `graph.commitLog` already includes the current commit, and `readAt(·, now)` resolves. This ordering is normative because it once diverged in practice; under the `rust-ssot` backend the wasm observer fan runs before the TS-side Phase F.6 retention, so an in-frame `readAt(derived, now)` disagreed between backends until #86 introduced `inBackendDispatchWindow` to force the post-F.6 view. A conformant backend MUST present the post-F.6 environment to observer frames.
- **Observer-error default.** A throwing observer is **caught**, reported via the `onObserverError` hook (`createCausl({ onObserverError })`; the §6.2 ObserverError sub-region §5.3 names its `ObserverErrorContext.source` labels), and dispatch **continues** with the remaining observers. The default policy when no hook is supplied is **report-and-continue** (`defaultOnObserverError`): a subscriber throw does **not** prevent the remaining observers from firing and does **not** fail the commit; `commit()` does not throw on an observer error. This continue-on-error semantics is required by the §18A.1.1 fire-order conformance surface: an engine that aborted the fan on the first throw would produce a different `changedNodes` observation stream and diverge from the reference traces.

§5.1 Amendment 6 (review 2026-07-06): Phase-D dynamic-dependency re-capture and rollback. The engine-level dynamic-dep contract (§9.1 row 7, §10.3's worked example) is pinned here at the phase level:
- **Re-capture every recompute.** Each Phase D recompute of a derived node **atomically replaces** that node's dependency set with the reads observed on *that run*: deps are re-captured on every recompute, not merely when a value changes. Stale deps that were not read this run drop out of the set; fresh deps read for the first time wire up. This is the mechanism behind §9.1 row 7's "stale deps drop AND fresh deps wire up."
- **Rollback restores dep edges too.** A throw escaping Phase D (or Phase F.5) lands in the §5.2 catch arm, which restores the pre-commit dependency sets **alongside** the derived values, `commitHistory`, `commitLogEntry`, and `now`. §5.2's enumeration names derived *values*; this amendment makes explicit that the node's dep *edges* are part of the same byte-identical restoration: a partially-rewired dep set is never observable after a failed commit.
- **`conditionalDeps` in the IR.** The IR's `IRDerived.conditionalDeps` field (§16.2.1) is **reserved-empty** in the shipped exporter, which emits `[]` unconditionally (the retired TypeScript floor's exporter did the same); the `UnknownDep`/`Cycle` checker passes that consume it are therefore vacuous on that field today. It is retained as a wire slot pending a populator; see the §16.2.1 "Current state" note.

§5.1 Amendment 7 (review 2026-07-06): the derived-value equality cutoff relation, pinned. §5.1 Phase B pins `Object.is` for **input** writes, but §5.1/§3 never stated which equality gates a **derived** node "changed" in Phase D, and the two engines that shipped when this amendment was written answer differently: the TypeScript floor (shipped today only by `causl/causl-core-ts`) uses `!Object.is(before, e.value)` (reference identity), while the `rust-ssot` engine, the one I ship, cuts a derived container off by a deterministic structural content-hash. A derived that returns a fresh-but-structurally-equal container therefore fires subscribers on **every** commit under the TS floor and **once** under the wasm engine; a `changedNodes` / fire-set divergence on a §18A.1.1 MUST-be-identical surface. Normative rule: a derived node counts as **changed** in Phase D **iff** its recomputed value differs from its previous value under the **canonical value-equality relation**, defined as structural deep-equality over the **shipped wire value domain**; the canonical JSON value the §18A.3 codecs cross (`encode_value_record`; §18A.3 "Wire-format discipline"): Null / Bool / Number / String / Array / Object, with `Set` / `Map` / `Date` / `Temporal` already replaced by tagged plain containers before the boundary (`packages/core/wasm/tagged-types.ts`). The domain is **not** §16.4.1's `State`: that section's own opening note marks it enumerator-model-only, "not the shipped engine's runtime state", so it cannot supply the domain a shipped-engine rule ranges over. Scalars compare as **ECMA-262 SameValue**; `Object.is`, with **no** carve-out: any two NaNs compare **equal**, `+0` and `-0` compare **distinct**. An earlier draft of this amendment asserted the opposite for signed zero; neither shipped engine implements that carve-out and neither ever did. The Rust engine's `is_same_value_f64` bit-compares non-NaN doubles precisely so the two zeros stay distinct (`causl/causl-core-rs` `tools/engine-rs-core/src/transition/publish.rs`), the JS-side canonicaliser this repository ships hashes them to different bytes (`packages/core/wasm/content-hash.ts`), and the TS floor's `Object.is` agrees with both. Containers (Array / Object / tagged) compare element-wise recursively, and the `rust-ssot` derived-cutoff marker (its content-hash) is a conformant O(1) encoding of this relation. `Commit.changedNodes` and the Phase-G/H fire-sets are derived from this relation; the shipped engine derives them through the content-hash marker, and a conformant floor must derive the same sets (the cross-repo finding above is exactly where the sibling floor still fails to). **Cross-repo finding, still live.** The TS floor's `Object.is`-only cutoff for container-typed deriveds is **NON-CONFORMANT** and MUST be promoted to structural cutoff. That finding does not describe this package's engine: since 0.5.0 the floor is shipped only by `causl/causl-core-ts`, so the clause binds that repository, its own SPEC is where the repo-local disclosure duty now lives, and promoting the cutoff is `causl-core-ts#58`'s work. While the clause bound any repository that ships the floor as a reachable engine, it required §18A.13.1 to disclose that the TS capability-fallback **over-notifies** container subscribers relative to the wasm default, and it **suspended for container-valued deriveds** §18A.12(b)'s "byte-identical per §18A.1.1" justification for `fallbackToTs`; for `causl/causl-wasm-ts` that duty was discharged at 0.5.0 by removal rather than by disclosure (§18A.13.1's capability fallback and §18A.12(b)'s `fallbackToTs` option no longer exist here, so there is nothing left to disclose or to suspend), and the non-conformance itself is **not** discharged by either the withdrawal or this note. This relation governs *cutoff / changed* only; the §15.1/#1124 read-*reference*-identity non-guarantee is unaffected (a container's identity across `read()` calls remains non-contractual). The engine puts this relation to a **second** job that nothing above authorises; eliding the recompute of a derived's transitive dependents; and that second use is bounded by **§5.1 Amendment 8** (#260), which enumerates the quotients the relation is permitted to have, declares that list closed, and states what a change to the relation costs a consumer. Read the two together before treating a cutoff verdict as a licence to skip work.

§5.1 Amendment 8 (#260, EPIC #255 T5): the permitted quotients for computation elision. Amendment 7 pins one equality relation and the engine puts it to **two** jobs whose soundness requirements are not the same. **Notification**, "did this derived move?"; is the job Amendment 7 authorises, and a relation that is merely conservative there costs at worst a suppressed fire. **Computation elision**; "may I skip recomputing every transitive dependent?"; is the job the engine also performs with the same verdict, and no SPEC text ever authorised it; the same conservatism there serves a stale value out of `read()` for the rest of the process. This amendment names that second use and states exactly how far it is licensed. The relation is a **lossy quotient**, permanently and by design, and it is **not** a congruence with respect to the computations it is used to elide. That is stated positively because an omission is not a denial: a reader who finds no disclaimer assumes the property the word names.

> **The permitted quotients, enumerated and closed.** Computation elision is permitted only when the equality verdict was reached by a traversal that **fully observed** both operands: for a plain object that is the **structural own-property walk**: own enumerable string keys, own non-enumerable string keys, and own symbol keys, each folded as a separately canonicalised section (`packages/core/wasm/content-hash.ts`, `TAG_OBJECT_EXTENDED`). The following quotients survive that walk. They are permitted, and are therefore excluded from every elision guarantee:
>
> 1. **Own-key order.** `{a:1,b:2}` and `{b:2,a:1}` reach one verdict.
> 2. **Prototype identity, and everything reached through it: for a value the walk reads at least one own property out of.** A class instance, an `Object.create(null)` bag and a plain object with the same own properties reach one verdict, as does an object whose only difference is an *inherited* enumerable property. **Narrowed by #258 (2026-08-03):** this row no longer covers a value the walk reads *nothing* out of. When a value has no own enumerable string key, no readable own hidden or symbol key, and a prototype chain that does not end directly at an object root, the walk has no own property to reach one verdict *with*; the empty record is all it has, and eliding over it is EPIC #255's defect class through a value class no allowlist can enumerate (a `#private` field, an internal slot, a closure captured by a prototype accessor, or an own accessor the hidden/symbol sections fold as a two-byte descriptor record without calling). Such a value routes to the opaque arm as `KIND_BARE_INSTANCE` and is excluded from computation elision under clause 1 exactly as the allowlisted host types are. `{}`, `Object.create(null)` and a cross-realm `{}` are **not** in the narrowing; an empty plain record is fully representable and keeps its cutoff; and neither is a class instance carrying own fields, which is the dominant adopter shape and is why this narrowing is not the blanket "all non-plain objects" form D04 rejected. Priced below under *the second refinement under the rule*.
> 3. **Property attributes other than enumerability.** `writable`, `configurable` and extensibility are not observed, so a frozen object and its mutable twin reach one verdict. Enumerability itself **is** observed: see the superseded note below.
> 4. **Accessors, read as their value.** An own accessor whose getter returns `1` and an own data property holding `1` reach one verdict. This is a decision, not an oversight: the question the relation answers is *what can the adopter read*, and an adopter can read a getter (#269).
> 5. **Structure sharing.** `{a:s,b:s}` and `{a:{…},b:{…}}` with two equal copies reach one verdict; the walk is structural and records no aliasing.
> 6. **Unregistered symbols sharing a description.** `Symbol.for(...)` keys separate through `Symbol.keyFor`; two `Symbol('dup')` keys have only their description and do not. An unregistered symbol has no serialisable identity, so this is a limit rather than a deferral (#269).
> 7. **Own non-index properties of an array.** They are outside the array arm entirely: enumerable or not. Reaching them means `Object.getOwnPropertyNames` on the array, which materialises one string per index; the allocation is refused on the commit path this module exists to make cheap (#269).
> 8. **A tagged container anywhere in the value.** `Set` / `Map` / `Date` / `Temporal` are replaced by tagged plain containers before the walk (`packages/core/wasm/tagged-types.ts`), and that pass rebuilds the tree from own **enumerable** keys: so it **re-erases** the hidden-key and symbol-key domains for the whole value. Rows 3 and 4 of the superseded list below are therefore closed for a value with no tagged container in it and open for a value with one.
>
> **[Superseded: the original five, four of which #269 closed (2026-08-02).]** This amendment was drafted against the canonicaliser at `67f66a2`, where the permitted list read: own-key **order**; `undefined` ≡ `null` inside a container; non-enumerable own properties **of plain objects**; symbol-keyed own properties; array holes. #269 ("the content hash must see what `read()` serves") closed the last four; `TAG_UNDEFINED` separates `undefined` from `null` inside a container, `TAG_OBJECT_EXTENDED` folds own non-enumerable and own symbol keys into the walk, and `TAG_HOLE` separates an array hole from a dense `undefined`. Stated plainly: **that is a strengthening.** Four value distinctions this amendment was going to license the engine to ignore, it now observes, so an adopter who could watch them change is no longer served a stale value through an elided recompute. The rows survive here only in the narrowed forms above, 6, 7 and 8; which are the residue #269 records rather than closes (`packages/core/test/content-hash-observational-fidelity-269.test.ts`). The original five are preserved as the dated record; the live list is the eight rows above.
>
> Each entry is measured rather than asserted. `packages/core/test/spec-amendment-8-quotients-gate-260.test.ts` runs the shipped canonicaliser (`packages/core/wasm/content-hash.ts`) on a pair of values per entry and fails if the pair stops colliding, so a quotient that closes is reported as a cross-engine event rather than silently dropped from this list; the same file asserts the four closed rows stay closed, so re-opening one is a failing test rather than a regression nobody reads. Own-key order is the clearest case of a quotient that is **not** a defect: the object arm sorts own keys deliberately, for parity with the Rust `is_same_value` object arm's set-equality and `hash_value_into`'s sorted-key fold (`causl/causl-core-rs` `tools/engine-rs-core/src/transition/publish.rs`). `Object.keys(o).join()` is an ordinary observable JS operation the relation is *mandated* to be blind to; closing that quotient would desynchronise the JS-side canonicaliser from the Rust core's sorted-key fold on a §18A.1.1 MUST-be-identical surface. It is not a defect to fix, it is a quotient to declare.
>
> The allowlisted host types: `RegExp`, `Error`, `Promise`, `WeakMap`, `WeakSet` (the D04-narrowed allowlist of #258); do **not** take the structural walk: they route to the opaque arm and are distinguished there. That routing is still owed. Measured on this head, `new Error('a')` and `new Error('b')` already separate; `message` is an own **non-enumerable** property, so #269's hidden-key section reaches it; but `/a/` and `/b/` collide, and a `Promise` and a `WeakMap` both hash as `{}`. The narrowing of rows 3 and 4 of the superseded list to *plain objects* was written because `Object.keys(new Error('a'))` is empty; it is now moot for `Error` and still live for the other four, which is why the opaque arm is the fix rather than a wider walk.
>
> **The sixth opaque class is a SHAPE, not a type (#258).** The five host types above are instances of one criterion: *the structural walk can read nothing out of this value*; and an allowlist cannot be the whole of that criterion, because an adopter can write a value with the same shape and no enumeration can name it in advance. So the domain has a sixth member defined by shape (`packages/core/wasm/opaque-domain.ts`, `isOpaqueBareInstance`; `KIND_BARE_INSTANCE`): **no own enumerable string key, every own hidden or symbol key unreadable to the walk, and; when there is no own key at all; a prototype chain that does not end directly at an object root.** Three shapes satisfy it and all three are gated by `packages/core/test/opaque-domain-shape-clause-258.test.ts`: a class instance whose state is a `#private` field or a prototype accessor over a closure; a value whose prototype is an arbitrary null-prototype bag carrying the state one link up; and a value whose entire own-key surface is own non-enumerable or symbol **accessors**, which the hidden and symbol sections fold as the two-byte `TAG_ACCESSOR` descriptor record without ever calling the getter. The last two are why the clause is stated as *readable* rather than as *present*: an own key that exists and yields nothing to the record is not evidence the walk saw anything. The prototype half is realm-safe by an intrinsic-signature test on the chain root, not by an identity comparison against this realm's `Object.prototype` (D06), so a cross-realm `{}` keeps its cutoff.
>
> The list is **closed**. A value class that introduces a new quotient re-opens this amendment; it does not get to ride the old text. A merely-conservative "looks equal" verdict authorises skipping a **notification**; it does not authorise skipping a **computation**.

**D03, and the three clauses that depended on it.** Clauses 1, 3 and 5 of #260 were drafted in two versions each, because D03: what the cutoff contributes for a slot holding a value outside the wire domain; was reopened as #302. #302 closed on 2026-08-02 choosing option (c), the congruence gate: an opaque-carrying value contributes no marker of its own. The identity-ticket candidate is **withdrawn**; its wording survives only in the issue trail and is non-normative wherever it is found. What ships is:

- **Clause 1: an opaque-carrying value refuses elision.** There is no reference-keyed opaque slot. A value outside the JSON projection does not participate in the equality relation at all, and a node whose value carries one is **excluded from computation elision** and recomputes. Distinguishing two such values from each other is #258's arm; declining to elide over either of them is this clause. **Shipped as of #258** (T3), which is also where the exclusion acquired its one qualification, stated here because the clause would otherwise be read as unconditional: the refusal is conditional on an opaque slot having MOVED. `packages/core/src/graph.ts` compares the opaque payloads the value carries by `Object.is`, at compare time, against the ones its predecessor carried, and elides only when every one of them is the same reference. A blanket refusal is what EPIC #255 §4 rejects as "an unconditional downstream wave forever", and it would break the canonical `useMemo(() => [rule(limit)], [limit])` consumer, whose cutoff `packages/core/test/opaque-cutoff-conformance-255-t1.test.ts` pins as a control. Nothing reference-keyed reaches the MARKER; that was #302's withdrawn option (a) and it stays withdrawn; so the interned set is still bounded by the distinct values rather than by the commit count. Measured on this head, on the shipped engine (`createCauslWasmSync`), on the xldatagrid fan-out `validators → visibleColumns → { validationResults, footerAggregates }`: a memoised `visibleColumns` rebuilt around a genuinely different validator closure reaches the **same** marker as its predecessor; a function carries no content-addressable payload and two closures are byte-equal under `TAG_OPAQUE`; and the derived is nevertheless **not** cut off, because the closure reference moved; `validationResults` recomputes and `read(validationResults)` serves the post-commit value. The paragraph is re-measured against the engine by `packages/core/test/spec-amendment-8-engine-claims-260.test.ts`, which fails if this text claims conformance while the engine elides. Promotion of the remaining arms is the work of #303 / #304 (T4a/T4b), which cover a BARE opaque value in a derived and on an input; this clause covers a value that CARRIES one.
- **Clause 3: the Rust replay-stability contract is UNAFFECTED.** No marker is process-local, because no marker encodes JS heap identity; the marker stays content-addressed under the gate. `mint_change_token`'s guarantee that the same `(slot, gen, value, intent)` yields the same `u64` on every platform and every replay (`causl/causl-core-rs` `tools/engine-rs-core/src/transition/publish.rs`, pinned by its `mint_change_token_is_replay_stable` test) therefore stands unchanged. This is recorded positively rather than dropped silently: `changed_nodes` includes recomputed deriveds, each of whose post-commit value is folded through that function, and the resulting token crosses the FFI, so "the Rust source is untouched by construction" would never on its own have implied "the Rust contract is untouched"; under the withdrawn candidate the contract would have moved without a line of Rust changing. EPIC #255's **OQ2** closes here: replay determinism is not made conditional by this work, no carve-out is needed, and the golden corpus needs no new pin.
- **Clause 5: the marker is taken at commit time.** Nothing is reference-keyed, so nothing is blind to a mutation the structural walk would have seen. What survives is narrower and true on either candidate: the marker is taken **at commit time**, so an in-place mutation of a container still retained by the adopter after that commit is invisible to the next cutoff. That is the form `causl/causl-core-ts#58` reports, and the invariant #268's Merkle sub-hash caching would have to pin before it could reuse a sub-hash across commits.

**The value domain this amendment governs.** Amendment 8 ranges over the same domain Amendment 7 pins, and states it here rather than inheriting it by reference: the shipped **wire value domain**; the canonical JSON value the §18A.3 codecs cross (`encode_value_record`); Null / Bool / Number / String / Array / Object, with `Set` / `Map` / `Date` / `Temporal` already replaced by tagged plain containers before the boundary (`packages/core/wasm/tagged-types.ts`), and the four non-JSON scalar classes (`undefined`, `bigint`, `symbol`, `function`) sitting outside it (`packages/core/src/value-domain.ts`). It is **not** §16.4.1's `State`, whose own opening note marks that section enumerator-model-only; anchoring a shipped-engine rule there is the defect #274 removed from Amendment 7, and re-introducing it one amendment later would make this restriction unenforceable again for the same reason.

**A refinement of the relation costs engine work, and the cost MUST be measured.** Computation elision exists to avoid recomputing transitive dependents, so any amendment that refines this relation increases engine work by construction. A refinement MUST ship a **measured** recompute-volume delta on a representative adopter fan-out, not a correctness witness alone. The measurement is a **count of dependent recomputes**, not a wall-clock latency: a count is exact, host-independent and re-derivable on every test run, whereas the wall-clock cells in `packages/core/bench/baseline.json` refuse to capture above `(load1 - 1) / cores > 0.2` and would have shipped `null` here. A per-call `contentHashMarker` hashing latency is **not** a recompute-volume delta and does not discharge this clause. Without the measurement, every future tightening lands on correctness grounds and the cost is discovered by an adopter.

**The first refinement under the rule, priced here rather than exempted.** #269 is a refinement by this amendment's own definition: four value classes newly distinguished; and it lands in the same change as the clause above, so the clause is discharged on its own first instance instead of starting life with an exemption. Measured on a 103-derived adopter fan-out (one memoised container → `visibleColumns` → 100 cell deriveds → one summary, driven by a single input; the xldatagrid shape of the clause-1 measurement, widened to a grid's width), which puts 102 dependents behind the memoised container: a commit that moves that container across a pair the relation still **conflates** costs **0** dependent recomputes, and a commit that moves it across any one of the four pairs #269 newly **distinguishes**; `undefined` versus `null` inside a container, an own non-enumerable key, an own symbol key, an array hole; costs **101**: `visibleColumns` plus all 100 cells. The 102nd dependent, the summary, stays elided under both arms, because the cells' scalar outputs do not move and the scalar cutoff still stops the wave one layer short of it. The delta this refinement costs is therefore **+101 dependent recomputes per affected commit** on that fan-out, up from 0, and it is paid only by commits whose derived container differs in exactly one of those four ways; a commit that changes nothing the relation distinguishes still costs 0. `packages/core/test/spec-amendment-8-engine-claims-260.test.ts` re-measures both arms on every run and fails if either number stops matching the figures written here.

**The second refinement under the rule: #258's bare-instance clause, priced.** The `KIND_BARE_INSTANCE` narrowing of row 2 above is a refinement by this amendment's own definition; a value class the relation used to elide over now refuses elision, so more nodes reach `changedNodes` and never fewer; and it shipped without the three things this amendment requires of one: the narrowed row, the direction in `CHANGELOG.md`, and a measured recompute-volume delta in the unit the clause names. All three land here. The measurement, on the same 103-derived grid fan-out the #269 delta used (one memoised container → `visibleColumns` → 100 cell deriveds → one summary, driven by a single input), on the shipped engine: a commit that moves the container across a value the walk still reads at least one own property out of; a class instance carrying own fields, an empty plain `{}`, an `Object.create(null)` record; costs **0** dependent recomputes, unchanged; a commit that moves it across a bare instance costs **101 dependent recomputes**, and all four shapes the clause covers measure the same 101: a `#private`-field instance, a value whose prototype is a null-prototype data bag, an own-non-enumerable-accessor class instance, and that instance's plain-object twin. The bare-instance refinement therefore costs **+101 dependent recomputes per commit that moves such a value**, up from 0. Nothing else on the fan-out moves: a graph holding no bare instance pays exactly what it paid before, because the clause is reached only from a value the walk read as the empty record, and the 102nd dependent, the summary; stays elided under both arms for the same reason it does under #269. A wall-clock figure would not have discharged this clause and `packages/core/bench/baseline.json` is not the measurement; `packages/core/test/spec-amendment-8-engine-claims-260.test.ts` re-measures both arms on every run and fails if either number stops matching the figures written here.

**Versioning policy for the cutoff relation.** A consumer observes this relation through `Commit.changedNodes` and the Phase-G/H fire sets, so a change to it is a change to a public surface, and `CHANGELOG.md`'s "SemVer once the first stable release ships" deferral does not reach it. From 0.4.0: a **refinement**; the relation distinguishes a pair it used to conflate, so more nodes reach `changedNodes` and more dependents recompute; is a **minor** change and MUST carry the measurement the clause above demands; a **coarsening**; the relation conflates a pair it used to distinguish, so a value change a consumer previously observed becomes invisible; is **semver-major**; and admitting a value class that introduces a new permitted quotient re-opens this amendment and is likewise **semver-major**. Every such change names its direction in `CHANGELOG.md`.

**The 1.0 direction on closures.** Closures stay in the adopter-facing value domain through 1.0. The repository held three incompatible positions on this, and the one that survives is the shipped one: `packages/core/src/value-domain.ts`, "an input may legitimately hold a callback/bigint". The `JsonRoundtrippable` SemVer-major narrowing scheduled at `docs/epic-1483/CONSTRAINTS.md` and `docs/epic-1133/PLAN.md` is **withdrawn as scheduled**, and both lines now cite this amendment rather than a break that is not coming. EPIC #255 makes closures-in-derived-values work correctly in 0.4.0 and thereby raises the migration cost of that narrowing, which is why the direction is settled here rather than deferred a third time. What the constraint should instead be *named*; a bound written for the engine value domain, JSON plus the tagged-container bridge, rather than for JSON, which as literally written would also refuse the `Set` / `Map` / `Date` / `Temporal` the bridge marshals correctly today; and the type-level disposition of the four non-JSON scalar classes are #270's work. This amendment settles only the direction: no type-level bound that would refuse a closure lands in 0.4.0 or 1.0. `CHANGELOG.md`'s 0.4.0 entry names the winning position.

§5.1 Amendment 9 (#267, D14): `derived(id, compute, { key })`: an adopter-elected **substitution of the relation's subject**. Amendment 7 pins *which* equality gates a derived node's "changed" verdict; it is silent on *what that equality is applied to*, and reads as though the answer is always the whole recomputed value. For a derivation registered with a `key` **projection** it is not, and this amendment says so rather than leaving the rule an adopter reads false for those nodes. Normative rule: when a derivation is registered with `options.key` (`DerivedOptions<T>`, §12.4), it counts as **changed** in Phase D **iff** `key(value)` differs from `key(previousValue)` under Amendment 7's canonical value-equality relation; the relation itself is unchanged, only its subject moves. Everything else about the node is unchanged: the value is retained and served by `read()` **verbatim** (a projection to a row count is not an instruction to forget the rows), the keyed node's own compute still runs whenever its dependencies move (`key` governs propagation, never evaluation), and the projection is **not** a bound on the value domain (§15.1 stays open). The projection is required to be **marker-expressible** rather than an arbitrary predicate for a structural reason, not an implementation one: a keyed verdict resolves through the one content-hash mint (`derivedKeyMarker`, `packages/core/wasm/content-hash.ts`), called by the `rust-ssot` compute-out encoder before it encodes and by the reference floor at publish, so an engine and its oracle cannot diverge on a keyed node any more than they can on an un-keyed one. A per-node `equals` predicate cannot have that property and is rejected on §18A.1.1 grounds, permanently.

**The opaque-projection case, decided.** A `key` that returns a value the marker cannot **represent**: `function`, `symbol` or `bigint`, the three the hash collapses onto `TAG_NULL` and therefore onto each other; is **accepted** at registration and **refuses elision**: the derivation is reported changed on every commit for as long as the projection stays opaque, and its dependent cone recomputes. It is not rejected at construction, and the alternative is refused on a stated ground rather than on preference: `key` is a function *of the value*, so a construction-time probe can only ever sample the registration-time value, and a projection that returns a plain object at registration and a closure three commits later would pass that probe and then have to fail mid-commit; converting a performance hint into a commit-aborting hazard. Refusing elision is the failure mode that is *slower*, never *wrong*, which is the exact criterion #302 used to choose the congruence gate (option (c)) for opaque values generally; `{ key }` **inherits** that gate rather than special-casing itself. `undefined` and `null` projections are **not** opaque: `TAG_NULL` is a faithful record for them (the `undefined === null` wire identity of §15.1's #270 amendment), so a projection that returns `null` on every recompute genuinely has not moved and elides. A projection that merely *contains* an opaque slot is governed by Amendment 8 clause 1 exactly as any other opaque-carrying value is, and that arm has **shipped** (#258): the clause-1 gate is applied to the **projection**, not to the recomputed value. Concretely, a keyed derivation elides only when `key(value)` and `key(previousValue)` agree under Amendment 7's relation **and** every opaque payload the projection carries is the same reference it was, compared by `Object.is` at compare time. Two consequences are stated rather than left to be inferred, because each of them is silent when it is wrong. (a) An opaque slot in the **value** that the projection does not select is irrelevant to the verdict. `visibleColumns` is closure-carrying by design and an unmemoised compute mints fresh closures on every recompute, so a gate read off the value would report changed on every commit whatever the projection did; the option would be inert on precisely the payload it exists for, which is a deletion of the feature and not the "slower, never wrong" trade this amendment elsewhere accepts. (b) An opaque slot the projection **does** select is decisive. The marker collapses two distinct closures onto one record; nothing reference-keyed is minted, by #302's choice; so without the gate a projection that names the closures as the cutoff key would elide the very swap the adopter elected to watch, which is EPIC #255's defect class reached through this option. The refusal stays conditional on a slot having moved, exactly as clause 1 requires, so a memoised projection keeps its cutoff.

**Why this is neither a refinement nor a coarsening under Amendment 8's versioning policy.** Amendment 8 makes a coarsening of the relation semver-major, and substituting a projection for a value plainly *can* make a change invisible that a consumer previously observed. It is not a coarsening as that clause means it, because the relation is not changed for any node: an un-keyed derivation; every derivation that exists in any graph written before this amendment; resolves through the identical relation over the identical subject, and `{ key }` is a per-node election the adopter makes at registration for a node they are writing. Amendment 8's measurement clause is likewise not triggered: it prices **refinements**, which increase engine work, and the elected arm can only ever *reduce* recompute volume relative to the same node without a `key`. The one arm that adds work is the opaque-projection refusal above, and it is bounded to nodes that both carry a `key` and return something non-representable from it; a shape the dev-mode opaque-slot diagnostic (#264 T9) already reports on the node's first publish.

**One deliberate carve-out.** `key` is **not consulted when the derivation's value is a scalar**. A scalar is already `O(1)` to compare and the engine cuts it off by its own wire scalar record; honouring a projection there would replace that record with a content-hash and move a scalar derived's residency into the host value cache, to buy a cutoff strictly *less* sensitive than the one already in force. The shipped gate is `keyedComputeOutMarker` (`packages/core/wasm/authoritative.ts`), the reference floor applies the identical gate at publish, and that shared placement is why the carve-out cannot become a divergence.

**A throwing projection is not a fallback.** `key` is consumer code invoked inside the commit window. A throw surfaces to the caller and the commit is rolled back atomically (§5.2), identically to a throwing `compute`; the engine does **not** degrade to hashing the whole value. Silent degradation would make a keyed graph's cutoff depend on whether some earlier commit happened to throw, which is a worse contract than the throw.
...
After the catch arm runs, the engine state is byte-identical to the pre-call moment, and the throw propagates to the caller. No subscriber has fired: Phases G and H sit *outside* the **rollback** envelope (the catch arm restores B–F.6, not G–H) but *inside* the **commit window** (§5.7); they never run on the failure path because the throw short-circuits before them, not because the commit has "ended." The commit window stays open from Phase A entry until Phase H completes; the rollback envelope is the narrower B-through-F.6 span the catch arm restores. This is what Elliott meant by "the next state is a function of the previous state"; it is also a description of what the implementation does, line for line.
...
The implementation seam is a single internal commit entry taking `(intent, run, options)`, which `commit` and `hydrate` both call; in the engine this package ships that entry is the Rust commit pipeline behind the bridge's `apply_commands` extern. `commit` is the public entrypoint with `intent` user-supplied and `originatedAt` left **`undefined`** (a plain commit does not stamp an origin time); `hydrate` is the privileged entrypoint with `intent: 'hydrate'` and `originatedAt: snap.time`. This makes `originatedAt` an optional field on the frozen `Commit`: `originatedAt?: GraphTime`, `undefined` on `commit`-issued records and `snap.time` on `hydrate`-issued records; and its **presence is the normative hydrate discriminator**: consumers test `commit.originatedAt !== undefined` to tell a hydrate-issued record from a plain commit, and MUST NOT assume `originatedAt` equals the commit's own `time` (it never does, because a plain commit does not carry one at all). This is exactly what the §16.2.1 IR contract records for `IRCommit.originatedAt` ("Absent on `graph.commit(...)`-issued records"); §5.3 and the IR agree. Future privileged callers (a server-replay primitive, a devtools-bridge fork operation if §13 ever reopens edit-mode time-travel) get added by extending that entry's options bag, never by writing a parallel pipeline. This is the §3 monotonicity invariant kept structurally rather than by hope; there is no API surface where `now` advances by zero ticks or two ticks, because there is no pipeline that does that (#366, #378).
...
The team rejected the "`hydrate` is its own pipeline" alternative because it doubled every invariant: Phase C advanced `now` in one place, hydrate's own clock-advance in another, and the bug was guaranteed within two refactors. Routing both entrypoints through the one internal commit entry is what makes the §3 monotonicity invariant a *fact about the call graph* rather than a comment in two implementations (#366, #378).
...
### 5.7 Re-entrancy and the commit window

The commit window is a load-bearing invariant the shipped engine enforces (and the reference floor agrees on), and I state it here so another conformant implementation cannot diverge while satisfying every other clause.

**The window.** The engine is in state `Committing` from the moment Phase A is entered until Phase H completes. The window therefore *includes* the Phase G/H observer-dispatch frames and the Phase D compute bodies: it does not close when Phase E freezes the `Commit`, nor when the catch arm's rollback envelope (Phases B–F.6, §5.2) ends. This is deliberate: in the shipped engine the Phase-G/H observer fan runs synchronously inside `apply_commands`, i.e. *inside* the same host-side commit window, so the window is open while observers fire and a conformant implementation must present it that way.

**Same-graph re-entry throws.** Any `commit`, `simulate`, or `hydrate` invoked on the **same** graph instance from any frame inside the commit window: including a Phase G per-node subscriber, a Phase H commit subscriber, and a Phase D `compute` body; MUST throw the typed `CommitInProgressError` **before staging any write**. The write is never partially applied; the throw is raised at the entrypoint, ahead of Phase A of the re-entrant call. (This is the same `CommitInProgressError` §5.4 names for `simulate` re-entry; §5.7 generalises it to the whole window and to all three mutation entrypoints.)

**Cross-graph commits are legal.** A `commit`/`simulate`/`hydrate` issued against a **different** `Graph` instance from a subscriber frame is legal: the committing flag is per-graph, not global. An observer on graph A may drive a commit on graph B synchronously; only re-entry into A's own window is refused.

**The sanctioned idiom.** The supported way to schedule a same-graph follow-up commit from inside an observer is microtask deferral (`queueMicrotask(() => graph.commit(...))`) which runs after Phase H has closed the window. Adopters relying on this pattern have their normative citation here rather than only in adapter docs.

Why this needs to be written down: the §5.2 phrasing (Phases G/H "outside the rollback envelope") could be read as "the commit has ended when observers fire," inviting a conforming implementation to clear its committing flag before Phase G and accept commit-from-subscriber. That reading diverges from the shipped engine (and from the reference floor) while satisfying every other written clause; §5.7 closes the gap.
...
**Controller region** (conceptual, not implemented). The diagram preserves the region as `CONCEPTUAL` so readers see the model / controller / engine separation §7 names, drawn at the same fidelity as the regions that ship. The states the diagram sketches: `Idle | Selecting | Drawing | Inspecting`; are placeholders; no engine code reads them. The reopen trigger is the same one §13 names for any deferred capability: when a devtools-bridge or application-layer use case actually needs a controller-state region tagged in the engine, it lands here, with the regions rebalanced and the chart updated. Until then, the controller layer is application-supplied (§7.2), and the conceptual region is the teaching diagram that explains *why* it lives outside `@causl/causl-wasm-ts`.
...
The package layout enforces the layering: `@causl/causl-wasm-ts` is the engine substrate; `@causl/react`, `@causl/sync`, `@causl/persistence`, `@causl/devtools`, `@causl/devtools-bridge`, `@causl/formula` are adapters that bridge the substrate to specific controller / view technologies; the application owns the model layer and supplies the controller layer.
...
`ReadOnlyGraph = Pick<Graph, 'read' | 'subscribe' | 'subscribeCommits' | 'now'>` is the canonical narrow type: the read-side capability slice every adapter needs and nothing more. It is `@causl/causl-wasm-ts/internal`-only per §12.3: applications should not name `ReadOnlyGraph` in their own types because the adapter that supplies the narrowed handle is the one that owns the slice. Per #371, #433, and #441, each adapter declares its own `Pick<Graph, ...>` alias for the slice it actually needs:

- `@causl/react`'s selector boundary uses `Pick<Graph, 'read' | 'subscribe' | 'now'>` (no `subscribeCommits`: selectors don't need the commit firehose).
- `@causl/persistence`'s observer uses `Pick<Graph, 'read' | 'subscribeCommits' | 'now' | 'commitLog'>` (no per-node `subscribe`: persistence reacts to commits, not to individual node changes).
- `@causl/devtools-bridge`'s host channel uses `Pick<Graph, 'read' | 'subscribeCommits' | 'now' | 'commitLog' | 'snapshotAt' | 'readAt' | 'dependencies' | 'dependents'>` (no `commit`, no `hydrate`: the bridge is read-only by §13's view-only-JUMP discipline).

Each adapter's slice is the smallest set of `Graph` methods that adapter actually calls. The slice is documented at the adapter's seam, gated by `narrowCapability`, and audited the same way §12.2's fifteen second-tier rows are: any adapter that wants to widen its slice writes the justification in the adapter's own README and the §12.3 row that catalogues `narrowCapability`'s consumers gets updated.
...
- **Inversion gate**: `@causl/causl-wasm-ts` may not import from any sibling adapter (`@causl/react`, `@causl/sync`, `@causl/persistence`, `@causl/devtools`, `@causl/devtools-bridge`, `@causl/formula`, `@causl/checker`, `@causl/bench`, `@causl/migration-check`). The engine substrate is the bottom of the layer stack; reaching into an adapter inverts the layering and creates the kind of two-way coupling that lets controller-shaped types leak back into the core barrel. The error message names the spec section (`SPEC §17.3 / §7`) so a developer who hits the rule has the rationale one click away.
- **Deep-path gate**: adapter packages may reach engine internals only through the documented `@causl/causl-wasm-ts/internal` entrypoint, never through `@causl/causl-wasm-ts/dist/*` or `@causl/causl-wasm-ts/src/*`. The `exports` field in `packages/core/package.json` already refuses to resolve such paths at runtime; this rule codifies the same contract at lint time so a future bundler or tsconfig change cannot silently re-open the back door.
...
3. **IR-level static rule in `causl-check`.** The Rust binary already runs as a CI gate (§16); a layering pass would walk `CauslModel.nodes` and assert no node's compute reaches across a layer boundary. Rejected on scope: the IR is the contract between `@causl/causl-wasm-ts` and the checker, and layering is a contract between `@causl/causl-wasm-ts` and the *adapter packages*; a different boundary at a different abstraction level. Layering violations live in import graphs, not in the IR. The lint rule sits at the right boundary.
...
The team rejected a "controller layer in `@causl/causl-wasm-ts`" alternative (a controller-shaped type that ships with the engine, so applications inherit a controller vocabulary) because the controller is the most application-specific layer in the §7 stack: a spreadsheet's controller, an asset-hierarchy editor's controller, and a booking system's controller share almost no vocabulary. Shipping a generic controller in `@causl/causl-wasm-ts` is the same mistake the previous draft made by listing eleven inspirations and stacking them with plus signs. The controller layer is application-supplied; the conceptual region in §6 is the teaching diagram that explains why.
...
```ts
import { defineMsgs, payload, type MsgOf, assertNever } from '@causl/react'
...
- **`whyUpdated(node)`** (in `@causl/devtools`): `DerivedNode<WhyUpdatedResult>`. Returns a derived node that recomputes once per commit and answers "what most recently caused this node to update?" Tags: `recomputed | directly-set | no-cause`.

- **`whyNotUpdated(node)`** (in `@causl/devtools`): `DerivedNode<WhyNotUpdatedResult>`. The dual of `whyUpdated`. Tags: `did-update | no-dep-overlap | object-is-deduped | no-cause`.
...
- **A `@causl/repl` package**. The original §11 prose promised "a REPL connected to a running graph." The primitives that make a REPL real have shipped: `liveDerived` (devtools-side hot-swap of compute closures), `replaceMany` (atomic batch swap), `commitMetadataDerived` (Phase F.5 seam); and the runnable artefact is `docs/demo.md`. A REPL is a usage pattern composed from those three primitives, not a new primitive that earns its own surface row.

**What the team rejected and why.** A "devtools panel" sitting next to the engine: Kay's pushback was unanswerable: a parallel observer system means the engine is *not* its own observer. `whyUpdated` returning a `WhyResult` synchronously; ergonomic, but breaks the §11 framing. A live `commitLog` slice via plain `derived(...)`; shipped in #383, regressed, reverted, replaced by the F.5 seam. A separate `@causl/repl` package; surface inflation against §17 commitment 6.

---

## 12. Public surface

I keep the contract small and audited. The `Graph` interface in `@causl/causl-wasm-ts` is the most-watched contract in the project. The previous draft of this section claimed "seven and only seven." That was a slogan, not a contract. The honest count is **twenty-one public `Graph` members today**: six canonical members the engine cannot exist without (§12.1) plus fifteen second-tier extensions that the engine could in principle be smaller without (§12.2) but that adapter packages and inspection primitives would then have to invent themselves and drift from. The canonical set of §12.1 lists *seven* rows, but the seventh (`createCausl`) is the construction **factory**, not a member of the `Graph` interface; counting it alongside the members, the audited public surface is **twenty-two entrypoints** (seven canonical + fifteen second-tier), of which twenty-one are `Graph` members. This single reconciled count now holds across §0, §12, and §18. Each second-tier row carries its own justification; the `clearCommitHistory` row that used to live there was struck in #401 because the justification had faded. The `stats` and `subscribeMany` rows were reconciled into §12.2 by the 2026-07-06 review amendment (H1) after each shipped without a §12.2 / §12.4 row.
...
| `Graph.subscribeMany` | `<Ts extends readonly Node<unknown>[]>(nodes: Ts, observer: (values: ValueMap<Ts>) => void, options?: SubscribeOptions) => Unsubscribe` | Batch per-node subscription (#766): one observer registered across a group of nodes, deduped to fire exactly once per commit in which any node in the group changed; cheaper than N independent `subscribe` calls. |
| `Graph.stats` | `() => EngineTelemetry` | Engine telemetry snapshot: live input/derived counts, node version, and the state hash the §18A.1.1 cross-backend byte-identity gate compares. Backs the §18A.3 `engine_stats` extern. |
| `Graph.now` (getter) | `GraphTime` | "What time is it?" without firing a commit. |

**Twenty-one public `Graph` members today (six canonical members + fifteen second-tier); seven canonical *entrypoints* counting the `createCausl` factory, which is not a `Graph` member.** The team audits this count quarterly. The previous spec promised seven and only seven; honesty cost the second tier. `subscribeMany` (#766) and `stats` were folded in by the 2026-07-06 review amendment (H1): each had shipped on the `Graph` interface without a §12.2 row, so the surface audit had drifted below the shipped interface; this reconciliation restores the invariant that §12.1 + §12.2 enumerate the whole `Graph` interface. The count here, in §0, and in §18 is now the single number twenty-one.

### 12.3 Internal-only adapter entrypoint (`@causl/causl-wasm-ts/internal`)

A separate entrypoint with strict capability discipline. Adapter packages import these via `@causl/causl-wasm-ts/internal`; application code never does. An ESLint rule (#393, #435) enforces the layering at the import statement.

| Surface | Used by | Why internal |
| --- | --- | --- |
| `dispose(graph, node)` | `@causl/react` `useCauslFamily` lifecycle (#124) | Adapter-owned lifecycle. |
| `narrowCapability(graph)` | `@causl/react` selectors and listeners | Mark Miller's principle of least authority. |
...
#### 12.3.1 Testing seam (`@causl/causl-wasm-ts/testing`, npm: `@causl/core-testing-internal`)
...
Three entrypoints, three audiences: `@causl/causl-wasm-ts` for application code, `@causl/causl-wasm-ts/internal` for adapter code, `@causl/causl-wasm-ts/testing` for test code. The boundaries are mechanically enforceable.

### 12.4 In-flight additions awaiting classification

*(Retired as a standing list by EPIC #283. The §395 cautionary tale: `snapshotAt` shipped straight to public bypassing this lot, and only entered §12.2 retroactively. Every new public-surface item passes through §12.4 before landing, with no exceptions; including the one below, which is in-flight rather than classified precisely because that rule has no exemptions.)*

| Surface | Signature | Classification pending on |
| --- | --- | --- |
| `DerivedOptions<T>` | `interface DerivedOptions<T> { readonly tag?: 'live' \| 'commit-metadata'; readonly key?: (value: T) => unknown }` | Whether `key` is a 1.0 API or earlier (#267, open). |
| `Graph.derived`'s third parameter | `options?: DerivedOptions<T>` | Additive widening of an existing §12.1 row, not a new row. |

**What this classifies.** `Graph.derived` has always carried an options bag; until #267 it was an inline anonymous type with one engine-internal member (`tag`), so nothing adopter-facing had ever entered through it and it never needed a §12 row. `key` (#267, D14) is the bag's first **consumer-facing** member and its first member generic in `T`, which is what promotes the bag to a named, exported `DerivedOptions<T>` and brings it here. The change is purely **additive**: `tag` is carried unchanged and no existing `derived(...)` / `liveDerived(...)` / `commitMetadataDerived(...)` call site is edited.

**What `key` is.** A **marker-expressible** cutoff projection: the adopter supplies a function from the derivation's value to a value the content-hash *can* represent, and the §5.1 Amendment 7 changed-relation is applied to that projection instead of to the whole value (see the §5.1 Amendment 7 addendum). A per-node `equals` predicate was **rejected** for the same surface: an arbitrary JS predicate cannot be expressed in the marker, so the engine and the §18A.1.1 reference oracle would resolve the cutoff verdict differently and the differential comparison would break by construction. A projection is resolved through the one shared mint, so it cannot.

**Two open questions #267 records and this row does not close:** whether `key` participates in replay determinism, and whether it ships at 1.0 or earlier. The third (what happens when `key` itself returns an opaque value) is closed, in §5.1 Amendment 7's addendum.

### 12.5 Adapter package surface

`@causl/causl-wasm-ts` is the most-watched contract; adapter packages change at their own pace. SPEC §12 audits only the `@causl/causl-wasm-ts` `Graph` interface. Each adapter's public surface lives in its own README:

| Adapter | README |
| --- | --- |
| `@causl/react` | `packages/react/README.md` |
| `@causl/sync` | `packages/sync/README.md` |
| `@causl/persistence` | `packages/persistence/README.md` |
| `@causl/devtools` | `packages/devtools/README.md` |
| `@causl/devtools-bridge` (private; §13 deferred-product) | `packages/devtools-bridge/README.md` |
| `@causl/formula` | `packages/formula/README.md` |
| `@causl/checker` | `packages/checker/README.md` |
| `@causl/migration-check` | `packages/migration-check/README.md` |
| `@causl/bench` (workspace-private; not published, and NOT in this repository: the benchmark workspace lives in the `causl/causl-core-ts` fork) | `packages/bench/README.md` in that fork |
...
I am not promising any of the items in this section. Each row carries a status: **DEFERRED** (designed, may ship if a trigger fires) or **NOT-PLANNED** (the team has actively decided this is out of scope); and a reopen trigger if applicable. One row outgrew the section: §13.8's trigger fired, the Rust core shipped, and that row now stands as the dated record of the promotion rather than a deferral.
...
- **JUMP path in `@causl/devtools-bridge`**: switches from view-only `snapshotAt(t)` to branch-forking when `parentTime` is set.
...
| Multi-tab app with BroadcastChannel | One `createCausl()` per tab; `@causl/sync` brokers cross-tab |
...
`@causl/formula` ships as a separate adapter package; the core itself does not parse expressions, does not know about `=SUM(...)`, does not know about cell references. Reopen trigger for folding into core: none.

### 13.6 Async resources: DEFERRED (via separate adapter)

`@causl/sync` ships as the adapter that models async fetches as Events feeding Inputs, with the lifecycle in §6. Reopen trigger for folding into core: none.
...
### 13.8 Rust core: SHIPPED, and since 0.5.0 the engine this package ships (row closed; kept as the dated record)

**Where this row landed (read this first).** The trajectory this row records completed: the Rust core is the engine. It was promoted to the WASM-path default on 2026-06-21 (§19 trail), made unconditional by the §18A.8 fail-safe removal, and made this package's sole engine by §18A.13, with the roster collapse finished at 0.5.0 (§18A.13.1). The paragraphs below are the dated record of how the row moved from NOT-PLANNED to that end state; where they speak of the TS engine as the floor, they describe the roster as it stood on their date and, today, the roster of `causl/causl-core-ts`.

**Reopen trigger FIRED (2026-06-19).** The §13.8 reopen trigger as previously stated: "the TypeScript core ships at the bar §10 demands, plus a real adopter with a workload the JS engine cannot meet"; is superseded by a governance decision. The team has committed to the Rust-backed wasm engine core as a **conformant alternative** to the TypeScript engine, with a named promotion path to **replace** the TS value-of-record when the GO/NO-GO criteria in §18A.7 are met. Post-0.9.0 epic #1133 (full Rust engine port) is no longer NOT-PLANNED; it is now **PLANNED with explicit gatekeeping**. The "NOT-PLANNED" label is retired for this row; the commitment surface now carries the alternative-engine contract (§17.6 Commitment 15) and the promotion gate (§18A.7).

**Current architecture (as of v0.9.0).** The Rust crates carry three responsibilities: no longer the "checker-only or scaffold-only" framing:
1. `tools/checker/Cargo.toml`: the static IR linter and the deferred bounded-enumerator instrumentation (§16).
2. `tools/engine-rs-core/`: the pure-algorithm state machine ported from §5's TypeScript implementation, carrying real types (generational `NodeId { slot: u32, gen: u32 }` per #1151, `JsonValue::Object(BTreeMap<SmolStr, JsonValue>)` per #1078, the seven-named-struct cell shape per #1077).
3. `tools/engine-rs-bridge-{serde,gc}/`: the JS↔WASM bridge crates backing the opt-in `@causl/causl-wasm-ts/wasm` substrate per §17.6, compiled to WebAssembly at the three host tiers (§17.6 host-tier matrix).

The **wasm engine core**: a conformant implementation of the §3 semantic equation and the §4–§12 public contract; is exposed through `@causl/causl-wasm-ts/wasm` and consumed by the **thin-TS-API layer** (`causl-wasm-ts`, the `@causl/causl-wasm-ts/wasm` integration surface, §18A.4), which delegates to the wasm core over FFI to satisfy the full public surface. The structural surface then internal to `engine-rs-core` (dependencies, dependents and their transitive closures, commit-log, commit-metadata, handle/disposal/StaleTx validation, stats) is lifted into the FFI contract (§18A.3); that lift was the spine of the work and has since landed (`causl/causl-core-rs#170`).

**The two-engine contract (dated record; live today only in `causl/causl-core-ts`).** `causl-ts` (the existing TypeScript implementation, staying supported as the floor) and `causl-wasm` (the Rust core compiled to WebAssembly, accessed via the thin TS API) are both conformant implementations of the §3 equation and the §12 public surface, byte-identical at the public-contract boundary via the cross-backend determinism gate (§15.1, per-flush under the batched-boundary opt-in per epic #1493). A developer swaps `backend: 'js'` for `backend: await loadWasmBackend()` in `createCausl(options)` and sees bit-identical behaviour at the public surface, minus the documented marshaling cost (§17.6 G.1: the 78× boundary tax is the standing cost of the wire). *(That swap is no longer expressible in `causl/causl-wasm-ts`: `createCausl`'s `backend` option was removed at its 0.5.0 (`causl/causl-wasm-ts#280`), which ships one engine. The sentence is the dated record of the two-engine contract and still describes `causl/causl-core-ts`, the org's only owner of the floor; §18A.13.1's withdrawal is the live disposition here.)* The full contract is §18A; Commitment 15 (§17.6) is its commitment-table row.

**Why the reopen fired now.** Enough scaffolding has landed (EPIC #680 Phase-1 bridges, epic #1493 batched-boundary infrastructure, the #1071/#1146 cross-backend byte-identity gate, the §18A.3 FFI surface plan, the #1333/#1124 FFI-boundary invariants) that the engineered shape is no longer speculative: exactly the §16.4 "engineered shape on the shelf" honesty, now promoted to a commitment. The decision to commit is a governance choice. The criteria in §18A.7 are the bars any *replacement* would clear; recording them here means the next SPEC amendment can note "all criteria met as of [DATE]" rather than re-litigate the architecture.

**Reopen trigger for *promotion to default/replacement*:** all GO/NO-GO criteria in §18A.7 met, recorded in a dated follow-up amendment by the same governance authority that opened this row. That trigger has since fired: the §19 trail records all five criteria GO on 2026-06-21 and the promotion that followed, so the "until then the TS engine is the unconditional floor" condition this sentence used to close with expired with it. The marshaling cost stays transparent in every adoption guide.

**What the team rejected and why.** Promoting the wasm engine to *default on merge of this amendment*: rejected: the costs (78× wire tax, read-identity break, doubled conformance burden) are real and the criteria in §18A.7 are not yet met; a silent switchover is exactly the §17.6 "acceleration-not-substitution" discipline this amendment tightens, not loosens. Keeping the row at NOT-PLANNED; rejected: the scaffold is no longer a scaffold, and pretending the Rust crates are "checker-only" is the dishonesty §13.8's prior current-state callout already strained against. Re-writing the TS engine *in Rust* (a line-by-line port carrying the TS substrate assumptions); rejected: the canonical way is a Rust engine stood up from scratch against the same §0/§12 contract and proven byte-identical via the cross-backend gate, not a transliteration (§18A.4). Committing to the wasm engine *without* the GO/NO-GO gate; rejected: that is promotion-by-stealth; the gate is what keeps the alternative→default step a named, dated, auditable decision.

### 13.9 GraphQL adapter: NOT-PLANNED

Out of scope. If an adopter ships GraphQL on top of `@causl/causl-wasm-ts` they ship it as their own adapter.

Reopen trigger: an adopter shipping GraphQL on top of `@causl/causl-wasm-ts` whose pattern is broadly enough useful to fold into a maintained sibling adapter. None today.

### 13.10 Devtools as a shippable product: NOT-PLANNED

I'll ship inspection primitives (§11): `whyUpdated`, `whyNotUpdated`, `commitLog`, `dependencies`, `dependents`, `liveDerived`, `replaceMany`, `commitMetadataDerived`. **I will not ship a devtools UI as a product.** The `@causl/devtools-bridge` package is the wire-protocol substrate that a downstream UI *would* consume; it ships as private precisely so a future UI initiative can ship it as its product.

Reopen trigger: a downstream initiative requesting a maintained UI wrapper plus the engineering capacity to support it.

**What the team rejected and why.** Promising any §13.5–§13.9 row as a v1 deliverable: the previous draft's "Phases 8-15" framing is the precise mistake §13 is correcting. Engineering edit-mode time travel before a use case; §13.2 is the design, frozen, but building it speculatively would add a `parentTime` field nothing reads. Designing multi-graph composition speculatively; §13.3's three-shape walk shows the only multi-time semantics that preserves §3 collapses back to single-time, and the engineered shape if reopened is the `@causl/coordinator` sibling adapter, not an `embedGraph` extension to core. Promoting the `Disposed` discriminator to compile-time; the runtime guard holds. Folding any adapter taxonomy into core; the semantic-core / adapter split is what makes both teachable.
...
| --- | --- | --- |
| Engine: `O(|affected|)` recompute | `perf-invariant; SPEC §14 gate` | `pnpm --filter @causl/causl-wasm-ts run test:perf-invariant` runs `packages/core/test/perf-recompute.test.ts` (#145). |
| React: re-render scope | `perf-invariant; SPEC §14 React subscription gate` | `pnpm --filter @causl/react run test:perf-invariant` runs the `useCausl`, `useCauslShallow`, `useSyncExternalStore`, `strictMode`, and `family-grid` suites; the family-grid leg ratchets the heap-delta gate under `CAUSL_HEAP_GATE=1` (#389). |
...
The team revised the bundle budget upward: **`@causl/causl-wasm-ts` ships in the 25-40 KB minified band, with 28 KB as the working target.** **Amendment (2026-08-03, §14.2.1, issue causl/causl-wasm-ts#352): the band was 15-30 KB with an 18 KB target and moved up 10 KB wholesale**; floor 15 → 25, outer ceiling 30 → 40, target 18 → 28; so the band keeps its shape (floor = target − 3 KB, outer ceiling = target + 12 KB) and the `size-limit` caps keep their 2 KB leading bound. The band moved because the ceilings had to; raising the caps alone would have left `full import` sitting exactly ON the old 30 KB outer edge, which is a band with no room left in it rather than a budget. The previous 4.5 KB / 6 KB ceilings were an aesthetic flex, not an engineering target; they forced every interesting invariant into the type system or into documentation, and documentation is not a runtime guarantee. The closed-PR record bears that out: PRs #266, #395, #420 kept fixing surface-count drift; PRs #383 / #455 kept rediscovering that primitives the team had cut for size were the primitives adopters needed; #390 / #454 kept patching distribution rails for adapter packages adopters did not want as separate installs. At 4.5 KB the engine is a primitive; at the working target it is a system; at the band's outer ceiling it is the system the team actually wants to defend. (Those were 18 KB and 30 KB when this was written; 28 KB and 40 KB since the 2026-08-03 amendment. The ratio the sentence is about did not move.)
...
2. **Production bytes pay only for invariants the type system cannot express.** Dev-mode warnings, source-mapped error chains, runtime IR validation: these run *in dev* and dead-code-eliminate at minify time behind `process.env.NODE_ENV !== 'production'`. The band's outer ceiling (40 KB since the 2026-08-03 amendment, 30 KB before it) applies to the production build; dev builds may exceed it for diagnostic value.
3. **The working target is 18 KB, not 30 KB** (28 KB since the 2026-08-03 amendment; the reasoning below is why a target exists at all, and it is unchanged by the number moving)**.** The team rejected "spend the budget because we have it." A bundle that ships at 22 KB of mediocre primitives is a regression against a 5 KB bundle of sharp ones. 18 KB is the calibration the team can defend; 30 KB is the ceiling that bounds the discipline.
...
| --- | --- | --- | --- | --- | --- |
| 1 | **Source-mapped error chains across the eleven typed-error hierarchy**: every `CycleError`, `NodeDisposedError`, `HydrationSchemaError`, `CapabilityViolation`, etc. carries `{ commit: Commit, site: CallSite, dependents: NodeId[] }` via `Error.cause` | Every error in `@causl/causl-wasm-ts` is a graph-shape error, and graph-shape errors are unsolvable without graph context. The current class-name-plus-string story is a lie about typed errors. | ~3 KB | ~4 KB | Hejlsberg / Eich |
| 2 | **Devtools wiring as a one-line opt-in (`createCausl({ devtools: true })`)**: `connectDevtools` from `@causl/devtools-bridge` folded into core, dead-code-eliminated when the flag is `false` or `NODE_ENV === 'production'` (extension-absent path) | Adopters install the bridge package on day six, after the first incident. Hiding devtools behind an extra install is self-sabotage; React DevTools succeeded by being the default. | ~0 KB (DCE) | ~3 KB | Markbåge / Linsley |
...
The five additions sum to about **5.4 KB of production bytes and 9.4 KB of dev bytes**. Layered on top of the current 3.4 KB minified `@causl/causl-wasm-ts` engine, the production bundle lands near **8.8 KB** with all five shipping: comfortably inside the then-15 KB floor and leaving 6-9 KB of headroom for the next adopter-driven cycle. The dev bundle lands near **12.8 KB**, still under the then-18 KB working target. (Both figures are as-measured at the time of that analysis and are left at their original numbers deliberately; restating them against the 2026-08-03 band would misreport what was measured.)
...
| --- | --- | --- |
| `@causl/causl-wasm-ts (full import)` | **30 KB** | Production minified. The band's working target plus ~2 KB headroom for the next adopter-driven addition. **Amendment (2026-08-03, issue causl/causl-wasm-ts#352): raised 20 → 30 KB with the whole band (+10 KB), and §14.2's band amended 15-30 → 25-40 KB in the same decision.** Raising the cap alone would have put this cell exactly on the old 30 KB outer edge; the band moved with it, so 30 KB is a cap inside a 25-40 KB band rather than the edge of one. |
| `@causl/causl-wasm-ts (createCausl-only)` | **16 KB** | The minimal import path: `createCausl`, the canonical seven, and the §14.2.1 additions that ride along. Imports of §12.2 second-tier methods grow the cell-size; consumers who only register inputs and read derivations stay near this floor. **Amendment (2026-07, issue #129 write-SSOT cutover): raised 15 → 16 KB.** The Phase A–C publication deletion pays ~0.4 KB of gated main-bundle arms inside `commitInternal` (the cache-baselined cutoff, the invariant pre-flight partition, the cell release, the commit-metadata flip backfill) that structurally cannot live off the main bundle; `commitInternal` was the one shared pipeline (§5 "one mutation pipeline"). *(That accounting is the dated record of the 2026-07 raise and is left as taken. `commitInternal` no longer exists in this package: `causl/causl-wasm-ts#279` deleted it with the rest of the TypeScript closure, and the 2026-08-03 amendment below re-set this cell afterwards.)* The heavy reroute bodies (the simulate dry-run facade, the commitLog window construction) live on the `@causl/causl-wasm-ts/wasm` subpath. What the bytes buy: the TS Phase A–H per-commit walk is deleted on the wasm default path and commit-path value retention drops to the documented 2× floor; the #129 acceptance criteria, i.e. deleting the "two commit pipelines in lockstep" the §18A.13 complexity-elimination rationale named. The cell still LEADS the §14.2 working target (the #161 leading-bound contract). **Amendment (2026-08-03, issue #352): raised 16 → 26 KB with the whole band (+10 KB).** Measured 17.56 KB on `main` @ `eb2215e` against the 16 KB cap, and 16.05 KB at `67f66a2`; so this cell had been over its ceiling since before the #255/#275 campaign began. `bundle-budget.yml` runs only on `release` and the campaign pushed with `--no-verify`, so neither the CI gate nor the pre-commit hook ever evaluated a commit on `main`. |
| `@causl/react` | **8 KB** | Increased from the previous 3 KB to absorb the §8.2 extension hooks (`useCauslShallow`, `useCauslFamily`, `useCauslSuspense`, `<Hydrate>`) at honest size, plus the dev-mode "Update did not commit" warnings (Dan Abramov championed) DCE'd in production. |
| `@causl/devtools-bridge (connectDevtools-only, absent-extension path)` | **5 KB** | Increased from 3 KB to absorb the bridge logic that now opts into core via `devtools: true`. The absent-extension path remains the floor; the present-extension path adds real wiring above this number, gated by `__CAUSL_DEVTOOLS__`. |

Each ceiling is a CI gate (the existing `size: bundle-size gate (#147)` job). A PR that crosses any ceiling fails the gate; the PR description must include the §14.2.1 written team consensus or the size-limit bump is rejected. The ceilings are budget caps, not targets: the team works to the **28 KB target on `@causl/causl-wasm-ts (full import)`** (18 KB before the 2026-08-03 +10 KB amendment), with the 30 KB cap reserved for the cycle's next addition and the band's 40 KB outer ceiling bounding the cycle after that. `MAX_HEADROOM_BYTES` is unchanged at 2 KB across that amendment: the band moved, the leading discipline did not.

**How the cells are keyed (the rows above and the config now agree).** `.size-limit.cjs` and `tools/size-budget/js-budget.cjs` used to key the two main-bundle cells and the two raw wasm-bridge cells by the **retired** specifier: literally `@causl/core (full import)`, `@causl/core (createCausl-only)` and `@causl/core wasm bridge, gc-{builtins,classic} (raw)`; while only the loader cell carried the current one. Grepping the config for the current specifier therefore found one cell of five, and prose could bind to config only on the **label suffix** rather than on the whole name. **Renamed 2026-08-03 by EPIC #275's W7 (issue #282)**, which owns the rename and takes it in the same change as the first committed bundle capture: a before/after table whose rows are labelled with a package that no longer resolves is unreadable at exactly the moment it matters. The five cells W7 measures against are now `@causl/causl-wasm-ts (full import)`, `@causl/causl-wasm-ts (createCausl-only)`, `@causl/causl-wasm-ts/wasm (Phase-1 loader + WasmBackend wrapper)` and `@causl/causl-wasm-ts wasm bridge; gc-{builtins,classic} (raw)`; the cell `name` is the `jsCeilings` key, so the SSOT keys moved with them. `tools/size-budget/__tests__/documented-ceilings.test.mjs` now binds README to the config on the **whole** cell name, and `tools/size-budget/__tests__/w7-capture.test.mjs` refuses any cell name that carries the retired specifier again. `@causl/core-rs` (the engine crate) and `@causl/core-ts` (the fork) are live names and are deliberately not swept.

**Where the printed numbers come from (#282).** Every measurement quoted in this section and in `README.md` is read out of a committed capture under `tools/size-budget/runs/`, produced by `pnpm size:capture`; `documented-ceilings.test.mjs` fails when the prose and the capture disagree in either direction, and `main-fast.yml` runs it on every `main` PR. Before #282 the three `tools/size-budget/__tests__` suites existed as a package script no workflow and no hook invoked, which is how README came to print 17.7 / 15.69 / 19.85 KB for cells measuring 19.58 / 17.56 / 20.54 kB, and how the prose came to assert `pnpm size` exits 0 for months after it had stopped doing so (issue #352).
...
- **`useTransition`-aware MVU runner.** Sebastian Markbåge wants this in core; Evan Czaplicki pushes back hard ("framework-agnostic core is incompatible with React-scheduler-aware code"). Unresolved. Likely outcome: stays in `@causl/react` until a non-React adapter forces the question.
- **Edit-mode time travel via `Commit.parentTime`.** Alan Kay wants the design in core (§13.2 already engineers it); Trygve Reenskaug vetoes core involvement ("a tool concern; ship it in `@causl/devtools` where roles like Historian and Editor live"). David Harel splits the difference: design lands in core (the `parentTime` field), UX lands in devtools. The split is honest but does not yet pay for itself.
- **`liveDerived` and `replaceMany` promoted from `@causl/devtools` to core.** Kay yes (the §11 REPL primitives), Reenskaug hard no (role primitives), Harel against on chart-edge grounds, Conal Elliott neutral. Three-to-one against; stays in `@causl/devtools`.
- **Bundled persistence with localStorage as default.** Tanner Linsley wants it in core; Matt Pocock pushes back on the storage-adapter pluggability argument. Compromise on the table: ship the `StorageAdapter` *protocol* in core and keep concrete adapters (`localStorage`, IndexedDB) in `@causl/persistence`. Not yet ratified.
...
- **`useCauslQuery(fetcher)`, `commitMutationQueue` in core.** Server-state and offline-first are real patterns, but neither is universal. Adopters reaching for them are already reaching for Query / SWR; relitigating that fight inside core is wrong. Stay in `@causl/sync`.
- **`@causl/formula`, `@causl/checker`, `@causl/migration-check` folded into core.** Tooling-adjacent, separate release cadence. Migration-check especially: a one-time-per-upgrade tool, not a runtime dependency.
...
The generator family is `propertyDag` from `@causl/core-testing-internal`. Every property in the canonical set imports it; every property in adapter packages that exercise the same shapes imports it. The shared generator is the discipline that makes the §15.1 properties auditable as a set.

**§15.1 Amendment: `graph.read(node)` reference identity is not contractually guaranteed (#1124).** `graph.read(node)` is **not** contractually required to return the same JavaScript reference across calls. Reference identity was an implementation detail of the TypeScript engine this package used to ship, and it is cache-dependent under the engine I ship now: the `rust-ssot` read path serves the common case from the `ValueHandleCache`, so identity is *stable* on cache hits and *fresh* on decode paths (post-hydrate values, and Rust-computed values not in the cache; §18A.5 carries the full account and the history of the earlier, wrong, uniformly-fresh story). Adopters who depend on identity for memoisation (e.g. `React.memo` keyed off the read return, `useMemo(() => transform(value), [value])`) therefore break *intermittently*, on the decode paths, which is harder to catch in an adopter's own testing than a uniform break would be. The contractual identity surface is **value identity**; `Object.is(read(node)@t, read(node)@t)` holds for any two synchronous reads at the same `GraphTime`, and `Object.is(read(node)@t, read(node)@t')` may or may not hold across commits depending on backend. Adopters who need a stable memoisation key per commit MUST key on `commit.time` (the `GraphTime` exposed on `Commit`) or the per-node version counter `EngineTelemetry` surfaces, **not** the read return reference. Cross-link: §17.6 (commitment 14, host-tier substrate compatibility) names the migration boundary where this contract bite materialises; adopter-facing guidance lives at `docs/wasm-adoption-guide.md` H1 and is mirrored in the host-tier callout at `packages/core/wasm/README.md`.

**§15.1 Amendment: the value domain a node may hold is not closed at the type level (#270).** §15.1's #1124 amendment pinned what `read()` may *return*; it never pinned what a node may *hold*, and three artefacts in this repository answered that differently; `docs/epic-1483/CONSTRAINTS.md`'s migratable-surfaces row made a `JsonRoundtrippable` deprecation lint a precondition for any architecture crossing an FFI boundary on the read path, `docs/epic-1133/PLAN.md`'s Phase F row was blocked on a `Node<T extends JsonRoundtrippable>` SemVer-major narrowing, and `packages/core/src/value-domain.ts`, the shipped code; recorded the opposite, that "an input may legitimately hold a callback/bigint" and that `read` returns the real reference on both engines of the then-shipped pair. The third is the one that describes running code, so the first two are superseded by it, not the other way round. **Decided: the adopter-facing value domain is OPEN.** No bound lands on `Graph.input`, `Graph.derived` or `Compute<T>`; not in 1.0, not after. **The cost of the alternative, named here because a decision that does not name it does not count as made:** `JsonRoundtrippable` means JSON, so a literal bound refuses `Set`, `Map`, `Date` and `Temporal` values alongside the closures it was aimed at; four types both shipped engines already marshal correctly through the tagged-container bridge (`packages/core/wasm/tagged-types.ts`), all four of which the named adopter stores today (`ColumnState.hidden`, `expandedRows`, `rowDetails: Map<string, Set<string>>`, calendar `Temporal.PlainDate` cells). The disposition of the four non-JSON scalar classes is settled with it: `undefined`, `bigint`, `symbol` and `function` are **admitted**; retained host-side, returned by reference from `read` (the reference floor agrees); and the NULL-record collapse `packages/core/src/value-domain.ts` applies to them (`isWireNull` / `inputValueChanged`, shipped by #151) governs the **cutoff/fire** decision only, never `read`. The narrow domain is real and now has a name, because §5.1 Amendment 7's cutoff relation ranges over it and had none to cite: **`WireValue`** (`packages/core/src/types.ts`, re-exported from the package barrel) is JSON extended by the four tagged containers, and it is a description of the wire, explicitly not a bound on any adopter-facing type parameter. **Missed precedence, recorded rather than quietly dropped:** `docs/epic-1483/CONSTRAINTS.md` required the deprecation lint to land *before* any FFI-crossing read path; the §18A.3 structural lift shipped and `rust-ssot` became the unconditional default with no such lint in existence, so that ordering had already been violated when this decision was taken. Because the decision is admit there is no deprecation to sequence and no adopter migration to write; but the rule fired unobserved once, and any future attempt to narrow this domain must treat that as evidence that a precondition written only as prose is not a gate. The gates that replace the prose: `packages/core/test/value-domain-bound-270.test.ts` fails if a bound appears on the adopter-facing type parameters, if this paragraph stops being the single place the decision is stated, or if the two reconciled documents regrow the narrowing; `packages/core/test/value-domain-open-270.test.ts` runs the evidence and is the compile fixture this section's decision rests on.
...
1. **Multi-process and multi-tab races are out of scope.** The property suite runs in a single process. `@causl/sync` handles the multi-process case at the network seam; the *cross-graph* race class is the §13 "multi-user synchronisation" item.
...
What ships in v1 is a **static IR linter**: `@causl/checker` (npm wrapper) plus the Rust binary `causl-check`, running a fixed set of one-shot passes over a JSON IR exported from `@causl/causl-wasm-ts`. What this section originally drafted; a bounded model checker that exhaustively enumerates async-resolution orderings, message-dispatch orderings, and conditional-derived branches; is **deferred and not planned** (#272 closed not-planned; EPIC #282 parks the work).
...
The race-class rows from §9.1 the linter actually addresses live in `docs/checker-coverage.md`: the single source of truth for coverage, re-baselined against §9.1's seventeen rows. The CI gate is the `checker-gate` job in `ci/parked/ci.yml`, required-green on every PR.

### 16.2 The IR contract

`CauslModel` is the wire format between `@causl/causl-wasm-ts` and `causl-check`. Its shape is closed at seven top-level fields and at two `kind` constants (the two-`kind` `IRNode` discipline preserved across the schema 2 → 3 widening that EPIC-1 PR-A applied; see §16.2.1 for the full schema 3 specification):
...
The schema version is `CAUSL_MODEL_SCHEMA = 3` (`packages/core/src/ir.ts`). The release-checker workflow at `ci/parked/release-checker.yml` enforces a three-way lockstep: Cargo `version` ↔ npm `@causl/checker` `version` ↔ `CAUSL_MODEL_SCHEMA` constant. A bump in any without the matching companion fails the `version-lockstep` job; nothing publishes on a mismatch.
...
**Current state (review 2026-07-06, M7 scope).** `IRDerived.conditionalDeps` is **reserved-empty**: the shipped `rust-ssot` exporter emits `[]` unconditionally (the TS floor's exporter did the same while it shipped here), so the field carries no data today and the `UnknownDep` / `Cycle` passes that consume it are vacuous on it. It is retained as a wire slot pending a populator. This is orthogonal to the *runtime* dynamic-dependency contract, which is live and normative: the engine re-captures each derived's dependency set on **every** Phase-D recompute and rolls the set back on a Phase-D throw (§5.1 Amendment 6); that runtime set simply is not projected into `conditionalDeps` yet. (A broader reconciliation of the schema-3 event machinery against the shipped subscribe-only exporters; the five unpopulated `IREvent` variants, the discarded `captureCallGraph`, per-export id minting, always-empty `bridges`; is tracked separately and is out of scope for this amendment.)
...
export interface IRDerived {
  ...
  /**
   * RESERVED-EMPTY (review 2026-07-06, M7). Both shipped exporters
   * emit `[]` unconditionally — there is no populator today. The
   * `UnknownDep` / `Cycle` checker passes that read this field are
   * therefore vacuous on it. Retained as a wire slot pending a
   * populator; see the §16.2.1 "Current state" note. Dynamic deps
   * are re-captured at runtime every recompute (§5.1 Amendment 6),
   * but that runtime set is not projected into this IR field yet.
   */
...
export interface IRSubscribe {
  ...
  readonly graphId: IRGraphId
  /**
   * Subscription id. NOTE (review 2026-07-06, L5): both shipped
   * exporters mint this id **per export**, not at registration, so
   * it is NOT stable across successive `exportModel` calls and MUST
   * NOT be used as a cross-export join key (e.g. `originEvent` /
   * dispose linkage). Registration-stable ids are a reserved future
   * tightening; today's value is export-local.
   */
  readonly id: string
...
Diff against the version-lockstep step in `ci/parked/release-checker.yml`:
...
- **`@causl/checker`**: bumps in lockstep.
- **`@causl/devtools-bridge`**: schema-3 PR adds projection of `events`; until then, ignores the field (forward-compatible).
...
> **Enumerator-model-only (review 2026-07-06, cross-ref §18A.1.1).** The `State` struct below: including its sorted `observers: ImOrdMap<NodeId, im::OrdSet<ObserverId>>`; is the **bounded-enumerator search model**, not the shipped engine's runtime state. Its sorted/`OrdMap` containers exist so the enumerator's state is content-addressable and hashable; they are **not** the normative Phase-G dispatch-order source. The shipped engine sources fire order from the per-cell insertion-ordered observer list and `subscriptions_in_order` (§5.1 Amendment 4, §18A.1.1 Amendment). Do not read `State::observers`'s sort order here as a fire-order mandate; a native backend that iterated a sorted observer map for its Phase-G fan would violate §5.1 Amendment 4.
...
The wire format is intentionally JSON, not bincode: `@causl/causl-wasm-ts` is TypeScript, the worker is Node, and JSON is the protocol every Node program already speaks. The `dashmap` content-addressed cache makes a hot-path hit O(1) without crossing the process boundary, paying back the parse overhead.
...
```ts
import { hypothesis, always, afterCommit, implies } from '@causl/hypothesis'
...
2. **Async resolution + Msg trajectory.** State space extends to pending resource resolutions and message-dispatch traversal. Acceptance: rows 2, 6 on a fixture with `@causl/sync` resources.
...
`@causl/checker` is published with `optionalDependencies` per-platform packages: `@causl/checker-{linux-x64, linux-arm64, darwin-x64, darwin-arm64, win32-x64}`. The wrapper resolves the matching package at install time via `os` / `cpu` filtering (no postinstall network fetch) and execs its binary on invocation.

The release workflow lives at `ci/parked/release-checker.yml`. A `checker-v*` git tag triggers the matrix across five `runs-on:` legs; `workflow_dispatch` runs the build-and-checksum legs without minting a tag. Per #390 / #454: the distribution rails were the open work that turned the linter from an in-tree CI binary into an adopter-facing dependency.
...
| 6 | Stale-async resolution | RUNTIME-ONLY | `originGraphTime` compare in `@causl/sync`. Network is the oracle. |
...
Tally (review 2026-07-06, L3: one convention: every row counted at the layer that ships **today**, with named-gate promotions noted rather than pre-counted): STATIC = 10 (rows 1, 2, 4, 8's declared-edge half, 12, 13, 14, 15, 16, 17), PROPERTY = 4 (rows 5, 7, 11, plus the conditional-dep half of 8), MODEL = the conditional-dep gaps in 7 / 8 plus commit-during-subscribe interleavings the property suite reaches but does not exhaust, RUNTIME-ONLY = 4 (rows 3, 6, 9, 10). Two rows promote to STATIC on a named gate and are counted here at their **present** layer, not their future one: row 3 (RUNTIME-ONLY today → STATIC when the §9 four-state union lands) and row 11 (PROPERTY today → STATIC when the `UseAfterDispose` pass ships); both promotions together take STATIC to 12. (The prior tally counted row 11 at its future layer inside STATIC while counting row 3 at its present layer; the tense inconsistency this rewrite removes.)
...
- **PR A: Schema 3 foundation (LANDED).** Bumped `CAUSL_MODEL_SCHEMA: 2 → 3`. Added `graphId` to `IRInput` and `IRDerived`; added event records `IRSubscribe`, `IRSubscribeCallback`, `IRUnsubscribe`, `IRDispose`, `IRRead`, `IRTxSet`; added the call-graph annotation slot on `IRCommit`; added `IRScope` and `IRBridge` registries. Updated the TypeScript exporter (`packages/core/src/ir.ts`, `graph.exportModel()`) to serialize subscriber-registry, disposal-tombstone, and (bounded) read-trace data the engine already tracks internally. Updated the Rust IR types (`tools/checker/src/ir.rs`) and the `Schema` pass to gate on `schema == 3`. Updated the version-lockstep workflow (`ci/parked/release-checker.yml`); Cargo `version` ↔ npm `@causl/checker` `version` ↔ `CAUSL_MODEL_SCHEMA` constant; to fail the release on a 2-vs-3 mismatch. No new pass code landed in PR A.
...
import { hypothesis, always } from '@causl/checker/hypothesis'
...
```ts
import { defineConfig } from '@causl/checker/config'
...
Production bytes pay zero. `causl-check` is a Rust binary distributed via `@causl/checker` per-platform `optionalDependencies` (already shipped: §16.7), `devDependency` only. Hypothesis files live under `tests/`, never resolved by the production bundle entry. The `@causl/checker/hypothesis` runtime helper ships in `@causl/core-testing-internal` (already a no-publish-to-prod path per §12.3.1). The size-limit ceilings in `package.json` do not move.
...
The previous spec ended with eight commitments, listed flat. The rewrite keeps those eight, marks each as **MECHANICAL** (a CI gate exists) or **DESIGN-DISCIPLINE** (the discipline is enforced by review policy), and adds the four §16A race-detection rows for the layer-classification contract. Twelve in total at signature; #1005 amended in commitment 13 (capability-cost residual) post-wave, taking the table to thirteen; #690's §17.6 amendment adds commitment 14 (host-tier substrate compatibility) as part of the WASM-backend EPIC #680, taking the table to fourteen; this amendment's §17.6 reopening (§13.8 promoted; the wasm engine committed as a conformant alternative) adds **commitment 15** (two-engine byte-identity conformance + the thin-TS-API-over-wasm-core architecture), taking the table to **fifteen**. The signature shape is unchanged: each row marked **MECHANICAL** or **DESIGN-DISCIPLINE**, with the anchor named. Kent Beck's framing of commitments-as-contract is the load-bearing critique. Robert Martin's TDD-as-professionalism framing reinforces the same line. Martin Fowler's interpretation-for-working-teams framing kept the design-discipline rows from being struck.

### 17.1 The fifteen commitments

The previous draft listed eight; §16A adds four race-detection rows; #1005's §17.5 amendment adds row 13 post-wave; #690's §17.6 amendment adds row 14 as part of the WASM-backend EPIC #680; this amendment's §17.6 reopening (§13.8 promoted; the wasm engine committed as a conformant alternative) adds **commitment 15** (two-engine byte-identity conformance + the thin-TS-API-over-wasm-core architecture), taking the table to **fifteen**. The signature shape is unchanged: each row marked **MECHANICAL** or **DESIGN-DISCIPLINE**, with the anchor named.
...
| 4 | Every discriminated union in §9 is a tagged union with exhaustiveness via `assertNever`. | MECHANICAL | `assertNever` from `@causl/causl-wasm-ts/internal`; per-adapter `*.test-d.ts` exhaustiveness fixtures. |
| 5 | The race-class catalogue in §9.1 is current as the engine grows. | DESIGN-DISCIPLINE | §9.1 plus the PR template anchor at `.gitea/PULL_REQUEST_TEMPLATE.md` (#399 / #430). |
...
| 8 | `causl-check` ships as a required CI gate alongside `tsc` and the property suite. | MECHANICAL | `checker-gate` job in `ci/parked/ci.yml`; v1 = static linter, deferred enumerator excluded. |
...
| 12 | Every new §9.1 row ships with a detection-layer assignment in the same PR. | DESIGN-DISCIPLINE | `.gitea/PULL_REQUEST_TEMPLATE.md` race-class anchor extended; §16A.1 layer table. |
| 13 | The replay-determinism contract premium is paid honestly: causl wall-clock stays inside `mobx_median × 3.0 ≤ causl_median ≤ mobx_median × 8.0` on contract-bearing cells, anchored at `1.84× engine baseline + 3.5× contract premium` per §17.5 (#1005 amendment). | MECHANICAL | `MEDIAN_BAND_INVARIANTS` in `packages/bench/src/hypotheses/causl-hypotheses.ts` **in the `causl/causl-core-ts` fork** (the benchmark workspace is not in this repository, so every `packages/bench/...` path in this section resolves there); `pnpm bench:check-hypotheses` median-band gate; `packages/core/test/properties/spec-17-capability-cost.property.test.ts`. |
| 14 | The `@causl/causl-wasm-ts/wasm` engine surface ships against a documented **three-tier host compatibility matrix** (`wasmgc-builtins`, `wasmgc-classic`, `serde-json`); a host that can run none of the tiers fails loud with the structured `WasmBackendUnavailableError`, never silently (§17.6, #690 amendment; the TS-engine fall-through the original row promised was retired by §18A.13, and its removal finished at 0.5.0, §18A.13.1, which records the accepted host cost; the matrix is down to ONE tier since, `wasmgc-classic`, per the §17.6 rider below). | DESIGN-DISCIPLINE | `packages/core/wasm/README.md` host-tier table + `detectBridge()` fallback chain; `docs/wasm-adoption-guide.md` adopter checklist; §17.6 host-tier table and the four-tier feature-detection contract. |
| 15 | I ship **one conformant implementation** of the §3 equation and the §12 public surface: `causl-wasm`, the Rust core reached through this package's thin TS API, exposing the **full** §12.1+§12.2 surface over FFI (not just commit/read/subscribe/derived). Conformance is held against the §18A.1.1 oracle: the pinned `causl/causl-core-ts` floor, reached as captured reference traces and as the live differential leg, plus the frozen golden corpus; the in-process two-engine pair this row originally named lives on in `causl/causl-core-ts` (all §18A.7 criteria were GO on 2026-06-21, and §18A.13 collapsed this package's roster to one at 0.5.0). | MECHANICAL (conformance-oracle gates + FFI-surface-completeness gate) with a DESIGN-DISCIPLINE rider (the promotion that discharged §18A.7 was a named, dated governance decision, not a CI flip) | §18A; `packages/core/test/properties/cross-backend-real-rust.property.test.ts` (the live foreign-floor differential, #126 / #279 S8); `packages/core/test/fixtures/reference-traces/` + `packages/core/test/fixtures/cross-engine/` (captured oracle + disposition ledger); `tools/engine-rs-bridge/tests/bridge_byte_identity.rs` in `causl/causl-core-rs`; the §18A.3 FFI-surface-completeness check; §17.6 Commitment 15 paragraph. |
...
**Contract sentence.** The opt-in `@causl/causl-wasm-ts/wasm` substrate ships against three host-tier bridges plus a documented TS-engine fallback, and every supported host runs *at least one* tier. The matrix is the public contract: adopters read off whether their host gets the fastest path (`wasmgc-builtins`), the WasmGC-only path (`wasmgc-classic`), the universal-compat path (`serde-json`), or the documented JS-engine fallback through `WasmBackendUnavailableError`. No host is silently stranded. *(Dated rider: for `causl/causl-wasm-ts` this sentence now has one tier and no fallback. `serde-json` was retired with the split-crate `engine_rs` lineage (issue #10), `wasmgc-builtins` by causl/causl-core-rs#355, and the TS-engine fallback at 0.5.0 per §18A.13.1. `wasmgc-classic` is the whole roster; a host that cannot run it is stranded loudly rather than silently, which is the half of this sentence that survives. See the rider under the matrix below.)*

**The three-tier host-substrate matrix.**

| Tier | Bridge id | Host floor | Bundle (Brotli) | Per-commit boundary cost | Promotes from |
| --- | --- | --- | --- | --- | --- |
| 1 | `wasmgc-builtins` **(RETIRED, causl/causl-core-rs#355)** | Chromium 131+ / Firefox 130+ / Node 22.6+ | ~45 KB | `externref` round-trip + `wasm:js-string` direct import; no UTF-8 copy on string-typed reads. Never actually loadable: see the rider below | (top tier) |
...
| 3 | `serde-json` **(RETIRED, issue #10)** | Universal: WebAssembly 1.0 baseline (Chrome 95+ / Firefox 102+ / Safari 16+ / Node 22.0+ / Cloudflare Workers / Deno 1.30+) | ~80 KB | `serde_json` serialise + parse per read | Tier 2 |
| Fail-loud (no fallback engine) | `WasmBackendUnavailableError` | Any host where `WebAssembly.Module` fails to instantiate (CSP `script-src` without `'wasm-unsafe-eval'`; embedded runtimes; `WebAssembly` undefined entirely) | 0 | n/a (no engine constructed) | Tier 3 (this row named a TS-engine fall-through until §18A.13 retired it; §18A.13.1 records the accepted host cost) |
...
**Dated rider (`causl/causl-wasm-ts`, 2026-08-07): the matrix above is down to one row, and the walk it describes does not run.** Two of the three tiers are retired. `serde-json` went with the split-crate `engine_rs` lineage (issue #10). `wasmgc-builtins` was retired by causl/causl-core-rs#355 on causl/causl-core-rs#210's measurement: its `wasm:js-string` imports ship `i32`-typed where the W3C builtins are `externref`-typed, so `WebAssembly.compile(bytes, { builtins: ['js-string'] })` refuses the artefact on every host, and the wasm-bindgen externref transform never rewrites the hand-written import block that produces them. It was therefore never loadable anywhere, which is why it is retired rather than deprecated: the retirement costs no host anything. causl/causl-core-rs#358 carries the ABI retype that would bring it back. What ships is `wasmgc-classic`, and `detectBridge()` returns it unconditionally, so probes 2 through 4 below describe a promotion path with nothing to promote to, and the "Promotes from" column has no live edge. The rows are kept rather than deleted because an adopter holding `bridge: 'wasmgc-builtins'` in a config needs to find out what happened to it. Asking for either retired id throws, and the gc-builtins message names the measurement rather than reporting a host mismatch, because it is not one. The fail-loud row is now the entire fallback contract: below `wasmgc-classic` there is a throw, not a TS engine (§18A.13.1).
...
| --- | --- | --- | --- |
| `@causl/causl-wasm-ts` main bundle | unchanged from pre-EPIC | 20 KB (§14.2) | Zero WASM imports, zero loader code: adopters who never call `loadWasmBackend()` pay nothing |
| `@causl/causl-wasm-ts/wasm` entry stub | 630 B Brotli | 3 KB ceiling (per #684 size-limit) | Public API surface: `loadWasmBackend`, `detectBridge`, `loadStreaming`, `wasmUrlFor`, `WasmBackendUnavailableError`, `BridgeFeatures` |
| `wasm-pkg/gc-builtins/` (Tier 1 artefact) **RETIRED** | ~110 KB raw / ~45 KB Brotli | 60 KB Brotli ceiling | Includes the JS bindings glue produced by `wasm-pack`. The tree and its size-limit cell were deleted by causl/causl-core-rs#355; nothing measures this row |
...
**The fail-loud floor of the matrix (what an unsupported host gets).**

Hosts that fail probe 1: `WebAssembly` undefined, CSP `script-src` rejecting `'wasm-unsafe-eval'`, embedded runtimes with WASM stripped; get `WasmBackendUnavailableError`, and since 0.5.0 `createCausl()` itself is construct-or-throw (§18A.13.1): there is no TS engine left in this package to degrade to. The error carries a structured `code` field (`CAUSL_WASM_NOT_BUILT`, `CAUSL_WASM_UNAVAILABLE`, `CAUSL_WASM_CSP_BLOCKED`, `CAUSL_WASM_BRIDGE_UNAVAILABLE`; plus `CAUSL_WASM_NOT_PRELOADED` and `CAUSL_WASM_ENGINE_UNAVAILABLE` at the construction seam, §18A.13.1) so an adopter decides deliberately what an unsupported host should do, in one `try`/`catch` at boot:

```ts
import { preloadCauslWasm } from '@causl/causl-wasm-ts/wasm'
import { createCausl } from '@causl/causl-wasm-ts'

await preloadCauslWasm() // the one async seam, at app init (§18A.12)

let graph
try {
  graph = createCausl() // returns the wasm engine or throws (§18A.13.1)
} catch (err) {
  // No engine exists on this host. This branch is the adopter's decision:
  // show an unsupported-host screen, report telemetry, or rethrow.
  throw err
}
```

**Commitment 14's framing has been amended twice, and both moves are recorded rather than overwritten.** The original #690 text read "the WASM substrate is *acceleration*, not *substitution*", with the TS engine as the unconditional floor beneath the matrix ("any host that runs JavaScript runs causl"). The §13.8 reopening retired that line in favour of "conformant alternative now, planned replacement under §18A.7", keeping the TS floor beneath it and requiring that any flip of `createCausl()` toward WASM amend §17.6 / §18A.7 explicitly and record the GO decision in a dated amendment. Both requirements were then met the way they demanded: the 2026-06-21 promote-default row, the §18A.8 fail-safe removal, and §18A.13 / §18A.13.1's roster collapse are those dated amendments. Today the matrix's meaning is narrower: which host gets which bridge is unchanged, and a host below every tier fails loud instead of landing on a TS floor this package no longer carries.
...
### 17.6.1 Commitment 15: two-engine byte-identity conformance and the thin-TS-API-over-wasm-core architecture

**Status:** new commitment added by this PR as the SPEC-side realisation of the §13.8 reopening. Commitment 14 (host-tier substrate compatibility) is unchanged in substance: its matrix is the public host contract; Commitment 15 documents that the engine *behind* that matrix is now a committed conformant alternative, and names the architecture (`causl-wasm-ts`, the thin TS API) and the gate (cross-backend byte-identity) that make "two interchangeable engines" a contract rather than a slogan. The full technical contract lives at §18A; this row is its §17 commitment.

**Contract sentence.** The engine ships two conformant implementations of the §3 semantic equation and the §12 public contract: **(A)** `causl-ts`, the TypeScript value-of-record, running natively on the JS event loop; the floor; **(B)** `causl-wasm`, the Rust core compiled to WebAssembly, accessed via a thin TypeScript API (`causl-wasm-ts`, the `@causl/causl-wasm-ts/wasm` integration surface) that delegates to the wasm core over FFI. An adopter constructs either by passing `backend: 'js'` or `backend: await loadWasmBackend()` to `createCausl(options)` and sees bit-identical behaviour at the public surface within the documented marshaling cost (§17.6 G.1). The two engines are interchangeable at the public-contract boundary; the TS engine is the floor until **all** §18A.7 GO/NO-GO criteria are met and a dated amendment records the promotion. *(Read this commitment as binding `causl/causl-core-ts`, which ships both engines. It no longer describes `causl/causl-wasm-ts`: the `backend` option was removed at its 0.5.0 (`causl/causl-wasm-ts#280`) and only implementation (B) is reachable there, so the byte-identity conformance obligation is discharged in that repository by there being nothing to compare; the differential oracle lives in `causl/causl-core-ts`. Neither engine's conformance is weakened by that; the comparison simply moved.)*

**The thin-TS-API-over-wasm-core architecture (the honest framing).** `causl-wasm-ts` is **not** a copy of the TS engine and **not** the heavy ~7.5k-LOC TS shell. It is a thin adapter that (1) implements the public `Graph` interface (§12.1 canonical seven + §12.2 second-tier fifteen) by delegating to the wasm core over FFI; (2) marshals parameters across the JS↔WASM boundary (§17.6 G.1 cost measured, not hidden); (3) lifts the structural `engine-rs-core` surface: dependencies/dependents (+ transitive closures), commit-log, commit-metadata, handle/disposal/StaleTx validation, stats; into the public FFI contract (today these are internal to `engine-rs-core`; §18A.3 makes them FFI-exposed and tested at the byte-identity gate); (4) preserves the §3 Theorems 2–4 lifecycle/rollback/atomicity guarantees and the §5 Phase A–H "no intermediate time" contract; (5) preserves the §15.3 per-commit subscriber-fire semantics (mid-batch fire under the epic-#1493 batched-boundary opt-in). This is an architectural departure from "one TS engine, period" to "two conformant engines, identical public surface, distinct substrates, byte-identical via the gate"; recorded as such, not minimised.

**Byte-identity gate (cross-backend determinism).** The §15.1 property suite (1000 trials per run, enforced by lint per §15.2) includes a per-flush cross-backend determinism check: given the same action sequence, the TS engine and the wasm engine produce **byte-identical** `Commit` records, graph snapshots, structural-query results (dependencies/dependents/commit-log/stats), and subscriber fire-order. The gate fires per-flush under the batched-boundary opt-in (epic #1493 C.5); for default single-commit envelopes (`afterN=1`) the check is per-commit and byte-identical to the bare TS engine. A PR that breaks byte-identity fails CI; this is the load-bearing test for "interchangeable at the public contract" and one of the two §18A.7 MECHANICAL gates. *(In `causl/causl-wasm-ts` since 0.5.0 the in-process pair no longer exists; the gate's role here is filled by the §18A.1.1 oracle: the live foreign-floor differential in `cross-backend-real-rust.property.test.ts` plus the captured reference traces, while the in-process two-engine gate lives on in `causl/causl-core-ts`.)*

**Marshaling-cost disclosure (standing, not hidden).** The JS↔WASM boundary cost is measured against the production `serde-json` bridge artefact (§17.6 Migration callout, Phase G.1 #1145): `op-rust-bridge-floor-1k` at **1.687 μs/round-trip** (FFI boundary alone), `op-wasm-boundary-1k` at **2.575 μs/round-trip** (boundary + `transition_phased(Action::Tick)`), and the F-marshal.6 full bidirectional commit envelope (PR #1478) at **15.64 μs/commit**. Projected per 10k commits: **156.4 ms** marshal cost; **78×** the current TS-engine `equality-cutoff × 10000` median of 2.017 ms. This is the *wire tax* before the wasm engine does any work; it is the floor any wasm-path adopter pays and the ceiling a post-promotion Rust-SSOT cutover must beat. Documented here so every adopter making a wasm-vs-js choice has the arithmetic in front of them.

**Why MECHANICAL with a DESIGN-DISCIPLINE rider.** The two halves of Commitment 15 reduce to gates that exist: byte-identity is a 1000-trial property comparator (a number → a comparator, like Commitment 13), and FFI-surface completeness is a fixture check that fails CI if any §12.2 row has no FFI extern (§18A.3). Those are MECHANICAL. The *promotion* half (flipping the alternative to default) is deliberately **not** mechanized: it is a named, dated governance decision by the same authority that opened §13.8, recorded in a follow-up amendment, exactly as §17.4 argues for the rows a CI flip would mis-fire on. Commitment 15 therefore carries both marks: the conformance is mechanical; the promotion is discipline.

**Compatibility with §17.4 / §17.5.** §17.5's lower-bound clause named "promotion of the opt-in `@causl/causl-wasm-ts/wasm` substrate to the default engine" as the architectural escape valve a sub-3× ratio would require; Commitment 15 is the contract that escape valve resolves into, and §18A.7 is its bar. The §17.5 3.0×–8.0× capability-cost band is **unchanged at v1.x**: the TS engine stays SSOT for adopter workloads (option-c Answer C), so the band only moves if/when the wasm engine becomes the default, which is the §18A.7-gated future decision §17.5 already scoped out.

**Migration (dated; superseded for this package).** As written, this paragraph promised adopters who never imported `@causl/causl-wasm-ts/wasm` zero behavioural change, with the TS engine as the default and the floor; it committed the alternative, not the default, and demanded a dated follow-up amendment for any promotion. The follow-ups it demanded then happened: the 2026-06-21 promote-default amendment, the §18A.8 fail-safe removal, and §18A.13 / §18A.13.1's roster collapse. Today every adopter of this package is on the wasm engine, the §18A.5 read-identity note is required reading for all of them rather than for a wasm-opt-in minority, and the two-engine semantics survive only in `causl/causl-core-ts`.
...
- **Fifteen commitments** (§17), the original eight plus the four §16A race-detection rows plus the post-wave capability-cost residual amended in by #1005 plus the host-tier substrate-compatibility row amended in by #690 plus the two-engine byte-identity conformance row (Commitment 15) added by the §17.6 reopening, each marked MECHANICAL or DESIGN-DISCIPLINE.

The §0 promise was a single-digit canonical surface, and the rewrite keeps the framing honest: the canonical seven (§12.1) plus a curated second tier (§12.2), audited quarterly. **Twenty-one public `Graph` members today**; six canonical members plus fifteen second-tier (seven canonical *entrypoints* counting the `createCausl` factory, which is not a `Graph` member); with a named promotion / demotion bar between §12.2 and §12.3 and a parking lot at §12.4 that stays empty until the next public-surface proposal lands.

The arc from §0 to §18 is the spec's own contract. §0 names what the previous draft tried to be and why that was the bug. §3 writes down the meaning. §4–§13 are implementation strategy for evaluating §3 efficiently while keeping the lifecycles, the layering, the surface, and the deferrals honest. §14–§16 are the gates that keep the implementation from drifting from the meaning; §16A is how those gates compose into a CI tier. §17 is the team's signature on the fifteen commitments that hold the rest in place. §18 is the arc closed.

If the fifteen commitments hold, the system has a chance to be small enough to teach, formal enough to verify, and live enough to be worth the comparison to spreadsheets that the previous spec made and could not back. If they do not hold, this becomes another state library nobody finishes reading. Everything else has to earn its place by being asked for, twice, by people who have already used the part the team shipped. That is what the team is committing to, and the spec is where that commitment is written down.

---

## 18A. The engine contract (causl-wasm through the thin TS API), with the cross-repo two-engine record

When I first wrote this section, §18 closed the arc on a single TypeScript engine and §18A was the contract for the second one: the wasm core committed in §13.8 and §17.6 Commitment 15. The roster has since inverted (§18A.13, finished at 0.5.0): the wasm core is the one engine this package ships, and this section is now two things at once. It is the live contract for that engine: the conformance oracle (§18A.1), the Node-target requirement (§18A.2), the FFI surface (§18A.3), the thin-TS-API definition (§18A.4), the read-identity migration (§18A.5), the FFI single-tick atomicity invariant (§18A.6, cross-ref #1333), the build &amp; packaging tooling contract by which a Python webapp CI/CD pipeline obtains the Node-target engine (§18A.11), the synchronous-construction contract (§18A.12), and the roster amendments (§18A.13, §18A.13.1). And it is the dated record of the two-engine contract under which the engine was proven: the GO/NO-GO promotion criteria (§18A.7), the NO-GO/revert conditions (§18A.8), the rejections (§18A.9), and the repository topology that still hosts the sibling floor (§18A.10). The discipline throughout is brutal honesty: the costs are named with the rationale, not buried in an adoption guide.

### 18A.1 One engine, one contract, one foreign oracle

`causl-wasm` is the conformant implementation of the §3 semantic equation and the §12 public surface (canonical seven + second-tier fifteen) that I ship: `engine-rs-core` (generational `NodeId`, `BTreeMap`-keyed JSON values, the seven-named-struct cell shape) compiled to WebAssembly and reached over FFI. `causl-ts`, the TypeScript value-of-record on the JS event loop, is the second conformant implementation of the same contract; it ships from `causl/causl-core-ts` and serves this repository as the conformance oracle (§18A.1.1). The *behaviour* at the public surface must not differ between the two; that is the whole point of the gate, and here it is judged against the foreign oracle rather than between two in-process engines.

#### 18A.1.1 The cross-backend byte-identity gate (load-bearing)

Given the same action sequence, the engine MUST produce `Commit` records, graph snapshots, structural-query results (dependencies, dependents, commit-log, commit-metadata, stats), and subscriber fire-order byte-identical to the reference floor's. The oracle has four arms. The LIVE arm (`packages/core/test/properties/cross-backend-real-rust.property.test.ts`, #126 / #279 S8) drives the §15 command alphabet, including a freshly generated `fc.commands` log per run, through the real Rust engine and through the pinned `causl/causl-core-ts` floor (`9c3d947`, resolved in-process from the checkout `CAUSL_CORE_TS_CHECKOUT` names) and diffs them after every command. The CAPTURED arm compares live runs against reference traces of that same foreign floor (`packages/core/test/fixtures/reference-traces/`, #277), with a mechanical distinct-engines refusal so a wasm run can never wear the oracle's name, and with `packages/core/test/fixtures/cross-engine/` as the per-surface disposition ledger. The FROZEN arm is the golden-vector corpus captured before the 0.5.0 strip (#39). The Rust side pins `tools/engine-rs-bridge/tests/bridge_byte_identity.rs` in `causl/causl-core-rs`. Property arms enforce at the §15.2 1000-trial floor. Zero-byte divergence on every replayed commit is the pass condition; any divergence is a CI failure and a §18A.7 Criterion-1 NO-GO. The container invariants that make byte-identity reachable on the Rust side are pinned by §5.1 Amendment 4 (#1333): the per-node subscriber index MUST preserve insertion order (`IndexMap<NodeId, SmallVec<[ObserverId; 2]>>`), and `engine-rs-core`'s prior `BTreeMap<NodeId, BTreeSet<ObserverId>>` (sorts by id) is **non-conformant**; promote it to insertion-order semantics concurrent with the FFI surface work (§18A.3).

**§18A.1.1 Amendment (review 2026-07-06): the shipped fire-order-bearing structures, named.** The "promote the `BTreeMap` to insertion order" mandate above is **stale against the shipped Rust core** and never named which structure is the normative Phase-G order source. Byte-identity is not carried by promoting `State::observers`; it is carried by **parallel** order-bearing structures the shipped engine already maintains: the **per-cell insertion-ordered observer list** (`engine-rs-core` `state.rs`, a `SmallVec` per cell) and **`subscriptions_in_order: Vec<SubscriptionRecord>`**, plus bridge-side ordering. The lookup-oriented containers; `State::observers` (still declared `BTreeMap<NodeId, BTreeSet<ObserverId>>`, sorts by id) and the node-grouped runtime index (`subscribers_by_node`, an `FxHashMap`); are **lookup indexes only and MUST NOT drive Phase-G dispatch order**. A refactor that started iterating `State::observers` for the fan would be simultaneously "conformant" by §16.4.1 (which specifies those sorted containers for the *enumerator* model) and non-conformant by this amendment and §5.1 Amendment 4; the two must not be conflated. Normative rule: Phase-G fire order is sourced from the per-cell insertion-ordered observer list and `subscriptions_in_order`; the sorted/hashed indexes exist for lookup and never for ordering.

### 18A.2 The Node-target requirement

The wasm core MUST ship a **Node.js target artefact** (`wasm-pack --target nodejs`) paired with a Node ESM loader hook, so `import { createCausl } from '@causl/causl-wasm-ts'` works on Node without bundler setup (the `--target nodejs` glue is synchronous at require-time, §18A.12, so no `await` reaches the call site). When I wrote this the ship was `--target bundler` only, and the `nodejs` target plus loader hook were the missing piece; both have since shipped (§18A.7 Criterion 4, GO on 2026-06-21). The requirement stays normative because it was a **hard requirement** for promotion and remains one for the shipped artefact: the thin TS API targets TS/Node apps, and a Node adopter must not have to reason about bundler integration to use the engine. Targets: Node.js 18+ (`WebAssembly.instantiate` baseline since 18.0.0; the shipped `wasmgc-classic` bridge needs only WasmGC; the Tier-1 `wasmgc-builtins` and Tier-3 `serde-json` floors this parenthesis used to quote are retired, per the §17.6 rider), TypeScript 5.0+, bundlers Vite / webpack 5+ / esbuild.

### 18A.3 The wasm-core FFI surface requirement (the spine of the work)

When I wrote this section the bridge exposed only the hot-path primitives (`apply_commands`, `read_input`, `read_derived`, the Phase D compute-bridge and Phase G subscriber-bridge, plus snapshot/hydrate/`bridge_id`), and the structural state lived **internal** to `engine-rs-core` (`State::dependents` reverse-adjacency, `State::commit_log` VecDeque, `State::observers` per-node index, `State::disposed` tombstones, per-cell generation tags / last-write-time / version). For the thin TS API to satisfy the **full** §12 surface without any JS-engine fallback, the wasm core MUST surface the structural inspection externs. That gap was the spine of the work, and the lift landed: `causl/causl-core-rs#170` closed with every projection read resolving from Rust, so the table below is the shipped extern contract rather than a plan.

**Required new externs (≈12–15, all `#[wasm_bindgen]`-public to wasm32-unknown-unknown):**

| Group | Extern | Returns | Backs §12 row |
| --- | --- | --- | --- |
| Structure | `read_dependencies(slot,gen)` | binary `Vec<NodeId>`, insertion-ordered forward edges (`DerivedCell::deps`) | `Graph.dependencies`, `Graph.explain` |
| Structure | `read_dependents(slot,gen)` | binary `Vec<(NodeId,ObserverId)>`, insertion-order per node (§5.1 Amd 4) | `Graph.dependents`, `Graph.explain` |
| Commit metadata | `read_commit_log()` / `read_commit_log_entry(i)` / `commit_log_len()` | JSON array / one `CommitRecord` / `u32` | `Graph.commitLog`, `Graph.commitMetadataDerived` |
| Handle/disposal | `check_disposed(slot,gen)` | `bool` (generation mismatch → thin layer throws `NodeDisposedError`) | disposal guard on `read`/`subscribe`/`explain` |
| StaleTx | `validate_tx_alive()` | `bool` (the §3.1 precondition gate; `false` → thin layer throws `StaleTxError`) | commit-pipeline precondition |
| Per-node metadata | `read_input_last_write_time(slot,gen)` / `read_derived_version(slot,gen)` | `Option<u64>` / `Option<u32>` | equality-cutoff / version-counter memoisation key (§18A.5) |
| Counts | `input_count()` / `derived_count()` | `u32` live-slot counts | `Graph.stats` |
| Telemetry | `engine_stats()` | JSON `{ inputCount, derivedCount, nodeVersion, stateHash }` (`EngineTelemetry` shape) | `Graph.stats`, the byte-identity gate's state hash |

**Wire-format discipline.** Every return value goes through the existing `encode_value_record` / `encode_dep_pairs_into` / `snapshot_state_inner` codecs so the cross-backend byte-identity gate (§18A.1.1) holds: JSON key order per §15.1, container insertion-order per §5.1 Amendment 4, numeric handling per §5.1 Amendment 7 (ECMA-262 SameValue over the shipped wire value domain; **not** §16.4.1, whose `State` is the bounded-enumerator model by its own opening note). Scalars (counts/times/versions) cross as native `u32`/`u64` wasm-bindgen values; the thin layer unpacks via the existing `unpack_value` pattern. **FFI-surface-completeness check (MECHANICAL):** a fixture (`tools/engine-rs-core/tests/ffi_surface_complete.rs` + the thin-layer adapter test) fails CI if any §12.2 row routes through a closure-private struct instead of an extern. This is the §17.6-Commitment-15 completeness gate and §18A.7 Criterion 2's MECHANICAL anchor. Scope/phasing: lands in parallel with or immediately after the epic-#1133 A.12 `Transitioner`/`Reader`/`Persister`/`Observable` trait scaffolding (#1360); the Phase D (#1136) and Phase G (#1135) FFI bridges are prerequisites.

**Honest caveat (gen-tag reconstruction).** #1151's generational tags are in-memory only (wire format is bare `u32` slot); on hydrate, deserialised `NodeId`s land with `gen=0`. The thin layer MUST NOT call `check_disposed` on ids reconstructed from IR or committed snapshots: only on ids returned by the engine's own `create_input` / `create_derived`. Documented as a precondition on the `check_disposed` extern, enforced by the thin layer's contract.

### 18A.4 The thin-TS-API definition (causl-wasm-ts)

`causl-wasm-ts` is the TS-side FFI binding for the wasm engine: the `Graph` facade delegates every public method through the JS↔WASM marshaler to the externs of §18A.3. (While both engines shipped here, the binding was the third actor at the `BackendEngine` seam in `packages/core/src/backend.ts`, alongside a `JsBackend` wrapping the TS engine; the seam survives as the injection point the wasm loader constructs through, and the roster behind it is one engine.) What `causl-wasm-ts` **is not**, stated plainly so the boundary stays honest: not a copy of `graph.ts`; not the ~7.5k-LOC TS shell under a new name; not "the same TS engine, just calling into WASM." The TypeScript value-of-record ships only from `causl/causl-core-ts` (§18A.10); an adopter of this package is on the wasm engine, with no opt-out. The canonical way this engine was built is **not** a rewrite of the TS engine in Rust; it is a Rust engine stood up from scratch against the same §0/§12 contract and proven byte-identical via the §18A.1.1 gate. `causl-wasm-ts` is the thin TS binding for that engine; the wasm core ships as a pre-compiled `.wasm` per the EPIC #680 wasm-pack integration, and `causl-wasm-ts` ships no Rust source. Which repository owns which artefact; the Rust engine source, the thin TS binding, and the TS value-of-record; is recorded in §18A.10. The build &amp; packaging tooling that produces the `--target nodejs` `.wasm` artefact (§18A.2) and places it where a consuming app vendors it is **not** part of `causl-wasm-ts`: it is the Python tooling (`build_wasm.py` + `package_wasm.py`) that lives in `causl/causl-core-rs`, whose contract is §18A.11. `causl-wasm-ts` *consumes* the placed artefact; the loader hook of §18A.2 resolves it via `node:fs`; and ships no Rust source and no build tooling.

### 18A.5 The read()-reference-identity migration (cross-ref §15.1 / #1124)

**The contract change at the Rust-swap boundary.** The TS floor cached values in its cells and returned the same JS reference per `read()` within a commit window. Per the §15.1 amendment (#1124), reference identity across commits is **not** contractually guaranteed; value identity at a fixed `GraphTime` (`Object.is(read(node)@t, read(node)@t)` for two synchronous reads at the same time) **is** preserved by the shipped engine (and by the reference floor). The earlier draft of this section claimed the real Rust engine returns "a **fresh object per call**" uniformly: corrected by the 2026-07-06 review amendment (H2): that is **not** what the shipped `rust-ssot` engine does, and the false uniform-breakage story describes the wrong hazard.

**Reference identity under the shipped `rust-ssot` engine is cache-dependent, not uniformly fresh.** The wasm marshaler retains a `ValueHandleCache` (`packages/core/wasm/authoritative.ts`, `#valueCache = new ValueHandleCache<NodeId>()`) that holds the **original adopter JS reference** for a marshaled value. The cache is non-evicting for live cells **by construction, not by capacity**: it used to be asked for that unboundedness with a `Number.MAX_SAFE_INTEGER` argument, which left the class doing LRU bookkeeping that could never evict, so issue #298 (PR #311) deleted the LRU, the capacity parameter and the constructor together; `ValueHandleCache` now declares none (`packages/core/wasm/value-buf.ts`). On the common path `read()` is served straight from that cache **without** a wasm read + `JSON.parse`, so it returns the *same* reference the adopter handed in; identity is *stable*, not fresh. A **fresh** object appears only on the cache-miss / decode paths: values reconstructed on hydrate, and Rust-computed values not present in the cache. Reference identity is therefore **non-deterministically unstable**; stable while the cache holds the value, fresh on decode paths; which is precisely why §15.1/#1124 removes it from the contract. The real hazard is *intermittent* (breaks on post-hydrate and Rust-computed reads, not on every commit), which is harder to catch in an adopter's own testing than a uniform break would be.

**The migration, staged:**
1. **v0.9.0 (dated record).** `WasmBackend` was a TS engine wrapped in the FFI shape; `read()` returned identical references trivially.
2. **At the Rust-swap boundary (the shipped `rust-ssot` engine, epic #1133 and successors): the live contract.** `WasmBackend` runs the real Rust engine behind the `ValueHandleCache`. `read()` returns the **original cached reference on the common (cache-hit) path** and a **fresh object only on decode paths** (post-hydrate values, Rust-computed values not in the cache). Adopters who memoised on read identity do **not** re-render every commit uniformly: they observe stable identity on cache-hit reads and then break *intermittently* on the decode paths. An adopter who audits against the shipped engine, sees stable identity, and concludes the audit is unnecessary is exactly the failure this corrected narrative warns against: the break is cache-dependent and non-uniform, not absent.
3. **Pre-migration checklist (`docs/wasm-adoption-guide.md`, required reading before a real-engine upgrade):** audit every `React.memo(...)` and `useMemo(() => transform(value), [value])` whose key is the `read()` return itself; migrate memoisation keys to `commit.time` (`GraphTime` on the published `Commit`) or the per-node version counter from `EngineTelemetry` (the `read_derived_version` extern, §18A.3).
4. **The guard.** `packages/core/test/properties/read-no-identity-contract.property.test.ts` wraps a backend in a fresh-copy decorator returning deep clones on every `read()`; the suite staying green proves no internal engine code relies on identity. This is the honest pre-disclosure of a silent break, surfaced before the swap rather than after.

### 18A.6 The FFI single-tick atomicity invariant (cross-ref §3 Theorem 2 / #1333)

Theorem 2 (glitch-freedom, "no intermediate time") was proven on the TS floor by single-threaded JS event-loop evaluation: Phase D evaluates and mutates the derivation table before Phase E publishes the `Commit`, so no subscriber ever sees a sealed `Commit` against a pre-commit derived value. **That proof does not survive an FFI boundary without additional machinery.** The §3 Amendment (#1333) pins the contract: when the engine runs outside the JS event loop (a WebAssembly linear-memory backend), the marshal of a `Commit` envelope across the host boundary MUST be atomic w.r.t. JS-observable scheduling; no microtask, `requestAnimationFrame`, `MessageChannel` callback, or any `await`-able continuation may run between engine-side Phase E (`Commit` sealed in engine memory) and host-side Phase G (subscriber callbacks fired in the TS host). The wasm bridge enforces this **by construction**: the FFI entry point (`apply_commands` / `commit_batch`) is a **synchronous** `#[wasm_bindgen]` extern, not async. Calling `await apply_commands(...)` from the TS host is syntactically permitted, breaks the uninterruptibility contract, and violates Theorem 2 silently; the engine cannot defend against a contract violation one layer up the stack. Documented at `packages/core/wasm/README.md` and the adoption-guide checklist; the Phase-G fire-order is pinned by §5.1 Amendment 4. A TS engine satisfies the invariant trivially (single-threaded); the engine I ship crosses the FFI boundary on every commit, so this clause is load-bearing here, and the synchronous extern is the single non-negotiable piece holding Theorem 2 across the boundary.

### 18A.7 GO/NO-GO criteria gating alternative → default/replacement promotion

**This gate is discharged; I keep the rows as the dated bar the engine cleared, and as the live gate for `causl/causl-core-ts`'s roster.** As written, the wasm engine was a committed **alternative**: it becomes the **default** (route through WASM first, TS fallback) or the **replacement** (TS demoted from value-of-record) **if and only if all** of the criteria below are met. The §19 trail records all five criteria GO on 2026-06-21, the default promotion, and the §18A.8 fail-safe removal; §18A.13 records this package's separate, gate-bypassed roster strip. **No single criterion is sufficient; all must pass.** Promotion is **not** automatic on merge: it is a named decision by the governance authority that opened §13.8, called out in a dated follow-up SPEC amendment (parallel to this one's structure), so the next amendment can record "all criteria met as of [DATE]" rather than re-litigate.

| # | Criterion | Type | Gate / measurement |
| --- | --- | --- | --- |
| 1 | **Correctness: cross-backend byte-identity.** TS and wasm engines produce byte-identical `Commit` records, snapshots, structural-query results, and subscriber fire-order at the same logical moment. | MECHANICAL | §18A.1.1 gate: 1000-trial per-flush cross-backend determinism (`cross-backend-determinism.property.test.ts`, `bridge_byte_identity.rs`). Zero-byte divergence at N=10/100/312/1000 × 1000 replays. |
| 2 | **Completeness: full FFI surface.** Every §12.1 canonical-seven + §12.2 second-tier-fifteen row is reachable over FFI with no JS-engine fallback for any row (dependencies/dependents +transitive, commit-log, commit-metadata, handle/disposal/StaleTx validation, stats). | MECHANICAL | §18A.3 FFI-surface-completeness fixture (`ffi_surface_complete.rs` + thin-layer adapter test): CI fails if any §12.2 row routes through a closure-private struct instead of an extern. |
| 3 | **Performance floor: acceptable marshaling tax.** Representative adopter workloads (10k-commit batches, dependency-heavy graphs, high-frequency ticks) stay under the named ceiling: **`single commit ≤ 250 μs p95 marshal overhead`** and **`batch / large mutation ≤ 5 ms p95 total`**. The 78× floor (156.4 ms / 10k vs 2.017 ms TS median) is the documented standing cost; the ceiling was **set from a cross-backend benchmark** (causl-wasm vs causl-ts, fresh-process per cell: single-commit ~220 μs p95 marshal overhead, large batch/mutation 1.7–4.5 ms p95) and tied to the §14 responsiveness budget; at 250 μs/commit a ≤200-commit interaction stays within the 50 ms RAIL synchronous budget (≤60 commits within one 16 ms frame), and a ≤5 ms batch leaves >11 ms of a 16 ms frame for render. It is the honest perf bar a replacement decision must accept for the committed adopter workloads; it gates "stays responsive," not "beats TS." | MECHANICAL (against the named ceiling) | §17.6 G.1 measurement + the epic-#1133 ceiling; `op-rust-batch-boundary-{10,100,312,1000}` confirms the crossing tax amortises exactly 1/N (1564/156/50.1/15.6 ns/op), so the ceiling is the per-flush marshal cost, not the per-commit tax. **Resolution (2026-06-19): GO / RESOLVED.** The criterion is satisfied on its stated "stays responsive within the §14 RAIL synchronous budget" basis (it gates responsiveness, not "beats TS"). Confirming benchmark this session (`pnpm exec tsx src/cli/run-bench.ts --suite taco-mutation --tier smoke`, fresh process per cell, threads=1, `engine` measurement = full commit→recompute cascade): **6/6 cells PASS the §14.2 RAIL budget (0 abs-breach, 0 regression)**; `causl-wasm` single-input-commit p50 0.92 ms / p95 2.5 ms (within the 50 ms single budget), batch input-commit (200-wide, fixture cap) p50 3.2 ms / p95 5.5 ms (within the 200 ms p95 batch budget), TACO 1000-cell-segment p50 6.8 ms / p95 15.9 ms (within the 200 ms p95 batch budget). The per-flush **marshal overhead** isolated by `op-rust-batch-boundary-{10,100,312,1000}` (1564/156/50.1/15.6 ns/op, amortising exactly 1/N) sits well under the 250 μs single-commit ceiling; the §14-budget end-to-end figures above are the full engine-exec latencies (commit + derive cascade, not isolated marshal) and stay inside the RAIL responsiveness window the ceiling exists to protect. The wasm/ts ratio (6.85×–28.84× p50 on this run) is consistent with, and does **not** refute; the standing 78×/~85× #1133 boundary-tax falsification (Criterion 3 gates responsiveness, not parity). Criterion 3 paired with Criteria 1, 2, 4, 5 forms the load-bearing conformance gate; **all five must be GO** for promotion to default/replacement, so this resolution makes Criterion 3 GO without by itself promoting the wasm engine. Supersedes the prior 2026-05-13 STOP-VERDICT framing (epic #1133, 156.4 ms/10k, 78× boundary tax) for the purpose of *this named criterion*: the STOP-VERDICT measured a "beats-TS" perf-floor that predated the §13.8 reopening and the §18A.7 criterion framing; the criterion-3 ceiling is a post-reopening "stays-responsive" gate, which is met. The #1133 measurement still stands as the documented standing cost (the 78× tax is named, not hidden); it informs the ceiling magnitude but no longer gates promotion under a "beats-TS" reading. **Pinned gate (review 2026-07-06, M2).** To make the MECHANICAL gate adjudicable; the cell above mixes three incommensurable measurements (per-flush crossing-tax `op-rust-batch-boundary` ns/op, the F-marshal.6 bidirectional envelope μs/commit, and end-to-end engine-exec RAIL latencies), so a regression could pass on one basis and fail on another; the single normative quantity Criterion 3 gates is: **the p95 of the F-marshal.6 bidirectional marshal envelope, measured per commit at `afterN=1` (per-commit, not N-amortised) against the production serde artefact, MUST be ≤ 250 μs.** The `op-rust-batch-boundary-{10,100,312,1000}` per-flush figures and the §14.2 RAIL end-to-end latencies are **supporting evidence** (they show the tax amortises 1/N and that the end-to-end path stays inside the RAIL responsiveness window), not the gate itself. A future regression is adjudicated solely against the pinned F-marshal.6 `afterN=1` p95. |
| 4 | **Node target + real-world adoption.** The `--target nodejs` artefact + ESM loader ships (§18A.2), **and** at least one adopter has shipped a production workload on `@causl/causl-wasm-ts/wasm` with the **real** Rust engine (not the TS-wrapper scaffold) at ≥Tier 2 (`wasmgc-classic` or better), proving bridge surface and perf are production-grade. | MECHANICAL (artefact) + DESIGN-DISCIPLINE (adopter attestation) | `wasm-pkg/nodejs/` artefact + loader hook in CI; the adopter attestation is recorded in the promotion amendment. |
| 5 | **TS-engine floor maintained.** The distribution's `backend: 'js'` floor stays supported and byte-identical to the wasm engine for all adopter code: no §12 row removed, no new **required** `createCausl()` option, no perf regression the TS engine could not also accept. **Sunset for `causl/causl-wasm-ts` by §18A.13 and finished at its 0.5.0**; `backend` and `engine` are removed there and there is no floor left to maintain, so the criterion is live only for `causl/causl-core-ts`. | MECHANICAL (regression) + DESIGN-DISCIPLINE (surface audit) | §12.2 quarterly audit (Commitment 2) + the §17.5 capability-cost band gate + the byte-identity gate (Criterion 1). |

### 18A.8 NO-GO and revert conditions

Promotion is blocked, and a promoted wasm-default is **reverted to TS-SSOT**, on any of:
- **Criterion-1 NO-GO:** byte-identity divergence at N≥10 trials on any extern (`commit_log`, adjacency, `engine_stats`, subscriber fire-order). As of the §18A.8 fail-safe-removal amendment (causl/causl-core-rs#169, 2026-06-21) the per-flush sticky-downgrade fail-safe is RETIRED (`rust-ssot` is unconditional) so the CI-blocking cross-backend determinism gate (§18A.1.1, 0-byte divergence) is the SOLE guarantee and the SOLE detector: a divergence at this gate is a HALT-before-merge condition (the gate stays CI-blocking), not a self-healing runtime downgrade. The revert path is now the governance revert of this amendment (re-pin the floor), not an in-process demotion.
- **Criterion-2 NO-GO:** any §12.2 row still routes through a closure-private struct instead of an extern (the thin layer is not feature-complete over FFI).
- **Criterion-3 NO-GO:** representative adopter workloads exceed the named ceiling (250 μs/commit p95 marshal overhead · 5 ms p95 per large batch/mutation): i.e. the 78× wire tax is not amortised below the bar the committed workloads require. (Promotion without this is a silent capability-cost regression: Theorem 3 atomicity still holds, but the wasm engine violates the TS engine's O(N) recompute property on graphs where the boundary cost outweighs the graph cost.)
- **Criterion-4 NO-GO:** no Node target, or no real-engine production adopter at ≥Tier 2: the bridge/perf are unproven at production scale.
- **Criterion-5 NO-GO:** any breaking change to `backend: 'js'`: a removed §12 row, a new required option, or a perf regression the TS engine could not accept. (Read with §18A.13: Criterion 5 is sunset for `causl/causl-wasm-ts`, where the option itself was removed at 0.5.0 as a named governance act rather than regressed into. This bullet governs `causl/causl-core-ts`.)

If fewer than all five are GO, the wasm engine **remains the opt-in alternative**: valuable for adopters with workload shapes where the WASM-side compute amortises the boundary tax (high-derivation-depth graphs, p99/p999-sensitive paths); but **not** the default. The TS engine is the floor in every NO-GO outcome. (A dated clause: all five went GO on 2026-06-21, and the §18A.8 amendment in the §19 trail then retired the runtime fail-safe, so today's revert path is the governance revert that row names; in `causl/causl-core-ts`, which still ships the floor, this clause reads as written.)

### 18A.9 What the team rejected and why

Promoting on merge (§13.8's rejection, restated for §18 readers): the costs are real and the §18A.7 gates are not met; promotion-by-stealth is exactly what §17.6 Commitment 14's "must amend explicitly" clause forbids. Hiding the 78× tax in an adoption guide rather than the SPEC; rejected: the arithmetic belongs in front of every wasm-vs-js decision (§17.6 G.1, §18A.7 Criterion 3). Making `read()` reference-identity a contract to avoid the migration bite; rejected: the §15.1/#1124 clarification already removed it; pretending otherwise would strand every `React.memo`-on-reference adopter at the swap. Mechanizing the *promotion* decision (auto-flip on green gates); rejected per §17.4: the conformance is mechanical, but flipping the value-of-record is a governance act that wants a named author and a dated amendment, not a CI toggle. Transliterating the TS engine into Rust; rejected: a from-scratch Rust engine proven byte-identical (§18A.4) is the only shape that earns "single-implementation correctness" rather than carrying two copies of the same substrate assumptions in two languages.

### 18A.10 Repository topology

The engine contract spans three first-party repositories under the `causl` org; each owns one artefact of the §18A surface, and no repo duplicates another's role. The split is recorded here so the FFI source-of-truth (§18A.3), the thin TS binding (§18A.4), and the TS value-of-record floor (§13.8, now `causl/causl-core-ts`'s alone) are never confused for one another:

| Repository | Role in the engine contract (and the cross-repo two-engine record) | Owns |
| --- | --- | --- |
| `causl/causl-core-rs` | The **wasm causl-core / engine** repo: the source of truth for the `causl-wasm` engine. The Rust engine (`tools/engine-rs-core`) compiled to WebAssembly via the bridge (`tools/engine-rs-bridge`). | The §18A.3 FFI surface (the externs `causl-wasm-ts` binds over), `engine-rs-core`, `engine-rs-bridge`, the byte-identity gate's Rust side (`bridge_byte_identity.rs`), and the **Python build &amp; packaging tooling** (`scripts/build_wasm.py` + `scripts/package_wasm.py`, §18A.11) that produces the Node-target artefact and places it at a consumer-stated destination. |
| `causl/causl-wasm-ts` | The **TypeScript API for `causl-wasm`**: the thin TS binding of §18A.4 (a binding over the `causl-wasm` FFI, **not** a copy of the TS engine) plus the loaders, setup, examples, and adoption docs that let TypeScript/Node.js web apps **integrate the placed** wasm engine. **One artefact, one engine.** As of §18A.13 (2026-06-19) this is the **wasm-only client**, and as of §18A.13.1's **withdrawal amendment (0.5.0)** that is once again true without qualification: `causl-wasm` is its sole engine, `createCausl()` returns it (sync, post-preload) or **throws**, and there is no degradation path off it. The pure-TS `causl-ts` floor is **not an engine here**; `createCauslTs` is absent from the package barrel and from every declared subpath, and the two options that used to select it (`engine: 'js-ssot'`, `backend: 'js'`) were removed at 0.5.0. The TS *closure* the wasm `Graph` used to be constructed inside is gone as well. **`del-final` LANDED** at `causl/causl-wasm-ts#279` slice S12 (EPIC #275): `createCauslTs`, `commitInternal`, the structural facade and the `injectedBackend` seam are out of the source tree, and `packages/core/wasm/index.ts` constructs the `Graph` on the authoritative engine directly. The §18A.3 lift had completed earlier and independently (`causl/causl-core-rs#170`: all four projection reads, `read_at`/`__snapshot`, `_export`, `subscribeCommits` and `explain`, resolve from Rust), so the lift never explained why the closure outlived it; slice S12 is what removed it. The §12 conformance reference is now the frozen golden-vector corpus plus the differential oracle in `causl/causl-core-ts`. The value-of-record floor lives **only** in `causl/causl-core-ts`, which is its sole owner in the org. The build &amp; packaging tooling is **not** here; it is the Python tooling in `causl/causl-core-rs` (§18A.11); this repo consumes the placed `.wasm`. | The §12 `Graph` contract as a thin binding over the `causl-wasm` FFI (§18A.4), the `--target nodejs` loader hook (§18A.2) that resolves the placed artefact via `node:fs`, the wasm-adoption guide, and the wasm-only distribution contract (`DISTRIBUTION.md`, §18A.13 + §18A.13.1). Named first-party integration consumers: `iasbuilt/xldatagrid` and `iasbuilt/webapp` (Node.js TS web apps). |
| `causl/causl-core-ts` | The **`causl-ts`** reference engine: the TypeScript value-of-record / §13.8 unconditional floor, and since `causl-wasm-ts` 0.5.0 the **only** repository in the org that ships it; plus the §18A.13 **differential oracle** (the dual-engine repo where the byte-identity gate still runs after `causl-wasm-ts` can no longer host it), the **benchmark** repo, the **loader-port source** for `causl-wasm-ts`'s wasm loader, and the conformance and historical R&D surface. **This repository stays dual-engine.** | The `causl-ts` engine (the floor; retained here unconditionally; the §18A.13 wasm-only strip is scoped to `causl-wasm-ts` only), the cross-backend benchmark/conformance harness (the byte-identity gate's JS side, `cross-backend-determinism.property.test.ts`), the frozen byte-identity golden-vector corpus that survives the strip (#39), the historical wasm-cutover R&D, and the floor half of the `causl/causl-wasm-ts#272` in-place-mutation divergence (`causl-core-ts#58`). |

The `causl-wasm` engine named throughout §18A is produced by `causl/causl-core-rs`; `causl-wasm-ts` ships no Rust source (§18A.4) precisely because the FFI source of truth lives in `causl/causl-core-rs`. This topology is descriptive: it records where the committed artefacts live; it does not alter the §18A.7 GO/NO-GO criteria or the §17.6 Commitment 15 surface. The `causl-wasm-ts` → wasm-only divergence recorded in the rows above (its sole engine is `causl-wasm`; the TS value-of-record floor lives only in `causl/causl-core-ts`) is the §18A.13 amendment: it changes the engine *roster* per repo, not the shared §12 contract or §3 semantics; which is why the divergence is carried in §18A.13 + `causl-wasm-ts`'s `DISTRIBUTION.md` rather than in a forked spec.

**`SPEC.md` is not byte-identical across the two repositories, and has not been for some time.** The *intent* that the shared contract stay one text is unchanged; the *fact* is not, and earlier drafts of this section, §18A.12 and §18A.13 asserted the fact. Measured 2026-08-02: `causl/causl-wasm-ts` `f4cfdb7` against `causl/causl-core-ts` `9c3d947`; that repository's copy carries **no** §5.1 Amendment 5, 6 or 7 and **no** §5.7. That structural statement is the criterion; a diff total is not, having moved four times in a fortnight as both copies took independent doc repairs. The divergence is recorded in `causl-wasm-ts`'s `DISTRIBUTION.md`. Reconciling it; porting the missing amendments and re-converging the two files; is `causl/causl-core-ts#58`'s work, and until it lands no document in this repository may assert the identity.

### 18A.11 Build &amp; packaging tooling (Python, in `causl/causl-core-rs`)

A webapp must be able to take the `--target nodejs` artefact of §18A.2 and place it into its own deployment **reproducibly**: build-once / place-where-told, not rebuild-per-app and not fetch-late. This subsection is the contract on what tooling the producer ships so that pipeline exists and is honest about its failure modes; it is a packaging contract, not a promotion criterion, and like §18A.10 it does **not** alter the §18A.7 GO/NO-GO criteria.

**The tooling is Python, and it lives in `causl/causl-core-rs`** (the engine repo, §18A.10), not as a JS / CI-workflow kit in `causl-wasm-ts`. The user's webapp CI/CD pipeline is Python, so the producer ships **two stdlib-only Python scripts** (no `pip install` step, no third-party deps) under `causl-core-rs/scripts/` that a Python pipeline drives directly:

| Script | Role |
| --- | --- |
| `causl-core-rs/scripts/build_wasm.py` | The **BUILD** half. Compiles the Rust `causl-wasm` core to a **size-optimised, node-target** `.wasm` (`wasm-pack --target nodejs` with the lean serde-free feature set, then external `wasm-opt -Oz` with the WasmGC/reference-types/bulk-memory feature flags; the Cargo release profile's `opt-level="z"` / `lto="fat"` / `codegen-units=1` / `strip` / `panic="abort"` knobs are verified, not edited), staging a clean build directory containing the optimised `.wasm` + the node glue + `.d.ts` typings + `package.json`, ready for the package step. This is the only place a **Rust toolchain** is needed: the producer's build script; and it is the producer-side concretion of §18A.2's Node-target requirement. |
| `causl-core-rs/scripts/package_wasm.py` | The **PACKAGE** half: **the script the Python webapp pipeline plugs in.** It takes a build directory and **places** the node-consumable artefact set (the `.wasm` + node glue + typings) at a **consumer-stated destination** (`--dest`, REQUIRED), then writes a deterministic **version + per-file-sha256 checksum manifest** (`causl-wasm.manifest.json`) the pipeline can verify against. No network. Idempotent. Fails **loudly** (non-zero exit + clear stderr) if the source is missing/incomplete, a file cannot be hashed, or the destination is unwritable. |

**How a Python webapp pipeline consumes it.** The pipeline states *where* the engine goes and calls the package script: it does not author a YAML kit or reason about bundler integration:
...
That single call builds the size-optimised node-target engine (`--build` shells out to `build_wasm.py`) and lands the artefact + version/checksum manifest at `WASM_TARGET_DIR`; the consumer never rebuilds the Rust→WASM engine in its own app logic: it states the destination and gets a pinned, verified placement. The contract the tooling MUST satisfy:

| Requirement | What it means |
| --- | --- |
| Versioned + checksum-pinned | `package_wasm.py` emits a manifest carrying the engine version (from the bridge crate's `Cargo.toml`) + per-file sha256 + byte counts; the placed artefact is identified by version and checksum, not by wall-clock or rebuild. |
| No deploy/runtime network | The build is producer-side; the package step only **places local files** at `--dest`. Nothing is fetched at deploy or runtime: the engine is vendored into the consumer's deploy artefact, offline-resolvable. |
| Deploy-shape coverage | The placed node-target artefact + manifest works across the common deploy shapes: container image, serverless bundle, and SSR Node server; without per-shape rework of the loader hook (§18A.2); the consumer points `--dest` at whatever directory its shape vendors from. |
| Fails loudly on mismatch | A missing/incomplete source, an unhashable file, an unwritable destination, or a version/checksum mismatch is a **hard, loud failure** (non-zero exit): never a silent fall-through to an unpinned, wrong, or partial engine. This is the §17.6 Commitment 14 "no adopter stranded" discipline applied to packaging: a host is not stranded on a corrupt, missing, or version-skewed `.wasm`. |
| No Rust toolchain for the consumer | Only `build_wasm.py` (producer-side) needs the Rust toolchain. The consuming Python pipeline only needs CPython stdlib to run `package_wasm.py`; this is the same discipline as §18A.4's "`causl-wasm-ts` ships no Rust source": a webapp adopter never needs a Rust toolchain, only a version and a checksum. |
| Stdlib-only + documented | Both scripts are CPython-stdlib-only (`subprocess`/`argparse`/`pathlib`/`hashlib`/`json`/`shutil`/`tomllib`) so they drop into any Python pipeline with no dependency install, and each carries a documented CLI surface so the pipeline is reproducible from the docstring, not reverse-engineered from the source. |

**The repo split this lives in (§18A.10), restated for the boundary.** The BUILD + PACKAGE scripts are in `causl/causl-core-rs`: the engine repo owns producing and placing its own artefact. `causl/causl-wasm-ts` is the **TypeScript API + TS-side integration** (§18A.4): it consumes the *placed* `.wasm` (the loader hook of §18A.2 resolves it via `node:fs` from wherever `--dest` put it) and exposes the §12 `Graph` surface over FFI. `causl-wasm-ts` ships no Rust source and no build tooling; the produce-and-place tooling is the engine repo's, the consume-and-bind surface is the client's. The artefact this tooling packages is the §18A.2 `--target nodejs` engine; the host-tier discipline it must not violate is §17.6 Commitment 14's; the packaging must not strand a host, the same way the host-tier matrix must not strand a browser host.

---

### 18A.12 Synchronous construction of the wasm engine (preload + sync-factory split): SPECIFIED 2026-06-19 (PLANNED; implementation tracked by epic causl/causl-wasm-ts#29)

This subsection refines §18A.6 (single-tick atomicity) and §18A.2 (Node-target loader) with a **synchronous-construction contract** for the wasm engine, so a consumer can build a wasm-backed `Graph` inside synchronous React render/hooks and synchronous SSR without an `await` at the call site. It is a pure additive capability: it does **not** change the default engine (§18A.7 is untouched), does **not** alter byte-identity (§18A.1.1), and does **not** promote the wasm engine.

**The one unavoidable async is COMPILE, not INSTANTIATE.** The wasm artefact is ~630 KB (`packages/core/wasm-pkg/gc-classic-bundler/causl_engine_bridge_bg.wasm`; the `gc-builtins-bundler/` tree this sentence used to name alongside it was deleted with the bridge, causl/causl-core-rs#355). A synchronous `new WebAssembly.Module(bytes)` on a >4 KB module on a browser main thread is spec-prohibited, so the COMPILE step (`await WebAssembly.compile(bytes)`) and the two dynamic `import()`s (the `_bg.js` sidecar and the `causl-compute-imports.js` snippet) MUST remain async. The INSTANTIATE step is a **synchronous primitive**: `new WebAssembly.Instance(module, imports)` performs the identical work as `await WebAssembly.instantiate(module, imports)` from an already-compiled `WebAssembly.Module`, at any size, with zero await. The bundler sidecar's `__wbg_set_wasm(exports)` (`causl_engine_bridge_bg.js`) is wasm-bindgen's `initSync` minus the compile; the synchronous re-point seam. The `--target nodejs` glue (§18A.2) is **already** synchronous end-to-end (`readFileSync` + `new WebAssembly.Module` + `new WebAssembly.Instance` at module-eval).

**The contract (additive to §18A.4's `@causl/causl-wasm-ts/wasm` surface):**

1. `preloadCauslWasm(opts?): Promise<CauslWasmModule>`: the ONE async seam, called once per process+bridge at app init. Pays `await WebAssembly.compile` + the sidecar/snippet dynamic imports, caches the compiled `WebAssembly.Module` (+ sidecar + compute-imports) keyed by bridge in a `compiledModuleByBridge` map distinct from the live-instance `modulePromiseByBridge`. Idempotent (concurrent calls share one compile; transient-failure drops the cache entry for retry). Companion `isCauslWasmPreloaded(bridge?)`/`getPreloadedCauslWasm(bridge?)` expose **resolved** state synchronously (set in the `.then`, not merely the in-flight promise).

2. `createCauslWasmSync(handle?, create?): Graph`: FULLY SYNCHRONOUS. `new WebAssembly.Instance(handle.module, imports)` → `sidecar.__wbg_set_wasm(instance.exports)` → `__enableAuthoritativeWasm` → `createCausl({ injectedBackend })`. Validates `handle.bridge === resolved bridge`. **MUST NOT silently await.** When nothing is preloaded for the bridge it throws `CauslWasmNotPreloadedError` naming `preloadCauslWasm()`; preserving the §18A loud-fail discipline, because a wasm-authoritative graph and a TS graph differ in `read()`-identity/commit-clock, so a silent swap is a glitch-freedom hazard. *(Amended by §18A.13.1's withdrawal, `causl/causl-wasm-ts` 0.5.0: this clause used to name a second option; `{ fallbackToTs: true }` returned `createCauslTs()`, justified by "safe while both engines ship because js-ssot is byte-identical per §18A.1.1". Both halves of that justification failed. `causl/causl-wasm-ts#272` established that the two engines are **not** byte-identical on in-place mutation of a committed value, and `causl/causl-wasm-ts` no longer ships both. The option is removed there rather than ignored; in `causl/causl-core-ts`, which does still ship both, the soft path is that repository's to keep or retire.)*

3. `createCauslWasm(opts?)` (the existing async factory) is **retained** and re-expressed as `const h = await preloadCauslWasm(opts); return createCauslWasmSync(h, opts)`: one instantiate codepath, provably `preload ∘ sync`, zero drift.

**Atomicity (cross-ref §18A.6 / §3 Theorem 2).** The compile `await` happens at app init, **outside any commit envelope**; `createCauslWasmSync` performs no await between the §5 Phase E and Phase G boundaries. Theorem 2's single-tick invariant is untouched: the construction await is hoisted to bootstrap, where async is already tolerated.

**Node vs browser (cross-ref §18A.2).** On Node/SSR the `--target nodejs` glue is synchronous at require-time, so `createCauslWasmSync` works **without** a prior `await`: server render stays synchronous. In the browser the bundler path requires `await preloadCauslWasm()` to complete **before first render** that constructs a wasm graph; this ordering is load-bearing for SSR↔CSR hydration parity and removes the #523 type-to-edit seed race (no engine is constructed mid-render in an unresolved-promise state). If an app skips the preload, `createCauslWasmSync` throws; it never silently blocks, and in `causl/causl-wasm-ts` as of 0.5.0 it never degrades either (§18A.13.1's withdrawal removed the `fallbackToTs` escape hatch there).

**Multi-instance isolation (cross-ref §18A.6): RESOLVED via `engine_id` multiplexing (epic `causl/causl-wasm-ts#42` multiplex track, shipped 2026-06-20).** Originally, multiple `new WebAssembly.Instance` of one compiled `WebAssembly.Module` each had independent linear memory but all funnelled through the **one** module-level `wasm` slot in the shared bundler sidecar (`__wbg_set_wasm`, last write wins) and the global compute/fire handlers; so ≥2 genuinely-independent authoritative (rust-ssot) graphs would alias one `ENGINE_STATE`/handler set; a probe proved silent multi-graph corruption, held at bay by an at-most-one **fail-closed** stopgap. The chosen fix is **`engine_id` multiplexing** (not the rejected per-instance cache-busted-sidecar path, which caps at the glue-pool depth and risks eval/CSP): `causl-wasm-ts` instantiates the bridge **once per process** (one shared `WebAssembly.Instance` + one sidecar) and routes every graph through it by a minted per-instance `engine_id`. The Rust `ENGINE_STATE` is a per-`engine_id` registry; the `DERIVED_FN_TABLE`, observers, firings, ObserverErrors and dynamic read-deps are all keyed by `engine_id`; the compute/fire handlers are a per-`engine_id` `Map`; `INTERN_TABLE`/`VALUE_POOL` stay shared (opaque module-global handles that never collide). N concurrent authoritative graphs are fully isolated; no value/firing/clock/stats cross-talk, verified at N=16 (the xldatagrid many-grid scenario); and the fail-closed at-most-one stopgap (`MultiInstanceUnsupportedError`) is **removed**.

**Build/packaging (cross-ref §18A.11).** Directive #1 needs no Rust regeneration: the nodejs glue is already sync and the bundler `_bg.js` already exports `__wbg_set_wasm`. The one additive packaging requirement is that `package_wasm.py` place the `causl-compute-imports.js` snippet next to the `.wasm`/`_bg.js` in the consumer's `wasm-pkg/<bridge>/` so the sync factory always has a real sidecar+snippet and the throw-stub path is dead. A build-parity CI assertion pins the two sync seams (`new WebAssembly.Instance` in the nodejs glue; exported `__wbg_set_wasm` in the bundler `_bg.js`) so a wasm-pack/wasm-bindgen upgrade that drops either fails loud.

**Status.** SHIPPED 2026-06-19 in `causl/causl-wasm-ts` and `causl/causl-core-ts` (the authoritative loader source). This subsection's text is intended to be the same in both repositories; it is not verified to be, and as of 2026-08-02 `SPEC.md` as a whole is **not** byte-identical across them: see §18A.10 for the measurement and `DISTRIBUTION.md` for the divergence record.

---

### 18A.13 causl-wasm-ts → wasm-only core (COMMITTED, gate-bypassed, executed wire-before-cut): dated amendment 2026-06-19

**Governance disposition (recorded, named, dated).** The team commits `causl/causl-wasm-ts` to ship the **wasm engine as the sole default engine** and to **remove the pure-TypeScript engine (`createCauslTs`) from `causl-wasm-ts`**. This is a *governance decision*, taken by the same authority that opened §13.8, recorded here as the dated follow-up amendment §13.8/§18A.7 anticipated. It **deliberately bypasses the §18A.7 GO/NO-GO promotion gate for `causl-wasm-ts`** and states the bypass plainly rather than pretending the criteria are met.

**Why the gate is bypassed (the honest rationale).** §18A.7's gate exists to protect a *perf-and-correctness floor* and to prevent *promotion-by-stealth*. This amendment is neither stealthy (it is named and dated) nor perf-motivated:
- **Driver = complexity-elimination, not performance.** The committed value is shedding the dual-engine maintenance burden: two conformant §12 implementations, the doubled conformance surface, the TS structural scaffolding; for a single Rust-authoritative engine. The §17.6 G.1 ~78× boundary tax and the 6.85×–28.84× per-commit median (§18A.7 Criterion-3 Resolution) are **explicitly accepted as UX-immaterial**: they stay within the §14 RAIL responsiveness budget (Criterion 3 is already GO on a "stays-responsive" basis, 2026-06-19). Perf is therefore **not** a gate or an argument anywhere in this amendment. `causl-wasm-ts` is an enterprise-only distribution where this trade is appropriate.
- **Scope = `causl-wasm-ts` only.** The fork `causl/causl-core-ts` **keeps** the dual-engine TS floor: it remains the differential-test oracle (§18A.1.1 JS side), the benchmark repo, and the source of the authoritative Rust→wasm loader being ported into `causl-wasm-ts`. "Wasm-only" is scoped to the shipped `causl-wasm-ts`; it does **not** remove `causl-ts` from the org or from CI.

**Executed in WIRE-BEFORE-CUT order (an ordering constraint, NOT a gate).** The repo must never be left with zero working engine; the strip is therefore the *last* step, after a working wasm engine is wired and made default:
1. **WIRE: port the authoritative loader.** Port the fork's real Rust→wasm loader + wasm-bindgen sidecar wiring (`tryInstantiateConsolidatedBridge`, `buildBridgeImportsFromSidecar`, `__enableAuthoritativeWasm`/`getAuthoritativeEngine`, the `__wbg_set_wasm` sidecar re-point) into `causl-wasm-ts`. The `.wasm`/`_bg.js` artefacts are **already vendored** in `causl-wasm-ts` (`packages/core/wasm-pkg/{gc-classic,gc-builtins}-bundler/`); the `causl-compute-imports.js` snippet is added via `package_wasm.py` (§18A.11). At this step `causl-wasm-ts`'s `instantiateBackend` stops throwing "backend artifact … not yet built" and stops wrapping `createCausl()` (the current TS-backed shim, `WasmBackend.#graph = createCausl()`).
2. **FLIP + SYNC: make `createCausl` the wasm engine, synchronously.** `createCausl()` in `causl-wasm-ts` **runs the wasm engine** (sync, post-preload); it is **not** a wrapper to `createCauslTs`. The §18A.12 `preloadCauslWasm()` + `createCauslWasmSync()` split is the mechanism that lets `createCausl` stay synchronous (one `await preloadCauslWasm()` at app init; sync `createCausl()` at every render/hook site thereafter). Consumers (e.g. `iasbuilt/xldatagrid`'s 7 sync `createCausl()` sites) keep calling `createCausl()` **unchanged** and transparently get wasm; the earlier "switch xldatagrid to `createCauslWasm`" / ~62-file async refactor is **dropped**.
3. **CUT: delete the TS engine.** Remove the pure-TS engine source and `createCauslTs` from `causl-wasm-ts`. `createCauslTs` becomes either a thin alias that throws with migration guidance or is removed in a major version. **Status (0.5.0):** the *surface* half completed; `createCauslTs` is absent from the barrel and from every declared subpath, and the last two adopter-reachable routes to it (`engine: 'js-ssot'`, `backend: 'js'`) were removed with the §18A.13.1 fallback. **Status (dated rider, `causl/causl-wasm-ts#279` slice S12):** the *source* half; `del-final`, deleting the closure itself, tracked by EPIC #275; has **LANDED**. `createCauslTs`, `commitInternal`, the structural facade and the `injectedBackend` seam are out of the tree. The surface/source distinction was load-bearing for as long as the two halves disagreed, and it names no gap now: no adopter can reach the TS engine, and there is no TS engine left in the tree to reach.

**Spec-stance reversal (stated, not hidden).** For `causl-wasm-ts`, the §13.8/§18A.1/§17.6-Commitment-15 framing "`causl-ts` is the unconditional floor" is **reversed**: in `causl-wasm-ts` the wasm engine is the sole engine; there is no TS floor in this distribution. §18A.7 Criterion 5 ("TS-engine floor maintained, `backend:'js'` stays supported") is **sunset for `causl-wasm-ts`** and **retained for `causl-core-ts`** (which keeps both engines). §17.6 Commitment 14's "fall-through to the TS engine" host-fallback is **retired for `causl-wasm-ts`**; a host where wasm cannot instantiate fails loud (§18A.11 no-strand discipline) rather than degrading to TS; this is the accepted cost of the enterprise-only, complexity-driven distribution. *(§18A.13.1 partially reversed this sentence for the implicit `createCausl()` path between 2026-06-23 and `causl/causl-wasm-ts` 0.5.0. That reversal was **withdrawn**; the sentence above is again the whole of the stance, and §18A.13.1 is the dated record of the round trip.)*

**Conformance oracle survives the strip, and this is what it is now.** The §18A.1.1 gate is differential and needs a second implementation; a wasm-only `causl-wasm-ts` cannot host the second engine as its own product, so the oracle re-homed rather than died. Four arms carry it: (1) the LIVE foreign-floor leg, where the differential (`cross-backend-real-rust.property.test.ts`) imports the pinned `causl/causl-core-ts` checkout (`9c3d947`, resolved from `CAUSL_CORE_TS_CHECKOUT`) and runs it in-process against the real Rust engine on run-time-chosen workloads (#279 S8); (2) the CAPTURED reference traces of that same foreign floor (`packages/core/test/fixtures/reference-traces/`, #277), guarded by a mechanical distinct-engines refusal so a wasm run can never wear the oracle's name, with `packages/core/test/fixtures/cross-engine/` as the per-surface disposition ledger; (3) the frozen golden-vector corpus captured before the cut (#39); (4) `causl-core-ts` itself, which retains `causl-ts` plus the in-process byte-identity harness. Removing `causl-ts` from `causl-wasm-ts` does **not** remove the proof of correctness from the org.

**SHIPPED-vs-IN-PROGRESS (honest, dated).**
- **SHIPPED 2026-06-19:** the §18A.12 sync split; the vendored bundler artefacts in `causl-wasm-ts`; the authoritative loader in the fork (the port SOURCE).
- **SHIPPED 2026-06-20 (the wire-before-cut public strip, executed in order: not gated on perf):** (a) the loader port into `causl-wasm-ts`; step 1 WIRE (#32): `instantiateBackend` no longer throws "not yet built" and `createCauslWasmSync` instantiates the real vendored `_bg.wasm`; (b) the `createCausl`→wasm flip + sync preload/factory; step 2 FLIP (#33): a bare `createCausl()` routes through the registered wasm sync constructor once `preloadCauslWasm()` has run for the default bridge, and falls back to the internal TS floor otherwise (kept non-breaking for sync callers that never preload); (c) the **public** TS-engine strip; step 3 CUT (#34): `createCauslTs` is removed from the public `@causl/causl-wasm-ts` barrel, so an adopter can no longer pick the TS engine directly. The frozen golden-vector corpus (#39) was captured before the cut.
- **SHIPPED 2026-06-20 (multi-instance isolation: epic `causl/causl-wasm-ts#42` multiplex track, superseding the old `causl/causl-core-rs#143`):** true per-instance isolation for N concurrent authoritative graphs via **`engine_id` multiplexing**; one shared `WebAssembly.Instance` routed by a per-graph minted `engine_id`, with the Rust `ENGINE_STATE`/`DERIVED_FN_TABLE`/observers/firings/ObserverError/read-deps all `engine_id`-keyed and per-`engine_id` compute/fire handlers. Verified isolated at N=16 (the xldatagrid many-grid scenario); the fail-closed at-most-one stopgap is removed. This unblocks `causl-wasm-ts`'s many-graph consumers (xldatagrid).
- **SHIPPED at `causl/causl-wasm-ts` 0.5.0 (the roster collapses to one: `causl/causl-wasm-ts#280`):** the §18A.13.1 capability fallback is **withdrawn**, and with it the two options that reached the TypeScript floor from a public factory (`CreateCauslOptions.engine`, `CreateCauslOptions.backend`), the `WasmEngineMode` / `DEFAULT_WASM_ENGINE_MODE` / `resolveWasmEngineMode` surface on the `/wasm` subpath, `WasmBackendOptions.engine`, and the `AuthoritativeWasmOptions.fallbackToTs` / `.fallbackToJs` soft path. `createCausl()` is **construct-or-throw**: it returns the wasm engine or it fails during construction, with no partially-built graph and no async window. Step 2 FLIP's "falls back to the internal TS floor otherwise" clause above is therefore **retired**; it is the dated record of 2026-06-20, not the live contract. The accepted cost is a dropped host tier (hosts below the declared engine floor, stated once on `WASM_HOST_FLOOR` in `packages/core/src/wasm-registry.ts` and corrected against the artefact's measured requirements by causl/causl-wasm-ts#426), recorded in §18A.13.1. The deprecation discipline of §18A.13's own two-release cycle was followed rather than exempted: 0.4.0 warned, 0.5.0 flipped, 0.6.0 deletes the inert type members held back so an adopter's exhaustive `switch` keeps compiling.
- **DEFERRED: re-scoped per a grounded FFI-marshaling forecast (2026-06-20):** the **fully-authoritative Rust *structural* surface**; dependencies/dependents (+transitive), commit-log, commit-metadata, handle/disposal/StaleTx validation, stats, explain, export/snapshot, readAt/snapshotAt; over the §18A.3 FFI surface (the *internal* TS-free milestone; originally `causl/causl-core-rs#142`). Today `causl-wasm-ts`'s wasm path is still TS-structural-backed (the §18A.4 hybrid; read/subscribe/commit-clock/derived are wasm-authoritative, the structural projections answer from the **per-instance** TS `Graph` facade; which §18A.6 establishes is built *on top of* the backend and is itself per-instance-isolated). **[Superseded by Epic #170 / §18A.13.1 (2026-06-23); flagged by the review amendment 2026-07-06, M3.]** This "structural projections answer from the per-instance TS `Graph` facade" description is **no longer current**: Epic #170 lifted the four projection reads to Rust, so under `rust-ssot` no adopter read consults the TS `entries` map, and the retained `createCauslTs` is a clean standalone capability-fallback (§18A.13.1), not wasm-path structural scaffolding. The paragraph is preserved as the dated record of the pre-#170 hybrid. Even after the CUT (#34) removed `createCauslTs` from the *public* surface it survived INTERNALLY, and once `causl/causl-wasm-ts` 0.5.0 withdrew the §18A.13.1 fallback it survived as **nothing an adopter could reach at all**: the structural closure the wasm `Graph` was constructed inside and the suite's §12 conformance reference, neither an engine choice nor a fallback. `causl/causl-wasm-ts#279` slice S12 (`del-final`) deleted that closure, and the conformance reference moved to the frozen golden-vector corpus plus the `causl/causl-core-ts` differential oracle. **The forecast showed lifting the structural surface as a STANDALONE step is the wrong ordering:** the structural ops are *cold* (zero render-path frequency; ~0 ms/frame at scale) so there is no perf prize, and lifting them in isolation *adds* per-op extern + serde + byte-identity-gate machinery **without** retiring the TS commit bookkeeping (`entries`/`dependents` maps) those methods read from; net complexity rises. The complexity win materialises **only** when the commit/write SSOT itself moves to Rust (the **rust-ssot cutover**, a separate forthcoming epic): then the TS bookkeeping is deleted wholesale and the structural ops + the final `createCauslTs` deletion come along **for free** as Rust reads. The §18A.3 FFI-surface-completeness fixture (`ffi_surface_complete.rs`, landed) is preserved as the gate that cutover will satisfy. This amendment is honest that "truly TS-free" is sequenced **with** the SSOT cutover, not before it.

**What this amendment does NOT claim.** It does not claim the §18A.7 criteria are met (they are bypassed by governance for `causl-wasm-ts`). It does not retire `causl-ts` from the org (the fork keeps it as oracle/benchmark/floor). It does not use perf as a justification (perf is accepted as immaterial within §14 RAIL). *(When written, it also disclaimed full structural authority on the ground that the §18A.3 lift had not yet finished. **That disclaimer has expired.** Epic `causl/causl-core-rs#170` closed with all four projection reads: `read_at`/`__snapshot`, `_export`, `subscribeCommits`, `explain`; resolving from Rust, so the Rust engine **is** fully structural-authoritative under `rust-ssot`. What remained was `del-final`, the deletion of the retained TS engine itself, tracked by EPIC #275, and `causl/causl-wasm-ts#279` slice S12 landed it.)*

**SPEC.md byte-identity: the standing intent, and the fact it is not.** When this amendment was written, `SPEC.md` was byte-identical across `causl-wasm-ts` and the TS-floor repository, and the recommendation below was framed as preserving that. **It is no longer a fact.** Measured 2026-08-02 (`causl/causl-wasm-ts` `f4cfdb7` against `causl/causl-core-ts` `9c3d947`), the two copies differ; the TS-floor repo's copy carries **no** §5.1 Amendment 5, 6 or 7 and **no** §5.7. The divergence is recorded in `DISTRIBUTION.md` and its reconciliation is `causl/causl-core-ts#58`'s. **The recommendation itself stands unchanged:** do **not** fork `SPEC.md`; the §12 public contract and §3 semantics are identical across both; only the *engine roster* differs, and §18A.13 + §18A.10 (repo topology) are where that is recorded, so the shared spec is carried as one text ("for `causl-wasm-ts`, X; for the TS-floor repo, Y") plus a `causl-wasm-ts`-specific `DISTRIBUTION.md`. What changed is only that the invariant is now an **objective to restore** rather than an invariant to preserve, and no section may state it as current until it holds again. **The 0.5.0 roster edit was taken in exactly that direction** (`causl/causl-wasm-ts#281`): the single-engine roster is written into §18A.10, §18A.13 and §18A.13.1 in the shared "for `causl-wasm-ts`, X; for the TS-floor repo, Y" form, no section was forked or given a repo-specific variant, and the diverging-section list in `DISTRIBUTION.md` is unchanged by it; so the roster rewrite adds **no** new divergence. Closing the residual four-section gap is still `causl/causl-core-ts#58`'s. **The 2026-08-07 single-engine spec rewrite (this change) moves further in the same direction under the same rule.** I re-aimed the two-engine sections of this copy at the one engine this package ships, and I moved every statement that binds the sibling repository (the §5.1 Amendment 7 floor non-conformance finding above all) into explicitly cross-repo paragraphs that point at `causl/causl-core-ts` and the issue that owns each item. No section was forked or renamed and the diverging-section table gains no row, but the shared-one-text objective now runs through `causl/causl-core-ts#58` porting the missing amendments *and* deciding how its copy carries the roster sections; until both copies re-converge, neither may state the identity as current.

---

## 19. Amendment trail (perf workstream: May 2026)

The May 2026 perf workstream produced five SPEC amendments (the original four plus the post-wave §17 commitment-13 amendment landed by #1005); the parallel WASM-backend EPIC #680 produced one more (§17.6's commitment 14, landed by #690); the 0.9.0-readiness sweep adds a §15.1 contract-clarification amendment (#1124) for the Markbåge/Miller H1 risk surfaced by the WASM adopter audit, plus the §17.6 current-state callout (#1150) recording the 213 KB serde-bridge raw-byte divergence; the post-STOP-VERDICT documentation pass on epic #1133 adds two FFI-boundary amendments (#1333): §3 Theorem 2 uninterruptibility and §5.1 Phase G subscriber container pin; capturing invariants any future native backend (Rust, C++, FFI-to-anything) must honor, landed standalone-value after the A.1 perf-floor probe fired the kill gate by 35× and the engine port was frozen. Each is recorded inline at its host section (§3, §5.1, §9.1, §11.1, §15.1, §16.6, §17.5, §17.6); this trail is the one-page reference. Amendments are independent but compose: §5.1 Amendment 1 plus §16.6 Amendment 2 together make `cap=0` plus precondition-gating the new fast path for adopters that do not consume commit history; §17.5 frames the residual the wave program could honestly defend after that fast path landed; §17.6 names the architectural escape valve §17.5's lower-bound clause flagged; promotion of the opt-in `@causl/causl-wasm-ts/wasm` substrate (Rust-backed per the scaffolded `tools/engine-rs-core/` crate) to the default engine is the named candidate, the full Rust engine port that promotion implies lives on the shelf as post-0.9.0 epic #1133, and §17.6's host-tier contract is the bar that promotion would have to clear. The engine-bet governance amendment (2026-06-19) reopens §13.8, promotes epic #1133 from NOT-PLANNED to PLANNED, adds §17 commitment 15 and the §18A two-engine contract, and is the first §19 row that changes the *commitment surface* rather than recording a no-amendment engineering decision. One reading rule for the whole trail: rows are dated records, and several describe the engine roster as it stood on their date, including the TypeScript engine this package stopped shipping at 0.5.0 (§18A.13.1); I keep them verbatim as history, and the live roster statement is §18A.13.
...
### §15.1 Amendment: the value domain a node may hold is not closed at the type level (#270)

**Contract sentence:** added inline at §15.1. The adopter-facing value domain stays open: `Graph.input`, `Graph.derived` and `Compute<T>` carry no type-parameter bound, in 1.0 or after. The four non-JSON scalar classes (`undefined`, `bigint`, `symbol`, `function`) are admitted: retained host-side and returned by reference from `read` on both engines, with the NULL-record collapse in `packages/core/src/value-domain.ts` governing the cutoff/fire decision only. The narrower domain that genuinely exists; what survives an FFI marshal, and what §5.1 Amendment 7's cutoff relation ranges over; is named `WireValue` (`packages/core/src/types.ts`, on the package barrel): JSON extended by the `Set` / `Map` / `Date` / `Temporal` tagged containers. It is a description of the wire, not a bound.

**Evidence:** three artefacts disagreed and one of them described running code. `packages/core/src/value-domain.ts` states that "an input may legitimately hold a callback/bigint" and that `read` returns the real reference on both engines; `packages/core/test/value-domain-open-270.test.ts` executes that claim rather than quoting it, on `rust-ssot` and on the pure-TS floor: `Object.is(read(node), fn)` holds for a closure-valued input, a closure-valued derived returns a live callable over the *recomputed* dependency value, and `Set` / `Map` / `Date` / `Temporal` deriveds read back as real instances through `packages/core/wasm/tagged-types.ts`. Those are the values a `JsonRoundtrippable` bound would have refused, so the bound was never a one-line change: it was a first-day break of working adopter code, for a defect (`undefined → null` cutoff divergence, #151) already fixed in the cutoff. The companion source gate `packages/core/test/value-domain-bound-270.test.ts` holds the decision in place; it fails if a bound appears on the adopter-facing type parameters, if the §15.1 sentence stops being the single statement of the decision, or if `docs/epic-1133/PLAN.md` / `docs/epic-1483/CONSTRAINTS.md` regrow the narrowing.

**Migration:** none: the decision preserves the shipped behaviour and removes a planned break. `docs/epic-1133/PLAN.md`'s Phase F row is unblocked and `docs/epic-1483/CONSTRAINTS.md`'s value-type row now reads "none required" instead of scheduling a deprecation. One thing is recorded rather than dropped: `CONSTRAINTS.md` had required the `JsonRoundtrippable` deprecation lint to land *before* any FFI-crossing read path, and the §18A.3 structural lift plus the `rust-ssot` default crossed that boundary while no such lint existed. The precedence was missed. It costs adopters nothing here, because the answer turned out to be admit; but the rule fired unobserved, and the §15.1 amendment says plainly that a precondition written only as prose is not a gate.
...
> **Context clarification (2026-06-19).** The 78× boundary-tax measurement this row inherits its post-STOP-VERDICT framing from (epic #1133, 156.4 ms / 10k commits, A.1 perf-floor probe) **predated the §13.8 governance reopening** that committed the wasm engine as a PLANNED conformant alternative with an explicit §18A.7 criterion-3 ceiling gate (250 μs single / 5 ms batch p95). The criterion-3 ceiling is framed as a **"stays responsive" gate within the §14 RAIL synchronous budget, not a "beats TS" comparison** as the prior falsification framing implied. As of 2026-06-19 **Criterion 3 is GO under the stated ceiling** (see the §18A.7 Criterion-3 Resolution row below and the inline §18A.7 Resolution): the confirming benchmark passes 6/6 against the §14.2 RAIL budget and the per-flush marshal cost amortises 1/N below the 250 μs ceiling. The prior STOP-VERDICT (156.4 ms / 10k) **informs the ceiling magnitude** but is **superseded** by the named criterion-3 gate for the purpose of the alternative→default promotion decision; it still stands as the documented standing cost (the 78× tax is named, not hidden), and the serde-bridge raw-byte re-tightening this row tracks remains deferred to the post-STOP path (DROP / PIVOT / DEFER) independent of the criterion-3 disposition.

**Contract sentence:** the §17.6 commitment-14 row's 200 KB raw / 80 KB Brotli ceiling for the Tier 3 (`wasm-pkg/serde/`) artefact is **not** relaxed. The current-state callout immediately under the table records: the v0.9.0 artefact ships at **213 KB raw / 66 KB Brotli** (Brotli 14 KB under the canonical 80 KB cap; raw 13 KB over the canonical 200 KB cap); the root `package.json` size-limit cell `@causl/causl-wasm-ts wasm bridge; serde-json (raw)` is set to **230 KB** (ratcheted by PR #1112) so the bundle-size CI gate absorbs the measurement without false-firing on the documented gap; the Brotli post-build ceiling for the serde bridge is documented at **70 KB** in `packages/core/wasm-pkg/README.md` (enforced by `pnpm wasm:build`, not by a size-limit cell; the `//size-limit-wasm` comment in root `package.json` names that boundary). The divergence is recorded per Option C of #1150 (document the gap rather than silently exceed the SPEC).
...
### §13.8 / §17.6 / §18A Amendment: wasm engine core committed as conformant alternative + planned replacement (epic #1133 reopened, 2026-06-19)

**Status:** SPEC text edited inline at §13.8 (NOT-PLANNED → COMMITTED-ALTERNATIVE-with-promotion-path; epic #1133 promoted to PLANNED), §17.1 (commitment count 14 → 15; Commitment 15 row added), §17.6 ("acceleration-not-substitution" line retired; §17.6.1 Commitment 15 paragraph added), and new section §18A (the two-engine contract). This is the governance-decision successor to the #1493 C.7 and #1515 V2-final "NO amendment" rows: those rows recorded that the *engineering* (batched boundary, Rust-SSOT mechanics) required no contract change; **this** row records the *governance commitment* that the wasm engine is now a committed conformant alternative and a planned replacement; which **does** change the commitment surface (§13.8 status, §17 count, §18A contract).

**Contract sentence:** two conformant engines: `causl-ts` (the floor) and `causl-wasm` (Rust→WebAssembly via the `causl-wasm-ts` thin TS API); byte-identical at the §12 public boundary by the cross-backend determinism gate (§18A.1.1), with the wasm core required to expose the full §12.1+§12.2 surface over FFI (§18A.3) and the TS engine the unconditional floor until all §18A.7 GO/NO-GO criteria are met and a dated follow-up amendment records the promotion. The 78× marshaling tax (§17.6 G.1), the `read()`-reference-identity break (§15.1 #1124 / §18A.5), and the FFI single-tick atomicity invariant (§3 Theorem 2 / #1333 / §18A.6) are documented as the standing costs of the commitment, not hidden.

**Evidence:** the engine-bet decision (`.causl-bench-research/interop/DECISION-causl-wasm-engine-bet.md`, 2026-06-15) and the scaffolding that made the shape non-speculative: EPIC #680 Phase-1 bridges, epic #1493 option-c batched-boundary infrastructure (#1513), the #1071/#1146 cross-backend byte-identity gate, the §3/§5.1 #1333 FFI-boundary invariants, and the §15.1 #1124 read-identity clarification. The promotion gate (§18A.7) inherits the V2.4 #1535 LOAD-BEARING GO/NO-GO discipline (1000 trials × 0 byte divergence) and the V2.5 #1544 sticky-downgrade fail-safe (§18A.8).

**Migration:** none for adopters who never import `@causl/causl-wasm-ts/wasm`: the TS engine stays the default and the floor; this amendment commits the **alternative**, not the **default**. Adopters who opt into the wasm path gain the two-engine semantics and the documented marshaling cost, and must read §18A.5 before upgrading to a real-Rust build. Promotion to wasm-as-default requires all §18A.7 criteria met and a dated follow-up amendment; explicitly **out of scope** of this amendment, consistent with how #1124 / #1333 / #1493-C.7 / #1515-V2-final were sequenced (document the commitment now; amend at the decision that actually flips the value-of-record).

### §18A.7 Criterion 3: perf-floor ceiling validation: GO / RESOLVED (epic #1515 v2.x real-Rust-SSOT, criterion-3 gate, 2026-06-19)

**Status:** SPEC text edited inline at §18A.7: Criterion 3's Gate/measurement cell now carries a dated **Resolution (2026-06-19): GO / RESOLVED** subsection with the measured numbers. This row records the resolution on the §19 one-page reference. It does **not** promote the wasm engine: Criterion 3 is one of five load-bearing criteria (§18A.7), and **all five must be GO** before a dated promotion amendment flips the value-of-record. This row moves Criterion 3 from "not-yet-confirmed" to GO under the stated ceiling; promotion stays gated on Criteria 1/2/4/5.

**Contract sentence:** Criterion 3 (the §18A.7 perf-floor: `single commit ≤ 250 μs p95 marshal overhead`, `batch / large mutation ≤ 5 ms p95 total`) is **GO** on its stated "stays responsive within the §14 RAIL synchronous budget" basis; it gates responsiveness, **not** "beats TS." The prior 2026-05-13 #1133 STOP-VERDICT (15.64 μs/op marshal cost ⇒ 156.4 ms / 10k commits ⇒ 78× the entire 2.017 ms TS workload) **predated** the §13.8 reopening that reframed the wasm-engine status from NOT-PLANNED to PLANNED-with-gates and predated the §18A.7 named-criterion framing; the criterion-3 ceiling was set **post-reopening** as a responsiveness gate, not a comparison gate. Measured against the ceiling this session: the per-flush marshal overhead isolated by `op-rust-batch-boundary-{10,100,312,1000}` (1564/156/50.1/15.6 ns/op, amortising exactly 1/N) is well under the 250 μs single-commit ceiling, and the §14.2 end-to-end RAIL budget; the responsiveness window the ceiling exists to protect, passes 6/6.

**Evidence:** confirming benchmark re-run this session: `pnpm -F @causl/causl-wasm-ts build` then `pnpm exec tsx src/cli/run-bench.ts --suite taco-mutation --tier smoke --output /tmp/perf-resolve --json` (fresh process per cell, threads=1, `engine` measurement = full commit→recompute cascade, host Apple M5 / Node v26): **6/6 cells PASS the §14.2 RAIL responsiveness budget, 0 abs-breach, 0 regression-fail.** `causl-wasm` single-input-commit p50 0.92 ms / p95 2.5 ms (50 ms single budget); batch input-commit (200-wide, fixture cap) p50 3.2 ms / p95 5.5 ms (200 ms p95 batch budget); TACO 1000-cell-segment p50 6.8 ms / p95 15.9 ms (200 ms p95 batch budget). These end-to-end engine-exec figures (commit + derive cascade, NOT isolated marshal) sit inside the RAIL window; the isolated per-flush marshal cost (the quantity the 250 μs ceiling actually bounds) is the `op-rust-batch-boundary` 1/N-amortising tax above. (Numbers re-confirmed on a quiet host post-workflow; single-commit p95 2.5 ms reproduces exactly; an earlier same-session run under concurrent build load inflated batch p95 to ~70 ms, still within the 200 ms RAIL budget, so the GO verdict is load-independent.) The wasm/ts ratio (6.85×–28.84× p50 on this smoke run) is consistent with the standing 78×/~85× boundary-tax falsification; the real-Rust-SSOT engine (epic #1515, V2.4 gate validation) delivers its value on GC-pressure / large-tree axes (the V2.3 #1533 re-measurement: ~16.6× p99.9 flattening) orthogonal to the median per-commit cost the #1133 STOP-VERDICT falsified, not on parity. Cross-backend byte-identity (§18A.1.1) passes 1000 trials × 0-byte divergence (Criterion 1); the FFI-surface-completeness fixture (§18A.3) passes CI (Criterion 2's MECHANICAL anchor).

**Migration:** none: documentation + criterion-status amendment only; no engine source change, no adopter-visible behaviour change. The TS engine remains the unconditional floor and the default. Criterion 3 GO does not promote the wasm engine; promotion requires **all** §18A.7 criteria GO and a separate dated amendment that flips the value-of-record (the same sequencing discipline #1124 / #1333 / #1493-C.7 / #1515-V2-final used). Re-run for confirmation: `cd causl-core-ts && pnpm -F @causl/causl-wasm-ts build && (cd packages/benchmarks && pnpm exec tsx src/cli/run-bench.ts --suite taco-mutation --tier smoke --output /tmp/perf-resolve --json)`; the per-flush marshal-cost axis is re-confirmed by `op-rust-batch-boundary-{10,100,312,1000}` under the F-marshal.6 bidirectional-envelope methodology.

### §18A.7 / §18A.9 / §17.6 Commitment 15 Amendment: all five GO/NO-GO criteria met as of 2026-06-21: wasm engine PROMOTED to the DEFAULT for the WASM path (the dated promotion decision §18A.7 reserved); promote-default (causl/causl-core-rs#169)

**§18A.7.amendment: the named, dated promotion decision.** This is the dated follow-up amendment §13.8 / §18A.7 / §17.6 Commitment 15 reserved: the one that records **"all five §18A.7 criteria met as of 2026-06-21"** rather than re-litigating them. It is taken by the same governance authority that opened §13.8, is named and dated (§18A.9's anti-promotion-by-stealth discipline), and is **not** a CI auto-flip (§17.4 / §18A.9: flipping the value-of-record is a governance act with a named author and a dated amendment, never a green-gate toggle). Effective this amendment, `DEFAULT_WASM_ENGINE_MODE` is `'rust-ssot'`; `createCausl()` / `createCauslWasm()` route commit through the V2.4 compare-and-PROMOTE engine **by default** for the WASM path. This is the **default** promotion (route through WASM first, TS oracle in lockstep), distinct from §18A.13's `causl-wasm-ts`-only **wasm-only** strip and from the §13.8 **replacement** (TS demoted from value-of-record), which stays gated on the §18A.3 structural-FFI lift.

**Status:** SPEC text: this dated §18A.7 amendment row added to the §19 trail; §18A.7's preamble "becomes the default … **if and only if all** of the criteria below are met … recorded in a dated follow-up SPEC amendment" is now SATISFIED by this row. The mechanical flip is `packages/core/wasm/index.ts` `DEFAULT_WASM_ENGINE_MODE: 'js-ssot' → 'rust-ssot'`; the V2.1 #1522 default-OFF acceptance property is RE-PINNED (not broken) to assert the new `'rust-ssot'` default plus that the V2.5 (K=1) sticky-downgrade fail-safe stays ARMED, and the full `@causl/causl-wasm-ts` suite (1006 tests) is GREEN at the new default with the cross-backend / golden byte-identity gates at 0-divergence.

**Contract sentence:** all five §18A.7 GO/NO-GO criteria are GO as of 2026-06-21, so the wasm engine is promoted from committed **alternative** to the **default** for the WASM path; the TS engine stays available as the explicit `engine: 'js-ssot'` floor opt and as the value-of-record in `causl-core-ts` (§18A.7 Criterion 5 retained): **for `causl/causl-wasm-ts` the floor-opt half of that sentence is superseded by §18A.13.1's withdrawal at 0.5.0, which removed the `engine` option; the value-of-record half is unaffected, and `causl-core-ts` is now its only owner**; and the V2.5 K=1 sticky-downgrade fail-safe (§18A.8 Criterion-1 NO-GO / revert condition) stays ARMED so a default graph that ever diverges demotes to `js-ssot` on the FIRST divergence; a default graph is never worse than the floor it replaces.

**The five criteria: all GO as of 2026-06-21 (the audit this amendment records):**

| # | Criterion | Disposition | Evidence cited |
| --- | --- | --- | --- |
| 1 | **Correctness: cross-backend byte-identity** | **GO** | The gate-canon validation: the frozen golden-vector corpus (§18A.10 / #39), the `bridge_byte_identity.rs` B.8 Rust side, and the 1004-test `@causl/causl-wasm-ts` suite at **0-byte divergence**. The §18A.1.1 per-flush cross-backend determinism property (`cross-backend-determinism.property.test.ts`) passes 1000 trials × 0-byte divergence at N=1/10/100/312, including the 🚦 V2.4 PROMOTED GO/NO-GO cell (Rust post-state promoted canonical, `divergedFlushCount === 0`). Re-pinned at the new default: the full suite is GREEN with `'rust-ssot'` as the engine-omitted default; every wasm test now exercises the promote path and finds 0 per-operation divergence vs the TS oracle. |
| 2 | **Completeness: full FFI surface** | **GO** | ffi-6 emptied the `TODO_SET`: the §18A.3 FFI-surface-completeness fixture (`ffi_surface_complete.rs` + the thin-layer adapter test) is GREEN; every §12.1 canonical-seven + §12.2 second-tier-fifteen row is reachable over FFI with no JS-engine fallback row, and CI fails if any §12.2 row routes through a closure-private struct instead of an extern. |
| 3 | **Performance floor: acceptable marshaling tax** | **GO / RESOLVED (2026-06-19), NOT a gate that blocks this flip** | Resolved in the §18A.7-Criterion-3 amendment (2026-06-19): the perf floor is satisfied on its stated "stays responsive within the §14 RAIL synchronous budget" basis (6/6 RAIL cells PASS; per-flush marshal overhead 1564/156/50.1/15.6 ns/op amortising 1/N, well under the 250 μs single-commit ceiling). Perf is **immaterial** to this promotion; it gates responsiveness, not "beats TS"; the standing 78×/~85× #1133 boundary-tax falsification is NOT refuted and is NOT an argument here (consistent with §18A.13's "perf is not a gate"). |
| 4 | **Node target + real-world adoption** | **GO** | The `--target nodejs` artefact + ESM loader ship (§18A.2); the real Rust engine (not the TS-wrapper scaffold) at ≥Tier 2 (`wasmgc-classic`) is wired (§18A.13 step-1 WIRE, #32) and `engine_id`-multiplexed for N concurrent graphs (§18A.12, verified N=16); the named first-party adopter `iasbuilt/xldatagrid` (its 7 sync `createCausl()` sites, §18A.13) consumes the placed wasm engine in a Node.js TS production workload. The adopter attestation is recorded here per §18A.7 Criterion 4. |
| 5 | **TS-engine floor maintained** | **GO** | `backend: 'js'` / `engine: 'js-ssot'` stays supported and byte-identical: no §12 row removed, no new **required** `createCausl()` option (the promotion is a default-value flip, zero-codemod, zero adopter-facing API change), no perf regression the TS engine could not accept. The js-ssot floor stays available as the explicit per-graph opt AND as the value-of-record in `causl-core-ts` (which retains both engines + the differential oracle, §18A.10 / §18A.13). *(Superseded for `causl/causl-wasm-ts` at its 0.5.0, `causl/causl-wasm-ts#280`: `backend` and `engine` were removed there, so this criterion is no longer satisfiable in that repository and no longer binds it; §18A.7 Criterion 5 is sunset there per §18A.7's own row, and this GO verdict is the dated record of 2026-06-21. The verdict stands unchanged for `causl/causl-core-ts`, which still ships both engines.)* |

**Why this is the DEFAULT promotion and not the REPLACEMENT (scope, stated not hidden).** §18A.7 names two promotion tiers behind the same five-criteria gate: **default** (route through WASM first; TS stays present, as oracle + floor) and **replacement** (TS demoted from value-of-record). This amendment promotes the **default** only: the TS `#graph` stays the synchronous per-commit ORACLE every flush byte-compares the Rust projection against, and the `engine: 'js-ssot'` floor stays a one-line opt. *(Both halves of that sentence are dated 2026-06-21 and neither describes `causl/causl-wasm-ts` today: the §18A.8 amendment below removed the per-flush oracle, and 0.5.0 (`causl/causl-wasm-ts#280`) removed the `engine` option, so there is no one-line opt to take there. Read it as the record of what this amendment decided, not as the live disposition; it still describes `causl/causl-core-ts`.)* The **replacement** (deleting the TS oracle) remains gated on the §18A.3 structural-FFI lift that §18A.13's "DEFERRED; re-scoped" row sequences with the rust-ssot commit/write SSOT cutover (causl/causl-core-rs#169); it does not ride this default flip.

**Relationship to §18A.13 (the wasm-only strip).** §18A.13 (2026-06-19/20) committed `causl-wasm-ts` to ship wasm as its sole **public** engine by **governance bypass** of this gate, driven by complexity-elimination, scoped to that one repo. THIS amendment is the orthogonal, gate-SATISFYING decision: it flips the default `engine` canonicality for the WASM path **across both repos** because the five criteria are now genuinely met. The two compose; §18A.13 removed the TS engine from `causl-wasm-ts`'s public surface; this amendment makes the Rust engine the default value-of-record for the WASM path everywhere; and neither uses perf as a justification.

**Migration:** none required of adopters: the promotion is a default-value flip with **no adopter-facing API change** (zero new required options, zero codemod). Adopters who omit `engine` transparently get the Rust-authoritative engine with the TS oracle in lockstep and the sticky-downgrade fail-safe armed. Adopters who want the prior behaviour pass `engine: 'js-ssot'` explicitly (the retained floor opt, §18A.7 Criterion 5). *(Superseded for `causl/causl-wasm-ts` by §18A.13.1's withdrawal at 0.5.0: the `engine` option was removed there and `'js-ssot'` cannot be passed. The advice in this dated row stands for `causl/causl-core-ts`, which still ships both engines.)* The §18A.8 revert path is unchanged and still armed: a per-flush byte-divergence sticky-downgrades the affected graph to `js-ssot` (K=1, `RUST_SSOT_DOWNGRADE_ERROR_CODE`); default and non-diverging graphs are byte-unaffected. Re-run for confirmation: `pnpm -F @causl/causl-wasm-ts typecheck && pnpm -F @causl/causl-wasm-ts test:run` (GREEN at the new default; the wasm-v2.x byte-compare + sticky-downgrade suites pass and the downgrade still fires on injected divergence) and `pnpm -F @causl/causl-wasm-ts build && pnpm size`.

### §18A.8 Amendment: Commit/Write Rust-SSOT cutover end-remove-downgrade: the TS-vs-Rust per-flush byte-compare ORACLE and the V2.5 (#1544) sticky-downgrade FAIL-SAFE are REMOVED; `rust-ssot` is now UNCONDITIONAL (fail-safe-removal, causl/causl-core-rs#169, 2026-06-21)

**§18A.8.amendment: the dated fail-safe-removal decision.** This is the dated follow-up that RETIRES the safety scaffolding the 2026-06-21 promote-default flip (§18A.7.amendment, above) left armed. With all five §18A.7 criteria GO and the cross-backend determinism gate (§18A.1.1) proven at **0-byte divergence at an extended trial count** (100,000 trials, `maxCommands` 2000, seed `1718935200`, well beyond the 1000-trial GO floor), the per-flush TS-vs-Rust byte-compare oracle and the V2.5 (#1544) Decision-6 sticky-downgrade fail-safe are no longer load-bearing; they are removed. Effective this amendment, `engine: 'rust-ssot'` (the WASM-path default) applies the Rust `commit_batch` post-state to the WASM-side mirror and returns the Rust-derived `Commit` **UNCONDITIONALLY**: there is no per-flush byte-compare, no rollback, and no demotion to `js-ssot`. This is the `end-remove-downgrade` slice of the Commit/Write Rust-SSOT cutover; it is sequenced BEFORE the del-* TS-pipeline deletions (the byte-compare oracle ran the full TS `commitInternal` on every flush, so the TS pipeline is only dead AFTER this slice retires the oracle).

**Status:** SPEC text: this dated §18A.8 amendment row added to the §19 trail; the §18A.8 "Criterion-1 NO-GO" bullet (above) is amended to record that the sticky-downgrade fail-safe is retired and the CI-blocking cross-backend gate is now the SOLE guarantee and SOLE detector. The mechanical removal is in `packages/core/wasm/index.ts`: the `STICKY_DOWNGRADE_K` / `RUST_SSOT_DOWNGRADE_ERROR_CODE` / `RustSsotDowngradedError` / `v2ByteCompareResult` symbols and the `BatchedFlush` compare/promote/rollback/downgrade machinery (`#jsCommits`, `#shadowCompareCount`, `#promotedFlushCount`, `#divergedFlushCount`, `#consecutiveDivergences`, `#stickyDowngraded` + their getters) are deleted; the `WasmEngineMode` discriminant and the `engine: 'js-ssot'` FLOOR opt are RETAINED (§18A.7 Criterion 5). The tests that asserted the sticky-downgrade FIRES are re-pinned to "rust-ssot is unconditional" (NOT masked); the full `@causl/causl-wasm-ts` suite (1002 tests) is GREEN and the cross-backend gate stays CI-blocking at 0-divergence.

**Contract sentence:** the wasm engine is the unconditional canonical authority for the WASM-path default (`engine` omitted ⇒ `rust-ssot`): its `Commit` records, snapshots, structural-query results, and subscriber fire-order are returned directly, with the **cross-backend determinism gate** (§18A.1.1, 0-byte divergence, CI-blocking) as the sole conformance guarantee. The wrapped TS engine is RETAINED only as the SSOT for `read` / `subscribe` / `exportModel` and for structural/disposal validation; it is no longer the per-flush byte-compare oracle and is no longer a runtime fail-safe (its deletion as the oracle here is distinct from the del-4 deletion of the `#graph` definition itself). **[Superseded by Epic #170 / §18A.13.1 (2026-06-23); corrected by the review amendment 2026-07-06, M3.]** This "TS engine is the SSOT for `read`/`subscribe`/`exportModel` and structural/disposal validation" sentence is **no longer accurate**: Epic #170 lifted the four projection reads to Rust, so under `rust-ssot` **no adopter read consults the TS `entries` map** (`packages/core/wasm/authoritative.ts` is RUST-AUTHORITATIVE with no wrapped-TS `#graph` consult). The retained `createCauslTs` became a clean standalone capability-fallback engine under §18A.13.1, not wasm-path structural scaffolding; and that fallback role was itself **withdrawn** at `causl/causl-wasm-ts` 0.5.0, leaving the closure with no adopter-reachable role at all in that repository. Read §18A.13.1 for the current architecture; this sentence is preserved for the audit trail, not as live contract. The `engine: 'js-ssot'` floor stays a one-line opt and the value-of-record in `causl-core-ts` (§18A.7 Criterion 5; un-regressed; this amendment removes only the `rust-ssot` shadow-compare/downgrade machinery, never the floor).

**What this severs.** This removes the last live COMMIT consumer of the wrapped TS `#graph` as the per-flush oracle/fail-safe: under the prior promote-default flip the TS commit ran on every flush AND the per-flush byte-compare diffed the Rust projection against it (sticky-downgrading on divergence); now the Rust post-state is trusted directly. The TS `#graph` survives this amendment as the read/subscribe/exportModel SSOT and the structural-validation seam; its definition is deleted later in the del-* sequence (del-4), not here.

**Why removing the fail-safe is safe (the point-of-no-return justification).** The sticky-downgrade was the K=1 tripwire the §18A.8 revert condition was built on; removing it means a default graph has no in-process fall-back to the TS floor. The justification is the extended-confidence pre-check this amendment records: the cross-backend determinism gate (the SOLE remaining guarantee) passes **0-byte divergence at 100,000 trials** (seed `1718935200`, `maxCommands` 2000), raising confidence far beyond the 1000-trial GO floor before the fail-safe came out. A divergence is now a HALT-before-merge condition at the CI-blocking gate, not a self-healing runtime downgrade; the revert path becomes the governance revert of this amendment (re-pin the floor as the default), consistent with §18A.9's "flipping the value-of-record is a governance act, not a CI toggle."

**Relationship to §18A.13 and the del-* sequence.** §18A.13 (the `causl-wasm-ts` wasm-only strip) removed the TS engine from `causl-wasm-ts`'s public surface; the §18A.7.amendment (above) made the Rust engine the default value-of-record for the WASM path; THIS amendment removes the per-flush oracle + fail-safe so `rust-ssot` is unconditional. It is sequenced BEFORE the del-* TS-pipeline deletions precisely because the byte-compare oracle ran the full TS `commitInternal` on every flush: the TS commit pipeline is only provably dead once this slice stops running it as the oracle. None of these slices uses perf as a justification (consistent with §18A.7 Criterion 3 / §18A.13).

**Migration:** none required of adopters: `rust-ssot` was already the default and the adopter-facing `Commit` already derived from the Rust post-state on the promote path; this amendment only removes the per-flush oracle + fail-safe behind it (zero new required options, zero codemod, no adopter-visible API change). Adopters who want the pure-TS floor pass `engine: 'js-ssot'` explicitly (the retained opt, §18A.7 Criterion 5). *(Superseded for `causl/causl-wasm-ts` by §18A.13.1's withdrawal at 0.5.0: the `engine` option was removed there and `'js-ssot'` cannot be passed. The advice in this dated row stands for `causl/causl-core-ts`, which still ships both engines.)* Re-run for confirmation: `pnpm -F @causl/causl-wasm-ts typecheck && pnpm -F @causl/causl-wasm-ts test:run` (GREEN; the re-pinned wasm-v2.x suites assert `rust-ssot` is unconditional and the sticky-downgrade scenario is gone), the cross-backend determinism gate at an extended trial count (`CAUSL_FUZZ_TIER=nightly pnpm -F @causl/causl-wasm-ts exec vitest run test/properties/cross-backend-determinism.property.test.ts`, 0-divergence), and `pnpm -F @causl/causl-wasm-ts build && pnpm size` (the bundle DROPS; the compare/downgrade machinery is gone).

### §18A.13.1 Amendment (2026-06-23; WITHDRAWN at `causl/causl-wasm-ts` 0.5.0): the implicit `createCausl()` capability fallback, and its removal

**Read this section as a dated record, not as a live obligation.** Between 2026-06-23 and `causl/causl-wasm-ts` **0.5.0** this amendment partially reversed §18A.13's fail-loud stance for the implicit `createCausl()` path in `causl-wasm-ts`. That reversal has been **withdrawn**. Nothing below places a requirement on `causl/causl-wasm-ts` today; §18A.13's "a host where wasm cannot instantiate fails loud" sentence is once again the whole of the stance there, without qualification. The paragraphs are kept rather than deleted because a governance reversal that is itself reversed is exactly the kind of round trip a reader six months later has to be able to reconstruct: and because the cost it was written to avoid was **accepted**, not avoided, and somebody has to be able to find out that it was.

**What the amendment did while it was in force (2026-06-23 → 0.5.0).** On a host where the WasmGC engine could not instantiate, `createCausl()` **degraded to `createCauslTs()`**: the retained internal TS engine, reached through a `JsFallbackBackend`; with a loud one-time `console.warn` plus a structured telemetry event, never silently. The fallback was **implicit-path only**: `createCauslWasm()`, `createCauslWasmSync()` and an explicit `engine: 'rust-ssot'` continued to fail loud with `CAUSL_WASM_ENGINE_UNAVAILABLE`, on the reasoning that a consumer who explicitly asked for wasm must never silently run on JS. The justification recorded at the time was that `causl-wasm-ts` shipped browser bundles to enterprise fleets where a bounded WasmGC gap was concentrated in change-controlled Mac/iOS fleets and policy-pinned browsers, and that the degrade cost ~0 KB because the TS engine was already inside the bundle.

**Why it was withdrawn (`causl/causl-wasm-ts#280`, released as 0.5.0).** Two reasons, and the first is the load-bearing one:

- **The fallback engine and the primary disagreed.** `causl/causl-wasm-ts#272` established that the TS floor and `rust-ssot` answer differently on in-place mutation of a committed value: a §18A.1.1 MUST-be-identical surface. A fallback that returns different answers from the engine it stands in for is worse than no fallback: it converts a host-support failure into a correctness divergence, silently, on the path an adopter is least likely to be watching. The justification in the paragraph above ("the TS engine is already proven equivalent") was true when written and stopped being true; the amendment did not survive that.
- **The implicit path was also the accident path.** Measured on `causl/causl-wasm-ts#273`: a `createCausl()` with no `preloadCauslWasm()` constructed a working graph on the TS floor with **zero** warnings, zero telemetry markers and zero events. A project whose tests omitted the preload and whose production preloaded therefore ran two engines that disagree, with the *tests* on the wrong one. 0.4.0 made that path warn; 0.5.0 makes it throw.

**The accepted cost, named rather than buried.** Hosts below the declared engine floor now **hard-fail** at `createCausl()` in `causl/causl-wasm-ts`, with no flag that restores the previous behaviour. The floor is stated ONCE, on `WASM_HOST_FLOOR` in that repository's `packages/core/src/wasm-registry.ts`, and this amendment deliberately repeats no version numbers: the tier list it originally carried here (a Safari floor of 18 with a macOS-15 pairing, a pre-119 Chromium/WebView2 pin, and a Node ceiling of 20) was corrected by causl/causl-wasm-ts#426, whose feature bisect of the shipped artefact found the governing requirement to be typed function references, so two of the three original rows understated the floor (Safari 18.0-18.1 and Node 21 refuse too) and the macOS pairing carried no information.

That is a decided trade recorded by EPIC `causl/causl-wasm-ts#275` §5 and by the `## [0.5.0]` entry of that repository's `CHANGELOG.md`, which carries the failure contract and a numbered migration. An adopter shipping to a below-floor tier has to decide deliberately what an unsupported host should do, and write it in a `catch`.

**Specified, not observed while the amendment was in force; the probe landed later.** For the whole of this amendment's life the runtime capability **probe was never wired**: `detectBridge()` returned the `wasmgc-classic` baseline unconditionally (filed upstream as `iasbuilt/causl#691`), which is why the fallback was described as scaffolded/specified rather than a live guarantee, and why the WasmGC-unavailable row of the 0.5.0 failure contract shipped **specified, not observed**. causl/causl-wasm-ts#426 later resolved the placeholder by **wiring rather than deletion**: `detectBridge()` now validates a probe module built from the shipped artefact's measured minimal feature set (typed function references plus reference types, bulk memory, sign-extension and non-trapping float-to-int; the artefact declares zero WasmGC heap types) and throws the typed refusal, naming the floor `WASM_HOST_FLOOR` declares, on hosts that refuse it. There is still no degrade decision for the probe to feed; it moves the same refusal earlier and names the floor. What remains unclaimed is a measurement on a real below-floor host at the version boundary; `DISTRIBUTION.md` §7.3 states that gap.

**What survives the withdrawal, and until when.** `onCauslCapabilityFallback` and the types its callback names (`CauslCapabilityFallbackEvent`, `CauslTsFallbackReason`), `CauslEngineMode`'s `'js-ssot'` member and `CauslTsFloorReason`'s `'wasm-fallback'` member remain **exported, `@deprecated`, and never firing**, and are deleted at 0.6.0. Deleting them in the release that starts throwing would have handed an adopter who registered a listener, or who writes an exhaustive `switch` over either union, a build error on top of a behaviour change. §18A.13's own warn(N) → flip(N+1) → cleanup(N+2) discipline was followed rather than exempted: 0.4.0 warned, 0.5.0 flipped, 0.6.0 cleans up.

**The two live obligations this amendment carried, and where they went.**

- **§5.1 Amendment 7's disclosure obligation is discharged by removal.** Amendment 7 requires that, while the TS floor's `Object.is`-only container cutoff remains non-conformant, §18A.13.1 **MUST disclose** that the capability fallback over-notifies container subscribers relative to the wasm default, and that §18A.12's "byte-identical per §18A.1.1" justification for `fallbackToTs` is suspended for container-valued deriveds. In `causl/causl-wasm-ts` there is no longer a capability fallback to over-notify and no `fallbackToTs` to justify, so the obligation is satisfied by the withdrawal rather than by a disclosure. The underlying non-conformance is unaffected and is **not** discharged: it belongs to the TS floor, the floor now has exactly one owner in the org, and promoting its cutoff to structural is `causl/causl-core-ts`'s work (`causl-core-ts#58`). For `causl/causl-core-ts`, which still ships both engines, Amendment 7's disclosure obligation stands unchanged.
- **The not-preloaded third case (review 2026-07-06) is discharged by the throw.** That paragraph named a second, distinct silent-degrade route the original amendment did not cover: a `createCausl()` issued when no module had been preloaded; or before an in-flight `preloadCauslWasm()` resolved; took a benign not-preloaded branch and constructed the TS floor with no warn and no telemetry, indistinguishable to operations from a genuine capability degrade, and load-bearing because reference identity and commit-clock differ by engine (§18A.5 / §15.1). It required a distinct one-time signal separating "wasm active" from "TS floor via missing preload". **0.5.0 replaces the required signal with a thrown error**, which is strictly louder and carries the same distinction on `error.code`: `CAUSL_WASM_NOT_PRELOADED` for "the subpath is imported but nothing resolved"; the accident, whose remedy is a one-line boot-ordering change; against `CAUSL_WASM_ENGINE_UNAVAILABLE` for "the subpath was never imported, or this host cannot run the engine". The startup-race hazard the signal was meant to surface cannot arise: no process can hold graphs from two engines, because only one engine can be constructed. The adopter obligation the paragraph stated survives as the §18A.12 boot rule; sequence `await preloadCauslWasm()` before the first `createCausl()`; and is now enforced rather than advised.

**Scope, and the shared-SPEC status of this section.** The amendment applied to `causl/causl-wasm-ts`'s implicit path only; so does its withdrawal. `causl/causl-core-ts` keeps both engines, keeps the §18A.7 Criterion-5 floor, and decides its own disposition of a soft fallback path: nothing in this withdrawal binds it. This section is part of the shared SPEC and is intended to read the same in both repositories; "shared" is an intent rather than a verified property, because the two copies diverge today (§18A.10, and the divergence record in `DISTRIBUTION.md`). It is written in the "for `causl-wasm-ts`, X; for the TS-floor repo, Y" form for that reason, so it ports without forking.
