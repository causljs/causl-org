...
| `causljs/causl-client` | The **TypeScript API for `causl-wasm`** — the thin TS binding of §18A.4 (a binding over the `causl-wasm` FFI, **not** a copy of the TS engine) plus the loaders, setup, examples, and adoption docs that let TypeScript/Node.js web apps **integrate the placed** wasm engine. The build &amp; packaging tooling is **not** here — it is the Python tooling in `causl-wasm` (§18A.11); this repo consumes the placed `.wasm`. | The §12 `Graph` contract as a thin binding over the `causl-wasm` FFI (§18A.4), the `--target nodejs` loader hook (§18A.2) that resolves the placed artefact via `node:fs`, the wasm-adoption guide. Named first-party integration consumers: `iasbuilt/xldatagrid` and `iasbuilt/webapp` (Node.js TS web apps). |
| `causljs/causl-ts-wasm-engine` | The **`causl-ts`** reference engine — the TypeScript value-of-record / §13.8 unconditional floor — plus the conformance and historical R&D surface. | The `causl-ts` engine (the floor until all §18A.7 criteria are met), the cross-backend benchmark/conformance harness (the byte-identity gate's JS side, `cross-backend-determinism.property.test.ts`), and the historical wasm-cutover R&D. |

The `causl-wasm` engine named throughout §18A is produced by `causljs/causl-wasm`; `causl-client` ships no Rust source (§18A.4) precisely because the FFI source of truth lives in `causljs/causl-wasm`. This topology is descriptive — it records where the committed artefacts live; it does not alter the §18A.7 GO/NO-GO criteria or the §17.6 Commitment 15 surface.
...
**Evidence:** confirming benchmark re-run this session — `pnpm -F @causl/core build` then `pnpm exec tsx src/cli/run-bench.ts --suite taco-mutation --tier smoke --output /tmp/perf-resolve --json` (fresh process per cell, threads=1, `engine` measurement = full commit→recompute cascade, host Apple M5 / Node v26): **6/6 cells PASS the §14.2 RAIL responsiveness budget, 0 abs-breach, 0 regression-fail.** `causl-wasm` single-input-commit p50 0.568 ms / p95 2.5 ms (50 ms single budget); batch input-commit (200-wide, fixture cap) p50 9.5 ms / p95 70.2 ms (200 ms p95 batch budget); TACO 1000-cell-segment p50 4.8 ms / p95 23.4 ms (200 ms p95 batch budget). These end-to-end engine-exec figures (commit + derive cascade, NOT isolated marshal) sit inside the RAIL window; the isolated per-flush marshal cost (the quantity the 250 μs ceiling actually bounds) is the `op-rust-batch-boundary` 1/N-amortising tax above. The wasm/ts ratio (6.85×–28.84× p50 on this smoke run) is consistent with the standing 78×/~85× boundary-tax falsification — the real-Rust-SSOT engine (epic #1515, V2.4 gate validation) delivers its value on GC-pressure / large-tree axes (the V2.3 #1533 re-measurement: ~16.6× p99.9 flattening) orthogonal to the median per-commit cost the #1133 STOP-VERDICT falsified, not on parity. Cross-backend byte-identity (§18A.1.1) passes 1000 trials × 0-byte divergence (Criterion 1); the FFI-surface-completeness fixture (§18A.3) passes CI (Criterion 2's MECHANICAL anchor).
