...
- **Criterion-1 NO-GO:** byte-identity divergence at N≥10 trials on any extern (`commit_log`, adjacency, `engine_stats`, subscriber fire-order). Mirrors the V2.5 (#1544) Decision-6 sticky-downgrade fail-safe: at K=1 divergence the graph sticky-downgrades to `js-ssot` with the structured `RUST_SSOT_DOWNGRADE_ERROR_CODE`; default and non-diverging graphs are byte-unaffected.
