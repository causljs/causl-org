...
**Single-authoritative-instance constraint (cross-ref §18A.6).** Multiple `new WebAssembly.Instance` of one compiled `WebAssembly.Module` each have independent linear memory and thread-local `ENGINE_STATE`, but all funnel through the **one** module-level `wasm` slot in the shared bundler sidecar (`__wbg_set_wasm` overwrites it; last write wins). For the js-ssot default (wasm is a byte-identical shadow, TS is SSOT) this is benign and `createCauslWasmSync` returns graphs sharing one authoritative wasm engine. For ≥2 genuinely-independent authoritative (rust-ssot) graphs, the factory MUST either re-import a fresh sidecar per instance (cache-busted dynamic import → per-instance `wasm` slot) or use the serialize/replace-`ENGINE_STATE` multiplexing seam, and MUST fail-closed (assert at-most-one authoritative instance) until one of those lands — never silently alias two graphs onto one `ENGINE_STATE`.
...
- **IN PROGRESS (the deeper complexity win, committed — the *internal* TS-free milestone, beyond the public strip above):** the **fully-authoritative Rust engine** that owns the *structural* surface too — dependencies/dependents (+transitive), commit-log, commit-metadata, handle/disposal/StaleTx validation, stats — over the §18A.3 FFI surface, with **no JS-engine fallback for any §12 row** (`causljs/causl-wasm#142`); and true per-instance isolation for N authoritative graphs (`causljs/causl-wasm#143`, fail-closed until it lands). Today `causl-client`'s wasm path is still TS-structural-backed (the §18A.4 hybrid; only read/subscribe/commit-clock are wasm-authoritative under rust-ssot), so even after the CUT (#34) removed `createCauslTs` from the *public* surface, `createCauslTs` survives INTERNALLY (`src/graph.js`) as the structural scaffolding the wasm path wraps. The end state removes even that internal scaffolding so `causl-client` is **truly TS-free**. The §18A.3 FFI-surface-completeness fixture (`ffi_surface_complete.rs`) is the mechanical proof that the last TS-fallback row is gone — this amendment is honest that the *public* strip (steps 1–3, shipped) lands before the *internal* TS-free milestone (the §18A.3 lift, #142, in progress).
